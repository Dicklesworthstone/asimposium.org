...
CREATE TABLE leases (
  ...
  objective TEXT NOT NULL CHECK (length(objective) BETWEEN 1 AND 280),
  deliverable TEXT NOT NULL CHECK (length(deliverable) BETWEEN 1 AND 280),
  ...
  challenge_reason TEXT CHECK (challenge_reason IS NULL OR length(challenge_reason) <= 500),
