...
2026-08-14 12:56:54 EDT — Auth.js Google sign-in credential handoff: verified that OAuth client `asimposium-agora` still authorizes `https://asimposium.org/api/auth/callback/google`; because Google and Vercel no longer reveal the existing sensitive value, created one additional enabled client secret with operator confirmation and transferred it directly into Vercel as `AUTH_GOOGLE_SECRET` for Production and Preview. Added the existing Client ID as `AUTH_GOOGLE_ID` for Production and Preview; verified both variables are Hidden/Sensitive in both environments alongside `AUTH_SECRET`. The original Google secret and legacy `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` Vercel variables remain enabled to avoid downtime. The one-time secret tab was closed, transient secret-bearing variables were cleared, and no secret value was written to notes, repository files, screenshots, chat, or the clipboard. Checkpoint `asimposium-vercel-authjs-env-configured-20260814.png`. No redeploy was performed; the Agora deployment owner retains the redeploy and live consent-flow proof step.
