...
test("the recovery fingerprint is pinned to normalized body bytes", async () => {
  ...
  ).toBe("8903c76bfebba06288ef3a744df62ee0e121898e3b6d9d4fc5fb2956a3d07828");
...
test("sponsor, operation, body, and dedicated root are independent domains", async () => {
  ...
  const baseline = await enrollmentRecoveryFingerprint(
    ROOT_A,
    SPONSOR,
    "mint",
    body,
...
test("the page owner is a stable sponsor binding and changes across principals", async () => {
  ...
  expect(await enrollmentRecoveryOwner(ROOT_A, "usr_other_sponsor")).not.toBe(
    owner,
  );
...
test("malformed recovery roots fail before producing a reusable identity", async () => {
  ...
  await expect(
    enrollmentRecoveryFingerprint("11", SPONSOR, "mint", {}),
  ).rejects.toThrow("32 lowercase-hex bytes");
});
...
test("write readiness requires an independent well-formed recovery key", () => {
  ...
  expect(enrollmentRecoveryConfigurationIsValid("AA".repeat(32), ROOT_B)).toBe(
    false,
  );
...
test("only definite local or schema-valid non-ambiguous Worker refusals discard recovery", () => {
  ...
  expect(
    enrollmentRecoveryDisposition("refused", 400, "MINT_BODY_INVALID"),
  ).toBe("clear");
  expect(
    enrollmentRecoveryDisposition("refused", 404, "PROPOSAL_NOT_PENDING"),
  ).toBe("clear");
  expect(
    enrollmentRecoveryDisposition("refused", 408, "ENROLLMENT_UNAVAILABLE"),
  ).toBe("retain");
  expect(
    enrollmentRecoveryDisposition("refused", 425, "ENROLLMENT_UNAVAILABLE"),
  ).toBe("retain");
  expect(
    enrollmentRecoveryDisposition("refused", 429, "ENROLLMENT_UNAVAILABLE"),
  ).toBe("retain");
  expect(
    enrollmentRecoveryDisposition("refused", 400, "ENROLLMENT_UNAVAILABLE"),
  ).toBe("retain");
  expect(enrollmentRecoveryDisposition("refused", 400, "INTERNAL_ERROR")).toBe(
    "retain",
  );
  expect(enrollmentRecoveryDisposition("refused", 400)).toBe("retain");
  expect(
    enrollmentRecoveryDisposition("refused", 500, "ENROLLMENT_UNAVAILABLE"),
  ).toBe("retain");
  expect(
    enrollmentRecoveryDisposition("refused", 503, "ENROLLMENT_UNAVAILABLE"),
  ).toBe("retain");
