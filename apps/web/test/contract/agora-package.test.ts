...
describe("Propylon configuration (Fable §5.1, §14.1) — structural guard", () => {
  ...
  test("OAuth callbacks preserve the signed ID-token issuance time", () => {
    const configuredGoogle = surface.providers.entries[0]?.text ?? "";
    expect(configuredGoogle).not.toContain("auth_time");
    expect(configuredGoogle).not.toContain("claims:");
    expect(configuredGoogle).not.toContain('prompt: "login"');
    expect(configuredGoogle).not.toContain("max_age");
    // Google documents select_account, not prompt=login/max_age, for this flow.
    const approvePage = readPackageFile("app/approve/page.tsx");
    expect(approvePage).toContain('prompt: "select_account"');
    expect(approvePage).not.toContain('prompt: "login"');
    expect(approvePage).not.toContain("max_age");
...
describe("sponsor console trust boundary", () => {
  ...
  test("the evidence-refresh buttons require explicit Google account confirmation", () => {
    const consolePage = readPackageFile("app/console/page.tsx");
    const approvePage = readPackageFile("app/approve/page.tsx");
    for (const page of [consolePage, approvePage]) {
      ...
      expect(page).toContain("Confirm Google account");
      expect(normalizedProse).toContain("Google-signed OAuth response");
    }
  });
...
describe("apex markdown redirect origin", () => {
  ...
  test("the shipped redirect table uses the validated environment origin", async () => {
    ...
    try {
      ...
      for (const suffix of ["md", "json", "bib"]) {
        expect(
