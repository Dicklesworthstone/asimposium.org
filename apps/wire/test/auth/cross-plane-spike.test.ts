...
const OUTER_SUPERVISOR_PROGRAM = [
  ...
  "    if read -t 0 <&8 2>/dev/null; then",
  "      if IFS= read -r child_record <&8; then",
  ...
  "    fi",
  "  fi",
  "  if read -t 0 <&7 2>/dev/null; then",
  "    request_status=0",
  '    IFS="$tab" read -r request_kind request_token request_signal request_extra <&7 || request_status=$?',
  "    if (( request_status == 0 )); then",
  '      [[ "$request_token" == "$token" && -z "$request_extra" ]] || kill -KILL 0',
  '      case "$request_kind:$request_signal" in',
  '        "SIGNAL:TERM")',
  '          [[ "$ack_delay" == "0" ]] || { IFS= read -r -t 1 _ <&7 2>/dev/null || :; }',
  ...
  '          [[ "$kill_hold_delay" == "0" ]] || { IFS= read -r -t 1 _ <&8 2>/dev/null || :; }',
  "          kill -KILL 0",
  "          ;;",
  '        "DIE:")',
