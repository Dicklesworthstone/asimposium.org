...
import {
  LEDGER_MOVES_BOUNDARY, loadLedgerMoves, MOVE_QUEUE_MAX_PAGES,
  MOVE_REVIEW_HISTORY_SQL, reviewTargetKey, selectLedgerMoves,
...
function item(patch: Partial<ReviewQueueItem> = {}): ReviewQueueItem {
  return { problem_id: "P-DEMO", claim_id: "C-1", version: 2, cursor: 10,
    kind: "conjecture", statement: "Untrusted mathematical statement.", falsifier: "A counterexample.",
    disposition: "open", need: "independent-review", best_recorded_tier: "none",
    direct_dependents: 0, dependents_capped: false, author_fellow_id: "F-AUTHOR",
    author_sponsor_id: "SP-AUTHOR", created_at: "2026-09-01T00:00:00.000Z",
    read_url: "/p/P-DEMO/claims/C-1@2.md?through=10", ...patch };
}
function templateFor(move: "review" | "add-refuter"): MoveTemplate {
  return { move, title: "Test template", trigger: "Test trigger", description: "Test description",
    availability: "available", target_contract: `/schemas/sessions.v1.json#/properties/${move === "review" ? "review" : "evidence"}_request`,
    request: { method: "POST", path: `/v1/sessions/{id}/${move === "review" ? "review" : "evidence"}`,
      auth: "fellow-bearer", idempotency_key_required: true },
    required_fields: ["body_md"], prefilled_hints: move === "review" ? {} : { direction: "refutes" } };
}
...
function queue(items: ReviewQueueItem[], patch: Partial<ReviewQueueResponse> = {}): ReviewQueueResponse {
  return { schema: "https://a.asimposium.org/schemas/review-queue.v1.json", policy: "review-discovery-v1",
    problem: "P-DEMO", candidates: items, scanned: items.length, next_after: null,
    selection_boundary: "test" as ReviewQueueResponse["selection_boundary"], omitted: [], ...patch };
}
function historyDb(rows: { claim_id: string; version: number }[] = []) {
  let reads = 0;
  const db = { prepare(sql: string) {
    assert.equal(sql, MOVE_REVIEW_HISTORY_SQL); reads += 1;
...
test("untrusted scientific prose and supplied URLs cannot enter trusted recommendation text", () => {
  ...
  const move = choose([item({ statement: attack, falsifier: attack, read_url: "https://attacker.invalid" })])[0];
...
for (const patch of [{ author_fellow_id: viewer.fellowId }, { author_sponsor_id: viewer.sponsorId }]) {
  test("author and same-sponsor work cannot masquerade as an independent review " + JSON.stringify(patch), () => {
...
for (const allowed of [
  ...
  assert.deepEqual(choose([item(), item({ claim_id: "C-2", need: "falsification-attempt" })], allowed), []);
});
test("review permission does not require permission to promote new claims", () => {
  ...
  assert.equal(choose([item({ need: "falsification-attempt" })], { session_open: true, review: true, promote: false }).length, 0);
});
test("authors may attempt falsification without manufacturing independent review credit", () => {
  const move = choose([item({ need: "falsification-attempt", author_fellow_id: viewer.fellowId })])[0];
  assert.equal(move?.move, "add-refuter");
  assert.deepEqual(move?.contract.prefilled_hints, { direction: "refutes", bears_on_kind: "claim", bears_on_id: "C-1", bears_on_version: 2, mode: "confirmatory" });
...
test("ranking preserves consequence ahead of move kind or author popularity", () => {
  const moves = choose([item(), item({ claim_id: "C-2", direct_dependents: 5, need: "falsification-attempt" })]);
...
test("duplicate input targets do not create repeated alternatives or mutate inputs", () => {
  const input = [item(), item()]; const before = JSON.stringify(input);
  assert.equal(choose(input).length, 1); assert.equal(JSON.stringify(input), before);
});
test("unavailable templates cannot become executable moves", () => {
  assert.deepEqual(selectLedgerMoves([item()], viewer, permissions, new Set(), () => ({
    move: "review", title: "Unavailable", trigger: "None", description: "None", availability: "unavailable",
    unavailable_reason: "Not mounted", next_step: "Read the record" })), []);
});
test("wrong template kind cannot be substituted", () => {
  assert.deepEqual(selectLedgerMoves([item()], viewer, permissions, new Set(), () => templateFor("add-refuter")), []);
});
test("permission refusal performs no queue or database reads", async () => {
  const h = historyDb();
  const result = await loadLedgerMoves(h.db, "P-DEMO", viewer, {}, {
    loadQueue: async () => { throw new Error("must not read"); }, templateFor });
  assert.deepEqual(result.moves, []); assert.equal(h.reads(), 0); assert.equal(result.degraded, false);
});
test("empty or filtered admission pages advance to useful later work", async () => {
  const h = historyDb(); const after = "2026-09-01T00:00:00.000Z|E-1"; const seen: unknown[] = [];
  const result = await loadLedgerMoves(h.db, "P-DEMO", viewer, permissions, {
    loadQueue: async (_db, query) => { seen.push(query); return seen.length === 1
      ? queue([], { scanned: 8, next_after: after, omitted: [{ reason: "not_review_ready", count: 8 }] })
      : queue([item()]); }, templateFor });
  assert.deepEqual(seen, [{ problem: "P-DEMO" }, { problem: "P-DEMO", after }]);
  assert.equal(result.moves.length, 1); assert.equal(result.degraded, false);
});
test("pagination has a hard bound and exposes the next discovery page", async () => {
  const h = historyDb(); let pages = 0;
  const result = await loadLedgerMoves(h.db, "P-DEMO", viewer, permissions, {
    loadQueue: async () => queue([], { scanned: 8, next_after: `2026-09-01T00:00:00.000Z|E-${++pages}` }), templateFor });
  assert.equal(pages, MOVE_QUEUE_MAX_PAGES); assert.equal(result.degraded, true);
...
]) test("queue scope cannot cross the requested problem " + JSON.stringify(invalid.problem), async () => {
  const h = historyDb();
  await assert.rejects(loadLedgerMoves(h.db, "P-DEMO", viewer, permissions,
    { loadQueue: async () => invalid, templateFor }), /MOVE_QUEUE_SCOPE_MISMATCH/);
});
test("repeating continuation fails rather than looping", async () => {
  await assert.rejects(loadLedgerMoves(historyDb().db, "P-DEMO", viewer, permissions, {
    loadQueue: async () => queue([], { next_after: "2026-09-01T00:00:00.000Z|E-1" }), templateFor }), /CURSOR_NOT_ADVANCING/);
});
test("duplicate admissions on separate pages fail rather than mix versions", async () => {
  await assert.rejects(loadLedgerMoves(historyDb().db, "P-DEMO", viewer, permissions, {
    loadQueue: async () => queue([item()], { next_after: "2026-09-01T00:00:00.000Z|E-1" }), templateFor }), /DUPLICATE_ADMISSION/);
});
test("missing content is reported as degraded, not empty successful discovery", async () => {
  const result = await loadLedgerMoves(historyDb().db, "P-DEMO", viewer, permissions, {
    loadQueue: async () => queue([], { omitted: [{ reason: "content_unavailable", count: 1 }] }), templateFor });
  assert.equal(result.degraded, true); assert.deepEqual(result.moves, []);
});
test("unmatched review-history rows fail closed", async () => {
  await assert.rejects(loadLedgerMoves(historyDb([{ claim_id: "C-99", version: 2 }]).db, "P-DEMO", viewer, permissions,
    { loadQueue: async () => queue([item()]), templateFor }), /MOVE_REVIEW_HISTORY_INVALID/);
});
...
for (const [label, change, expected] of [
  ...
  test(`SQLite review history: ${label}`, () => {
    ...
    try {
      ...
      const rows = db.query(MOVE_REVIEW_HISTORY_SQL).all(JSON.stringify([{ claim_id: "C-1", version: 2, cursor: 10 }]), "P-DEMO", "F-READER", 2);
