...
test("inbox links only target canonical invitation records, never work-product text", () => {
  ...
  for (const bad of [null, undefined, "C-1", "https://example.test/", "RR-../../secret", id + "\n"]) {
...
test("disclosed invitation operations all require Fellow authentication", () => {
  ...
  assert.equal(REVIEW_REQUEST_OPERATIONS.filter(row => row[0].startsWith("POST") && row[3]).length, 2);
});
test("OpenAPI distinguishes creation receipts from participant reads", () => {
  ...
  const post = reviewRequestResponses(path, origin, "POST") as any;
  assert.ok(post["201"]); assert.ok(!post["200"]);
  assert.equal(post["201"].content["application/json"].schema.$ref, `${origin}/schemas/review-requests.v1.json#/properties/receipt`);
  const get = reviewRequestResponses(path, origin, "GET") as any;
  assert.equal(get["200"].content["application/json"].schema.$ref, `${origin}/schemas/review-requests.v1.json#/properties/response`);
  const one = reviewRequestResponses(`${path}/{requestId}`, origin, "GET") as any;
  assert.equal(one["200"].content["application/json"].schema.$ref, `${origin}/schemas/review-requests.v1.json#/properties/view`);
});
test("write replay header and private cursor appear only on their own routes", () => {
  const path = "/v1/p/{problem}/review-requests", origin = "https://a.asimposium.org";
  assert.equal((reviewRequestParameters(path, origin, "POST")[0] as any).name, "Idempotency-Key");
  assert.equal((reviewRequestParameters(path, origin, "GET")[0] as any).name, "after");
