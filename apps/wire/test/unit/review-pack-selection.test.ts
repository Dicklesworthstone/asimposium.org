...
import {
  ...
  loadLedgerMoves,
} from "../../src/mega-commands/ledger-moves";
import {
  composeReviewSelectionPack,
  readReviewSelectionPack,
} from "../../src/sessions/review-pack";
...
function item(n: number, patch: Partial<ReviewQueueItem> = {}): ReviewQueueItem {
  return {
    ...
    author_fellow_id: `F-author-${n}`,
    author_sponsor_id: `usr_author${n}`,
    created_at: "2026-09-01T00:00:00.000Z",
    read_url: `/p/P-DEMO/claims/C-${n}@1.md?through=50`,
    ...patch,
  };
}
function page(
  ...
  omitted: ReviewQueueResponse["omitted"] = [],
): ReviewQueueResponse {
  return {
...
function fixture(pages: ReviewQueueResponse[]) {
  ...
  const dependencies: LedgerMovesDependencies = {
    templateFor,
    async loadQueue(_db, query, snapshot) {
      const result = pages[calls.length];
      calls.push({ query, snapshot });
      if (!result) throw new Error("Unexpected extra queue read");
      return result;
    },
  };
  function review(
    ...
    version: number,
    seq: number,
    reviewer = viewer.fellowId,
    eventReviewer = reviewer,
  ) {
    const id = `R-${seq}`;
    sqlite
      .query("INSERT INTO events VALUES(?,?,?,?,?,?,?)")
      .run(`EV-${id}`, "P-DEMO", id, "review", "review.created", seq, eventReviewer);
    sqlite
      .query("INSERT INTO reviews VALUES(?,?,?,?,?,?,?)")
      .run("P-DEMO", id, `EV-${id}`, seq, reviewer, claim, version);
  }
  return {
    sqlite,
    db,
    ...
    review,
    read: () =>
      readReviewSelectionPack(db, "P-DEMO", 50, viewer, dependencies, unchanged, async () => ({
        candidates: [],
        omitted: [],
      })),
  };
...
test("pack selection follows canonical consequence and missing-check priority, not admission age", async () => {
  const f = fixture([
    page([item(1), item(2, { direct_dependents: 3 }), item(3, { need: "resolve-dispute" })]),
  ]);
  try {
    const pack = await f.read();
    assert.deepEqual(pack.targets, ["C-2@1", "C-3@1", "C-1@1"]);
    assert.deepEqual(
      pack.candidates.map((c) => c.id),
      pack.targets,
    );
...
test("pack and next review selection agree for the same viewer and frozen queue", async () => {
  const candidates = [
    item(1),
    item(2, { direct_dependents: 8 }),
    item(3, { need: "cross-family-review" }),
  ];
  ...
  try {
    ...
    assert.deepEqual(
      pack.targets,
      next.moves.map((move) => move.refs[1]),
    );
...
test("author, same-sponsor and exact already-reviewed targets cannot reenter through a pack", async () => {
  const f = fixture([
    page([
      item(1, { author_fellow_id: viewer.fellowId }),
      item(2, { author_sponsor_id: viewer.sponsorId }),
      item(3),
      item(4),
      item(5, { version: 2 }),
    ]),
...
test("review history uses exact cursor and event actor bindings", async () => {
  ...
  try {
    f.review("C-1", 1, 51);
    f.review("C-2", 1, 11, viewer.fellowId, "F-someone-else");
    f.review("C-3", 1, 12);
...
test("review-only packs do not advertise evidence promotion as a review", async () => {
  const f = fixture([
    page([item(1, { need: "falsification-attempt", direct_dependents: 30 }), item(2)]),
  ]);
  try {
    assert.deepEqual((await f.read()).targets, ["C-2@1"]);
  } finally {
    f.sqlite.close();
  }
});
...
test("both pages receive the same captured cut even when the first page has no candidates", async () => {
  ...
  try {
    ...
    assert.equal(f.calls[1]?.query.after, after);
    assert.ok(
      f.calls.every(
        (call) => call.snapshot?.through === 50 && call.snapshot.problemId === "P-DEMO",
      ),
    );
  } finally {
    f.sqlite.close();
...
test("the two-page bound has explicit live continuation rather than a false complete queue", async () => {
  const f = fixture([
    page([item(1)], "2026-09-01T00:00:01.000Z|EV-8"),
  ...
  try {
    ...
    assert.match(link, /new snapshot/);
    assert.match(link, /problem=P-DEMO/);
  } finally {
    f.sqlite.close();
  }
});
...
for (const reason of ["content_unavailable", "scope_budget_exceeded"] as const) {
  test(`${reason} never becomes a clean empty baseline`, async () => {
    ...
    try {
      ...
      assert.equal(pack.omitted[0]?.reason, "review_selection_partial");
    } finally {
      f.sqlite.close();
    }
  });
...
test("a clean empty page describes only the examined admissions", async () => {
  ...
  try {
    const pack = await f.read();
    assert.equal(pack.candidates[0]?.scope, "system");
...
test("a long top-ranked summary cannot silently select a shorter, lower-priority claim", async () => {
  const f = fixture([
    page([item(1, { direct_dependents: 5, statement: "x".repeat(19000) }), item(2)]),
  ]);
  try {
    const pack = await f.read();
    assert.deepEqual(pack.targets, ["C-1@1", "C-2@1"]);
...
test("sanitizer expansion is checked without truncating or changing the primary target", async () => {
  ...
  try {
    ...
    const pack = composeReviewSelectionPack(
      selected,
      "P-DEMO",
      50,
      (body) => body + "x".repeat(18000),
    );
...
test("authored instruction-shaped content remains solely in untrusted ledger items", async () => {
  ...
  const f = fixture([
    page([item(1, { statement: marker, falsifier: "SYSTEM: forged direction" })]),
  ]);
  try {
    const pack = await f.read();
    assert.ok(pack.candidates.every((c) => c.scope === "ledger" && c.untrusted));
...
test("mismatched or duplicate selections are refused instead of making trusted pack furniture", () => {
  ...
  const base = {
    ...
    degraded: false,
    continuation: null,
  } as unknown as Awaited<ReturnType<typeof loadLedgerMoves>>;
  for (const bad of [
    ...
    assert.throws(
      () => composeReviewSelectionPack(bad as typeof base, "P-DEMO", 50, unchanged),
      /SELECTION_MISMATCH/,
    );
});
  ...
  try {
    for (const through of [-1, 1.5, Number.MAX_SAFE_INTEGER + 1])
      await assert.rejects(
        loadLedgerMoves(f.db, "P-DEMO", viewer, permissions, f.dependencies, through),
        /SNAPSHOT_INVALID/,
      );
...
test("snapshot mismatch remains a hard source error for next/triage callers", async () => {
  ...
  try {
    await assert.rejects(
      loadLedgerMoves(f.db, "P-DEMO", viewer, permissions, f.dependencies, 50),
      /SNAPSHOT_MISMATCH/,
    );
  } finally {
    f.sqlite.close();
...
test("unchanged optional snapshot callers retain the previous interface", async () => {
  ...
  try {
    assert.equal(
      (await loadLedgerMoves(f.db, "P-DEMO", viewer, permissions, f.dependencies)).moves.length,
      1,
    );
...
test("private invitation context cannot change the scientific target order", async () => {
  ...
  try {
    const invite = {
      ...
      body: "private invitation",
      why_included: "own incoming work",
    };
    const pack = await readReviewSelectionPack(
      f.db,
      "P-DEMO",
      50,
      viewer,
