...
import type { MoveTemplate, HelloAssignment } from "@asimposium/contracts";
...
const credential = { fellowId: "F-viewer", sponsorId: "usr_viewer", grantedResources: {},
  credentialId: "TK-viewer" } as FellowCredentialBinding;
...
function response(problem: string, cursor: number): ReviewQueueResponse {
  return { schema: "https://a.asimposium.org/schemas/review-queue.v1.json", policy: "review-discovery-v1",
    problem, candidates: [{ problem_id: problem, claim_id: "C-1", version: 1, cursor,
      kind: "conjecture", statement: "Explicit test statement", falsifier: "Explicit test falsifier",
      ...
      author_sponsor_id: "usr_author", created_at: "2026-09-01T00:00:00.000Z",
      read_url: `/p/${problem}/claims/C-1@1.md?through=${cursor}` }],
    scanned: 1, next_after: null, omitted: [], selection_boundary: "Explicit unit fixture" };
}
function fixture(options: { wrongCursor?: boolean; firstEmpty?: boolean; deny?: boolean } = {}) {
  ...
  const db = { prepare(sql: string) { return { bind(...bindings: (string | number)[]) {
    return { async first() { return sqlite.query(sql).get(...bindings) ?? null; },
  ...
  const dependencies: LiveMovesDependencies = { now: () => 1000, templateFor: template,
    firstClaimTemplate: () => template("state-claim"),
    authorize: ((input: { effect: string }) => ({ decision: options.deny || input.effect === "promote" ? "deny" : "allow" })) as LiveMovesDependencies["authorize"],
    async loadQueue(_db, query, snapshot) {
      calls.push({ ...query, snapshot });
      // Simulate a ledger append after membership captured its head.
      sqlite.query("UPDATE problems SET public_seq = public_seq + 1 WHERE id = ?").run(query.problem);
      const latest = sqlite.query("SELECT public_seq FROM problems WHERE id = ?").get(query.problem) as { public_seq: number };
      const cut = options.wrongCursor ? latest.public_seq : snapshot?.through ?? latest.public_seq;
      const page = response(query.problem, cut);
      if (options.firstEmpty && !query.after) return { ...page, candidates: [], scanned: 8,
        next_after: "2026-09-01T00:00:01.000Z|EV-8" };
      return page;
    } };
  const provider = new LedgerMovesProvider(dependencies);
  return { sqlite, db, calls, provider,
    next: () => provider.nextMoves({ problemId: "P-DEMO", fellowId: credential.fellowId,
      credential, db, role: "steward", effectivePermissions: { promote: true } }) };
}
...
test("an empty first page does not advance the captured problem cursor", async () => {
  const f = fixture({ firstEmpty: true }); try {
    const result = await f.next(); assert.equal(result.primaryMove?.move, "review");
    assert.equal(f.calls.length, 2); assert.ok(f.calls.every(c => c.snapshot?.through === 50));
    assert.match(JSON.stringify(result.primaryMove), /through=50/);
  } finally { f.sqlite.close(); }
});
...
test("triage preserves a distinct captured cursor for each assigned problem", async () => {
  const f = fixture(); try {
    const result = await f.provider.triageMove({ fellowId: credential.fellowId, credential, db: f.db,
      assignments: [{ problem_id: "P-OTHER" }, { problem_id: "P-DEMO" }] as HelloAssignment[] });
    assert.equal(result.degraded, false); assert.equal(result.move?.refs[0], "P-DEMO");
    assert.deepEqual(f.calls.map(c => [c.problem, c.snapshot?.through]).sort(), [["P-DEMO", 50], ["P-OTHER", 90]]);
  } finally { f.sqlite.close(); }
});

test("a source that returns a newer cursor cannot publish a mixed-snapshot recommendation", async () => {
  const f = fixture({ wrongCursor: true }); try {
    const result = await f.next(); assert.equal(result.primaryMove, null);
    assert.equal(result.degraded, true); assert.equal(result.degradedReason, "MOVES_UNAVAILABLE");
  } finally { f.sqlite.close(); }
});

test("preflight refusal still prevents source access and ignores caller permission hints", async () => {
  const f = fixture({ deny: true }); try {
    const result = await f.next(); assert.equal(result.primaryMove, null); assert.equal(f.calls.length, 0);
  } finally { f.sqlite.close(); }
});
