...
import {
  ...
  type ReviewMetadata,
  type ReviewQueueScience,
  readReviewQueue,
  verifiedReviewMetadata,
} from "../../src/discovery/review-queue-read";
import {
  REVIEW_QUEUE_DISCOVERY_SQL,
  REVIEW_QUEUE_METADATA_SQL,
} from "../../src/discovery/review-queue-sql";
...
function fixture() {
  ...
  function claim(
    ...
    sql
      .prepare("INSERT INTO claim_versions VALUES (?,?,?,?,?,?,?)")
      .run(p, id, version, "conjecture", statement, falsifier, contentDigest);
    return {
      claim_id: id,
      version,
      event_id: event,
  ...
  const science: ReviewQueueScience = {
    prepare: (db, _p, _c, _n, target) =>
      db.prepare("SELECT ? AS target_version").bind(target.version),
    fold: async (rows) => ({
      disposition: "open",
  ...
  const discover = () =>
    sql.prepare(REVIEW_QUEUE_DISCOVERY_SQL).all(null, null, "", "", "", 9) as ReviewAdmission[];
  const metadata = (a: ReviewAdmission) =>
    sql
      .prepare(REVIEW_QUEUE_METADATA_SQL)
      .get(a.cursor, a.cursor, a.cursor, a.head_id, a.problem_id, a.cursor) as
      | ReviewMetadata
      | null
      | undefined;
...
test("real SQLite: queue composes source-verified statements and exact links", async () => {
  ...
  try {
    ...
    assert.equal(response.next_after, null);
  } finally {
    f.sql.close();
  }
});
for (const mode of ["private", "unlisted", "resolved", "unpublished"] as const) {
  test(`real SQLite: ${mode} work is absent from discovery`, async () => {
    ...
    try {
      f.problem(
        "P-HIDDEN",
        mode === "private" ? "private-draft" : mode === "resolved" ? "resolved" : "active",
        mode === "unlisted" ? 1 : 0,
      );
...
test("real SQLite: published revision supplies new bytes but keeps original authorship", async () => {
  ...
  try {
    f.problem();
    f.claim("C-1", 1, { sponsor: "SP-ORIGINAL" });
    f.claim("C-1", 4, {
      version: 2,
      sponsor: "SP-EDITOR",
      statement: "A revised exact statement.",
    });
    const result = await readReviewQueue(f.db, {}, f.science);
...
test("real SQLite: later unpublished revision cannot displace the public head", async () => {
  ...
  try {
    f.problem();
    f.claim("C-1", 1);
    f.claim("C-1", 4, { version: 2, statement: "Not yet public." });
...
for (const change of [
  ...
  test(`real SQLite: ${change} is excluded without publishing retained bytes`, async () => {
    ...
    try {
      f.problem();
      f.claim("C-1", 1);
      const head = f.claim("C-1", 4, {
        version: 2,
        statement: "CANARY unavailable scientific text",
      });
      if (change === "redact-head")
        f.sql
          .prepare("UPDATE event_content SET redacted_at='removed' WHERE event_id=?")
          .run(head.event_id);
      if (change === "redact-author")
        f.sql
          .prepare("UPDATE event_content SET redacted_at='removed' WHERE event_id<>?")
          .run(head.event_id);
      if (change === "tamper-body")
        f.sql
          .prepare("UPDATE event_content SET payload_json=payload_json || ' ' WHERE event_id=?")
          .run(head.event_id);
      if (change === "tamper-falsifier")
        f.sql.exec("UPDATE claim_versions SET falsifier='Different falsifier'");
...
test("real SQLite: old captured head survives a concurrent revision", async () => {
  ...
  try {
    f.problem();
    f.claim("C-1", 1);
    f.sql.exec("UPDATE problems SET public_seq=1");
    const row = f.discover()[0];
    assert.ok(row);
    f.claim("C-1", 4, { version: 2 });
    f.sql.exec("UPDATE problems SET public_seq=4");
    const old = f.metadata(row);
    assert.ok(old);
    assert.equal(old.version, 1);
    assert.equal(await verifiedReviewMetadata(row, old), true);
  } finally {
    f.sql.close();
  }
});
test("real SQLite: a fully excluded page still advances past the scanned prefix", async () => {
  ...
  try {
    f.problem();
    for (let i = 1; i <= 10; i += 1) f.claim(`C-${i}`, i);
...
test("real SQLite: old dependent versions and malformed pins do not invent consequence", async () => {
  ...
  try {
    f.problem();
    const premise = f.claim("C-1", 1);
    f.claim("C-2", 2, { pins: [premise] });
    f.claim("C-2", 3, { version: 2, pins: [] });
...
test("real SQLite: oversized histories are omitted before scientific replay starts", async () => {
  ...
  try {
    ...
    let replayed = 0;
    const result = await readReviewQueue(
      f.db,
      {},
      {
        ...f.science,
...
test("real SQLite: missing falsifier is not presented as a ready conjecture", async () => {
  ...
  try {
    f.problem();
    f.claim("C-1", 1, { falsifier: null });
    const result = await readReviewQueue(f.db, {}, f.science);
    assert.equal(result.candidates.length, 0);
    assert.equal(result.omitted[0]?.reason, "not_review_ready");
  } finally {
    f.sql.close();
  }
});
