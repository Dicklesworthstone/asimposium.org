...
function fixture() {
  ...
  class Statement {
    ...
    rows() {
      return sqlite.query(this.sql).all(...this.bindings);
    }
    async all() {
  ...
  const db = {
    ...
    async batch(statements: Statement[]) {
      return sqlite.transaction(() =>
        statements.map((statement) => ({ results: statement.rows() })),
      )();
    },
  ...
  return {
    ...
    at,
    prepares: () => prepares,
    read: (through: number, after?: string) =>
      readReviewAdmissions(
        db,
...
test("a captured queue keeps the earlier head instead of borrowing a later revision", async () => {
  ...
  try {
    f.add(1, { claim: "C-1" });
    f.add(2);
    f.add(3, { claim: "C-1", version: 2 });
    const old = await f.read(2);
    assert.deepEqual(
      old.map((r) => [r.claim_id, r.version, r.cursor, r.scope_events]),
      [
        ["C-1", 1, 2, 2],
        ["C-2", 1, 2, 2],
      ],
...
test("pagination retains the same cut across new admissions and revisions", async () => {
  ...
  try {
    ...
    const first = await f.read(12);
    assert.equal(first.length, 9);
    const eighth = first[7]!;
    const after = reviewQueueAfter(eighth.created_at, eighth.admission_id);
    f.add(13, { at: f.at(9) });
    f.add(14, { claim: "C-10", version: 2 });
    const second = await f.read(12, after);
    assert.deepEqual(
      second.map((r) => r.claim_id),
      ["C-9", "C-10", "C-11", "C-12"],
    );
...
test("scope byte and event budgets count the captured ledger, not later traffic", async () => {
  ...
  try {
    ...
    assert.equal(frozen[0]?.scope_events, 1);
    assert.equal(frozen[0]?.scope_bytes, 2);
    const live = await readReviewAdmissions(f.db, { problem: "P-DEMO" });
    assert.equal(live[0]?.scope_events, 513);
    assert.ok(live[0]!.scope_bytes > 1024 * 1024);
  } finally {
    f.sqlite.close();
  }
});
...
for (const posture of ["private-draft", "resolved", "retired", "archived", "unlisted"]) {
  test(`present-day ${posture} prevents a frozen recommendation read`, async () => {
    ...
    try {
      f.add(1);
      assert.equal((await f.read(1)).length, 1);
      if (posture === "unlisted")
...
test("future and missing snapshots are not reported as empty scientific queues", async () => {
  ...
  try {
    f.add(1);
    await assert.rejects(f.read(2), /SNAPSHOT_UNAVAILABLE/);
    await assert.rejects(
      readReviewAdmissions(f.db, { problem: "P-MISSING" }, { problemId: "P-MISSING", through: 0 }),
...
test("zero is a valid captured empty baseline; unpublished envelopes stay out", async () => {
  const f = fixture();
  try {
    f.add(1, { publish: false });
    assert.deepEqual(await f.read(0), []);
...
test("invalid cuts and cross-problem snapshots fail before any database access", async () => {
  ...
  try {
    for (const through of [-1, 0.5, NaN, Infinity, Number.MAX_SAFE_INTEGER + 1])
      await assert.rejects(f.read(through), /SNAPSHOT_INVALID/);
    await assert.rejects(
      readReviewAdmissions(f.db, { problem: "P-OTHER" }, { problemId: "P-DEMO", through: 0 }),
      /SNAPSHOT_INVALID/,
    );
    await assert.rejects(
      readReviewAdmissions(f.db, {}, { problemId: "P-DEMO", through: 0 }),
      /SNAPSHOT_INVALID/,
    );
...
test("same local claim IDs cannot cross problem boundaries", async () => {
  ...
  try {
    f.add(1, { claim: "C-1" });
    f.add(1, { problem: "P-OTHER", claim: "C-1" });
    f.add(2, { problem: "P-OTHER", claim: "C-1", version: 2 });
    const rows = await f.read(1);
    assert.equal(rows.length, 1);
    assert.equal(rows[0]?.problem_id, "P-DEMO");
...
test("unavailable bodies remain admissions rather than changing page traversal", async () => {
  ...
  try {
    ...
    const rows = await f.read(10);
    assert.equal(rows.length, 9);
    assert.equal(rows[0]?.scope_events, 10);
    assert.equal(rows[0]?.scope_bytes, 0);
    const eighth = rows[7]!;
    assert.deepEqual(
      (await f.read(10, reviewQueueAfter(eighth.created_at, eighth.admission_id))).map(
        (r) => r.claim_id,
      ),
      ["C-9", "C-10"],
    );
