...
import {
  REVIEW_MATCH_NO_ACTIVE_SQL,
  REVIEW_MATCH_RECIPIENT_GUARD_SQL,
} from "../../src/review-requests/matching-sql.ts";
import {
  MATCH_AUTHOR,
  matchFellow,
  matchHash,
  matchTestDependencies,
...
test("matching chooses a different sponsor/family without interpreting model labels", async () => {
  ...
  try {
    ...
    f.sqlite
      .query("UPDATE enrollment_fellows SET model='beta/99', harness='another' WHERE fellow_id=?")
      .run(matchFellow(1));
    const match = await f.match();
    assert.equal(match?.reviewerId, matchFellow(2));
    assert.equal(match?.reviewerSponsor, "usr_2");
    assert.ok(match?.provenancePin.digest);
    assert.equal(
      f.rows("SELECT last_used_at FROM fellow_tokens WHERE credential_id='cred-2'")[0]
        ?.last_used_at,
      20,
    );
...
test("same-sponsor Fellows and the author cannot be selected", async () => {
  const f = reviewMatchingFixture();
  try {
    f.publish(f.person(1, "usr_author"), "beta");
    f.publish(MATCH_AUTHOR, "beta");
...
test("no declared author family means no inferred match and no roster scan", async () => {
  ...
  try {
    ...
    assert.equal(
      await f.match(matchTestDependencies, {
        ...f.target,
        pin: { event_id: "EV-1", digest: matchHash(body), payload_json: body },
      }),
      null,
    );
...
test("withdrawn or missing latest provenance cannot fall back to an older different-family declaration", async () => {
  for (const damage of [
    ...
    const f = reviewMatchingFixture();
    try {
      const one = f.person(1);
      f.publish(one, "beta");
      const latest = f.publish(
        one,
        damage === "missing-family" ? null : damage === "same-family" ? "alpha" : "beta",
        damage === "changed-sponsor" ? { sponsor: "usr_old" } : {},
      );
      if (damage === "redacted")
        f.sqlite
          .query("UPDATE event_content SET redacted_at='now' WHERE event_id=?")
          .run(latest.event_id);
      if (damage === "deleted")
        f.sqlite.query("DELETE FROM event_content WHERE event_id=?").run(latest.event_id);
      if (damage === "altered")
        f.sqlite
...
for (const change of [
  ...
  test(`recipient eligibility excludes ${change.split(" SET ")[1]}`, async () => {
    ...
    try {
      f.publish(f.person(1), "beta");
      f.sqlite.query(change).run(matchFellow(1));
      assert.equal(await f.match(), null);
    } finally {
      f.sqlite.close();
    }
...
test("prior committed review excludes only its exact target version even after body withdrawal", async () => {
  const f = reviewMatchingFixture();
  try {
    const fellow = f.person(1);
    f.publish(fellow, "beta");
    const r = f.publish(fellow, "beta", {
      object: "R-7",
      kind: "review",
      type: "review.created",
      body: {
        target_claim_id: "C-1",
        target_version: 1,
        scientific_provenance: { model_family_self_declared: "beta" },
      },
    });
    f.sqlite
      .query("INSERT INTO reviews VALUES(?,?,?,?,?,?,?)")
      .run("P-DEMO", "R-7", r.event_id, r.seq, "C-1", 1, fellow);
    assert.equal(await f.match(), null);
    const next = f.publish(MATCH_AUTHOR, "alpha", {
      object: "C-1",
      type: "claim.revised",
      version: 2,
      body: {
        ...
        base_version: 1,
        statement: "revised",
        scientific_provenance: { model_family_self_declared: "alpha" },
      },
    });
    assert.equal(
      (await f.match(matchTestDependencies, { ...f.target, claim_version: 2, pin: next }))
        ?.reviewerId,
      fellow,
    );
  } finally {
...
test("current token-family and sponsor revocation exclude otherwise eligible Fellows", async () => {
  for (const kind of ["sponsor", "family"]) {
    ...
    try {
      f.publish(f.person(1), "beta");
      if (kind === "sponsor")
        f.sqlite.query("INSERT INTO enrollment_sponsor_security VALUES(?,?)").run("usr_1", 10);
      else
        f.sqlite
          .query("INSERT INTO enrollment_fellow_security VALUES(?,?)")
          .run(matchFellow(1), 10);
      assert.equal(await f.match(), null);
    } finally {
      f.sqlite.close();
    }
...
test("a newer narrow credential cannot hide an older usable review credential", async () => {
  ...
  try {
    ...
    f.sqlite
      .query(
        "INSERT INTO fellow_tokens SELECT 'cred-new',fellow_id,sponsor_id,token_hash,20,expires_at,NULL,20,'bearer','[]','{}' FROM fellow_tokens WHERE credential_id='cred-1'",
      )
      .run();
...
test("private directives never cross into permission pins or match results", async () => {
  ...
  try {
    ...
    for (const table of ["fellow_tokens", "enrollment_grants"])
      f.sqlite
        .query(
          `UPDATE ${table} SET granted_resources_json='{"firstDirective":"PRIVATE-DIRECTIVE-SENTINEL"}' WHERE fellow_id=?`,
        )
        .run(matchFellow(1));
    assert.ok(await f.match());
    assert.ok(!JSON.stringify(await f.match()).includes("PRIVATE-DIRECTIVE"));
  } finally {
    f.sqlite.close();
  }
});
...
test("guard rejects changed recipient authority and active-target races", async () => {
  ...
  try {
    ...
    const input = {
      ...
      claim_version: 1,
      now: f.now,
      reviewer: match.reviewerId,
      match: match.eligibility,
    };
    const guard = () =>
      f.rows(
        `WITH input AS (SELECT ? AS j) SELECT ${REVIEW_MATCH_RECIPIENT_GUARD_SQL} AS ok FROM input`,
        JSON.stringify(input),
      )[0]?.ok;
    assert.equal(guard(), 1);
    f.sqlite
      .query("UPDATE problem_memberships SET role='observer' WHERE fellow_id=?")
      .run(matchFellow(1));
    assert.equal(guard(), 0);
    f.sqlite
      .query("UPDATE problem_memberships SET role='contributor' WHERE fellow_id=?")
      .run(matchFellow(1));
    f.sqlite.query("UPDATE fellow_tokens SET revoked_at=1 WHERE fellow_id=?").run(matchFellow(1));
    assert.equal(guard(), 0);
    f.invite(1, "offer");
    assert.equal(
      f.rows(
        `WITH input AS (SELECT ? AS j) SELECT ${REVIEW_MATCH_NO_ACTIVE_SQL} AS ok FROM input`,
        JSON.stringify(input),
      )[0]?.ok,
      0,
    );
