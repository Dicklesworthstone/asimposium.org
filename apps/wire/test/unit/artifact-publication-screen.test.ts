...
import {
  type BoundPublicationScreen,
  PUBLICATION_SCREEN_BYTES,
  PUBLICATION_SCREEN_TIMEOUT_MS,
  type PublicationPolicyCall,
  screenBoundArtifact,
...
const input = (
  body = '{"source":"theorem example : True := by trivial"}',
): BoundPublicationScreen => ({
  body,
  body_digest: digest(body),
  context_digest: digest("immutable-publication-context"),
  identity: { ...identity },
});
...
test("screens the entire digest-bound Unicode body exactly once", async () => {
  ...
  const result = await screenBoundArtifact(value, async (bound, signal) => {
    calls++;
    assert.deepEqual(bound, value);
    assert.equal(signal.aborted, false);
    assert.equal(digest(bound.body), bound.body_digest);
    return pass;
  });
  assert.equal(calls, 1);
  assert.deepEqual(result, {
    ...identity,
    evaluated_body_digest: value.body_digest,
    evaluated_context_digest: value.context_digest,
    decision: "pass",
...
test("accepts the exact UTF-8 byte ceiling without dropping the last character", async () => {
  ...
  assert.equal(observed, value.body);
  assert.equal(new TextEncoder().encode(observed).length, PUBLICATION_SCREEN_BYTES);
});
for (const body of [
  "",
  "a".repeat(PUBLICATION_SCREEN_BYTES + 1),
  "é".repeat(PUBLICATION_SCREEN_BYTES / 2 + 1),
]) {
  test(`refuses a whole body of ${new TextEncoder().encode(body).length} bytes before calling the provider`, async () => {
    let called = false;
    await assert.rejects(
      () =>
        screenBoundArtifact(input(body), async () => {
          called = true;
          return pass;
        }),
      /ARTIFACT_SCREEN_INPUT_INVALID/,
    );
    assert.equal(called, false);
  });
}
for (const change of [
  { body_digest: digest("other bytes") },
  ...
  test(`refuses malformed or mismatched receipt binding ${JSON.stringify(change).slice(0, 90)}`, async () => {
    ...
    await assert.rejects(
      () =>
        screenBoundArtifact({ ...input(), ...change }, async () => {
          called = true;
          return pass;
        }),
      /ARTIFACT_SCREEN_INPUT_INVALID/,
...
for (const decision of ["pass", "allow-with-warning", "quarantine", "reject"]) {
  test(`preserves the provider's ${decision} rather than upgrading it`, async () => {
    const result = await screenBoundArtifact(input(), async () => ({
      decision,
      coarse_category: "benign-context",
    }));
...
for (const answer of [
  ...
  "pass",
  {},
  { decision: "approved", coarse_category: "benign-context" },
  { decision: "pass" },
  { decision: "pass", coarse_category: "provider-unavailable" },
  { decision: "pass", coarse_category: "invented" },
  { decision: 1, coarse_category: "benign-context" },
]) {
  test(`malformed output is a fail-closed dependency failure ${JSON.stringify(answer)}`, async () => {
    const value = input(),
      result = await screenBoundArtifact(value, async () => answer);
    assert.equal(result.decision, "quarantine");
    assert.equal(result.coarse_category, "provider-unavailable");
...
test("an expired provider is aborted and a later pass cannot replace the timeout receipt", async () => {
  ...
  const result = await screenBoundArtifact(
    input(),
    (_bound, current) => {
      signal = current;
...
test("provider-supplied receipts and extra text cannot relabel the screened body", async () => {
  const value = input(),
    result = await screenBoundArtifact(value, async () => ({
      ...pass,
      evaluated_body_digest: digest("different body"),
      evaluated_context_digest: digest("different context"),
      model_version: "fake-model",
      policy_version: "fake-policy",
      configuration_digest: digest("fake"),
  ...
  assert.equal(result.model_version, identity.model_version);
  assert.equal(result.policy_version, identity.policy_version);
  assert.equal(result.configuration_digest, identity.configuration_digest);
  assert.equal(result.provider_status, "ok");
...
test("copies and freezes context before provider code can mutate it", async () => {
  ...
  const result = await screenBoundArtifact(value, async (bound) => {
    ...
    assert.equal(Object.isFrozen(bound.identity), true);
    assert.throws(() => {
      (bound as { body: string }).body = "mutated";
    });
