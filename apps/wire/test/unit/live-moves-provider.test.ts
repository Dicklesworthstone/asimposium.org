...
import { LedgerMovesProvider, type LiveMovesDependencies, TRIAGE_MAX_PROBLEMS, TRIAGE_CONCURRENCY } from "../../src/mega-commands/live-provider.ts";

const credential = {
  fellowId: "F-READER", credentialId: "FC-READER", sponsorId: "SP-READER", fellowStatus: "active",
  grantedScopes: ["promote", "review"], grantedResources: {}, issuedAt: 1, expiresAt: 999999,
} as unknown as FellowCredentialBinding;
const item = (problem = "P-DEMO", patch: Partial<ReviewQueueItem> = {}): ReviewQueueItem => ({
  problem_id: problem, claim_id: "C-1", version: 1, cursor: 10, kind: "definition", statement: "Definition.",
  falsifier: null, disposition: "open", need: "independent-review", best_recorded_tier: "none",
  direct_dependents: 0, dependents_capped: false, author_fellow_id: "F-AUTHOR", author_sponsor_id: "SP-AUTHOR",
  created_at: "2026-09-01T00:00:00.000Z", read_url: `/p/${problem}/claims/C-1@1.md?through=10`, ...patch,
});
function page(problem: string, items = [item(problem)]): ReviewQueueResponse {
  return { schema: "https://a.asimposium.org/schemas/review-queue.v1.json", policy: "review-discovery-v1",
    problem, candidates: items, scanned: items.length, next_after: null, omitted: [],
...
function templateFor(move: "review" | "add-refuter"): MoveTemplate {
  return { move, title: "Fixture", trigger: "Fixture", description: "Fixture", availability: "available",
    ...
    prefilled_hints: {}, required_fields: ["body_md"] };
}
function fixture(problems = ["P-DEMO"]) {
  ...
  const dependencies: LiveMovesDependencies = {
    ...
    authorize: (input) => {
      ...
      const denied = input.credential.fellowStatus !== "active" ||
        (input.credential.grantedResources.problemBinding !== undefined &&
          "problemId" in target && input.credential.grantedResources.problemBinding !== target.problemId) ||
        (input.credential.grantedResources.eventBudget !== undefined && input.usage.eventsRecorded >= input.credential.grantedResources.eventBudget) ||
        ((input.effect === "review" || input.effect === "promote") && !input.credential.grantedScopes.includes(input.effect)) ||
        (input.effect === "promote" && "membershipRole" in target && target.membershipRole === "observer");
      return denied ? { decision: "quarantine", effect: input.effect, handling: "blocked-pending-operator-review" }
...
function next(db: D1Database, binding = credential, problemId = "P-DEMO") {
  return { db, credential: binding, problemId, fellowId: binding.fellowId,
    role: "contributor" as const, effectivePermissions: {} };
}
...
test("production orchestration returns live work and central-policy permission hints", async () => {
  const f = fixture(); try {
    ...
    assert.equal(result.primaryMove?.move, "review"); assert.equal(result.degraded, false);
    assert.equal(result.effectivePermissions?.review, true);
    assert.ok(f.calls.some(call => call.effect === "review" && call.credential === credential &&
      call.target.kind === "existing-problem" && call.target.membershipRole === "contributor"));
  } finally { f.sqlite.close(); }
});
test("one primary and at most two alternatives retain scientific priority", async () => {
  const f = fixture(); try {
    f.dependencies.loadQueue = async () => page("P-DEMO", [1,2,3,4].map(n => item("P-DEMO", { claim_id: `C-${n}`, direct_dependents: n })));
    const result = await new LedgerMovesProvider(f.dependencies).nextMoves(next(f.db));
...
test("missing grants cannot be overridden by caller permission hints", async () => {
  const f = fixture(); try {
    const result = await new LedgerMovesProvider(f.dependencies).nextMoves({ ...next(f.db, { ...credential, grantedScopes: [] }),
      effectivePermissions: { review: true, promote: true, session_open: true } });
    assert.equal(result.primaryMove, null); assert.equal(result.effectivePermissions?.review, false); assert.equal(f.queueReads(), 0);
  } finally { f.sqlite.close(); }
});
test("observer with review scope gets review work but not new-claim promotion", async () => {
  const f = fixture(); try {
    ...
    assert.equal(result.effectivePermissions?.review, true); assert.equal(result.effectivePermissions?.promote, false);
  } finally { f.sqlite.close(); }
});
test("central admission refusal prevents all membership and queue reads", async () => {
  const f = fixture(); try {
    const result = await new LedgerMovesProvider(f.dependencies).nextMoves(next(f.db, { ...credential, fellowStatus: "suspicious_review" }));
    assert.equal(result.primaryMove, null); assert.equal(f.reads(), 0); assert.equal(f.queueReads(), 0);
  } finally { f.sqlite.close(); }
});
for (const bad of ["missing", "mismatched"] as const) test(`credential context ${bad} fails closed before I/O`, async () => {
  const f = fixture(); try {
    const req = next(f.db);
    const result = await new LedgerMovesProvider(f.dependencies).nextMoves(bad === "missing" ? { ...req, credential: undefined } : { ...req, fellowId: "F-OTHER" });
    assert.equal(result.degradedReason, "MOVES_UNAVAILABLE"); assert.equal(f.reads(), 0);
  } finally { f.sqlite.close(); }
});
test("grant usage includes events from all problems for this credential, not other credentials", async () => {
  const f = fixture(); try {
    f.sqlite.exec("INSERT INTO events (writer_credential_id, problem_id) VALUES ('FC-READER','P-OTHER'), ('FC-READER','P-DEMO'), ('FC-OTHER','P-DEMO')");
    const result = await new LedgerMovesProvider(f.dependencies).nextMoves(next(f.db, { ...credential, grantedResources: { eventBudget: 2 } }));
    assert.equal(result.primaryMove, null); assert.equal(f.queueReads(), 0);
...
] as const) test(`current SQL excludes ${name} despite stale caller role`, async () => {
  const f = fixture(); try {
...
test("queue failures return a coarse unavailable result without private exception details", async () => {
  const f = fixture(); try {
    f.dependencies.loadQueue = async () => { throw new Error("PRIVATE-SQL-CANARY"); };
    const result = await new LedgerMovesProvider(f.dependencies).nextMoves(next(f.db));
    assert.equal(result.degradedReason, "MOVES_UNAVAILABLE"); assert.ok(!JSON.stringify(result).includes("PRIVATE-SQL-CANARY"));
  } finally { f.sqlite.close(); }
});
test("triage chooses higher-consequence eligible work across assigned problems", async () => {
  const f = fixture(["P-AAA", "P-BBB"]); try {
    f.dependencies.loadQueue = async (_db, query) => page(query.problem, [item(query.problem, { direct_dependents: query.problem === "P-BBB" ? 10 : 0 })]);
    const result = await new LedgerMovesProvider(f.dependencies).triageMove({ db: f.db, credential, fellowId: credential.fellowId,
      assignments: [assignment("P-AAA"), assignment("P-BBB")] });
    assert.deepEqual(result.move?.refs, ["P-BBB", "C-1@1"]); assert.equal(result.degraded, false);
  } finally { f.sqlite.close(); }
});
test("triage has deterministic capped scope and at most two concurrent queue loads", async () => {
  ...
  const f = fixture(names); try {
    let active = 0; let peak = 0; const seen: string[] = [];
    f.dependencies.loadQueue = async (_db, query) => {
      seen.push(query.problem); active += 1; peak = Math.max(peak, active);
      await new Promise(resolve => setTimeout(resolve, query.problem === "P-AAA" ? 10 : 1));
      active -= 1; return page(query.problem);
    };
    const provider = new LedgerMovesProvider(f.dependencies);
    const req = { db: f.db, credential, fellowId: credential.fellowId, assignments: names.slice().reverse().map(assignment) };
    const result = await provider.triageMove(req);
    assert.equal(seen.length, TRIAGE_MAX_PROBLEMS); assert.equal(peak, TRIAGE_CONCURRENCY);
    assert.deepEqual([...seen].sort(), names.slice(0, TRIAGE_MAX_PROBLEMS));
    assert.equal(result.degradedReason, "MOVES_PARTIAL");
    assert.deepEqual((await provider.triageMove({ ...req, assignments: names.map(assignment) })).move, result.move);
  } finally { f.sqlite.close(); }
});
test("triage filters problem-bound credentials before considering other assignments", async () => {
  const f = fixture(["P-AAA", "P-BBB"]); try {
    const seen: string[] = [];
    f.dependencies.loadQueue = async (_db, query) => { seen.push(query.problem); return page(query.problem); };
    const binding = { ...credential, grantedResources: { problemBinding: "P-BBB" } };
    const result = await new LedgerMovesProvider(f.dependencies).triageMove({ db: f.db, credential: binding, fellowId: binding.fellowId,
      assignments: [assignment("P-AAA"), assignment("P-BBB")] });
    assert.deepEqual(seen, ["P-BBB"]); assert.deepEqual(result.move?.refs, ["P-BBB", "C-1@1"]);
  } finally { f.sqlite.close(); }
});
test("one failed problem does not discard another problem's verified move", async () => {
  const f = fixture(["P-AAA", "P-BBB"]); try {
    f.dependencies.loadQueue = async (_db, query) => { if (query.problem === "P-AAA") throw new Error("PRIVATE"); return page(query.problem); };
    const result = await new LedgerMovesProvider(f.dependencies).triageMove({ db: f.db, credential, fellowId: credential.fellowId,
      assignments: [assignment("P-AAA"), assignment("P-BBB")] });
    assert.deepEqual(result.move?.refs, ["P-BBB", "C-1@1"]); assert.equal(result.degradedReason, "MOVES_PARTIAL");
