...
async function fixture(
  run: (f: { db: D1Database; sql: Database; failAt: (index: number) => void }) => Promise<void>,
) {
  ...
  sql.exec(
    readFileSync(
      new URL("../../../../db/migrations/0064_inbox_and_follows.sql", import.meta.url),
      "utf8",
    ),
  );
  let failure = -1;
  function prepare(text: string) {
    ...
    return {
      ...
      async first() {
        return sql.query(text).get(...values);
      },
      async run() {
        return { meta: { changes: sql.query(text).run(...values).changes } };
      },
      execute() {
        return { results: sql.query(text).all(...values) };
      },
...
const input: NoticeCreateInput = {
  ...
  detail: "Re-orient before writing",
  causedByEventId: "E-revision",
  createdAt: 100,
  expiresAt: 1000,
};
...
describe("causal inbox delivery survives retries", () => {
  test("replaying a causal notice returns the original content and cursor", async () => {
    await fixture(async ({ db, sql }) => {
      ...
      const replay = await createInboxNotice(db, {
        ...input,
        title: "New template",
        detail: "Changed",
        createdAt: 200,
  ...
  test("acknowledged or expired notices are not resurrected by delivery retry", async () => {
    await fixture(async ({ db }) => {
      ...
      expect(
        (await getInboxNotices(db, "fellow-a", { unread_only: true, limit: 50 })).items,
      ).toEqual([]);
    });
  });
  ...
  test("thirty concurrent retries allocate exactly one logical delivery", async () => {
    await fixture(async ({ db, sql }) => {
      const replies = await Promise.all(
        Array.from({ length: 30 }, () => createInboxNotice(db, input)),
      );
  ...
  test("an explicit ID collision cannot return another recipient's notice", async () => {
    await fixture(async ({ db, sql }) => {
      ...
      await expect(
        createInboxNotice(db, { ...input, id: "NOT-fixed", fellowId: "fellow-b" }),
