...
import {
  type PublicationDrainerOptions,
...
export async function publicationFixture() {
  ...
  sql.exec(
    readFileSync(
      new URL("../../../../db/migrations/0070_artifact_publications.sql", import.meta.url),
      "utf8",
    ),
  );
  ...
  const clock = () => instant;
  const actor = {
    fellowId: "F-AAAAAAAAAAAAAAAAAAAAAAAAAA",
    credentialId: "T-ONE",
    sponsorId: "sponsor-one",
    name: "Example",
    model: "model",
    harness: "harness",
    issuedAt: instant - 1000,
    expiresAt: instant + 86_400_000,
    grantedScopes: ["promote", "upload-artifacts"],
    grantedResources: {},
    fellowStatus: "active",
  ...
  sql
    ...
    .run(
      ...
      new Date(instant + 86_400_000).toISOString(),
    );
  sql
    .query("INSERT INTO problem_memberships VALUES(?,?,'contributor')")
    .run(problem, actor.fellowId);
  const evidenceJson = JSON.stringify({
    evidence_id: "E-1",
    kind: "computation",
    body_md: "Exact reproducible observation",
  });
  ...
  sql
    .query("INSERT INTO events VALUES(?,?,1,'evidence.created','evidence','E-1',1,?,?,?,?,?,?)")
    .run(
      "EV-1",
      problem,
      evidenceDigest,
  ...
  sql
    ...
    .run("EV-1", evidenceDigest, evidenceJson);
  const bytes = new TextEncoder().encode("theorem example : True := by trivial\n");
  const digest = await artifactSha256(bytes);
  sql
    .query(
      "INSERT INTO artifact_uploads VALUES(?,?,?,?,'verified',?,?,'text','text/plain; charset=utf-8')",
    )
  ...
  function prepare(text: string, values: unknown[] = []): Statement {
    return {
      ...
      values,
      bind(...items: unknown[]) {
        return prepare(text, items);
      },
      async first() {
        return sql.query(text).get(...(values as never[])) ?? null;
      },
      async all() {
        return {
          success: true,
          results: sql.query(text).all(...(values as never[])),
          meta: { changes: 0 },
        };
      },
      async run() {
        const result = sql.query(text).run(...(values as never[]));
    ...
    async batch(statements: readonly Statement[]) {
      ...
      try {
        const results = statements.map((s) => {
          const result = sql.query(s.text).run(...(s.values as never[]));
          return { success: true, results: [], meta: { changes: result.changes } };
        });
        sql.exec("COMMIT");
        return results;
  ...
  function bucket() {
    const data = new Map<string, { bytes: Uint8Array; httpMetadata: Record<string, string> }>();
    let writes = 0;
    ...
    const port = {
      ...
      async put(key: string, value: Uint8Array, options: { httpMetadata: Record<string, string> }) {
        ...
        writes++;
        data.set(key, { bytes: value.slice(), httpMetadata: options.httpMetadata });
        return { size: value.length };
      },
    };
    return { bucket: port as unknown as R2Bucket, data, hooks, writes: () => writes };
  }
  const privateStore = bucket(),
    publicStore = bucket();
  ...
  const options: PublicationCommandOptions = {
    db,
    privateBucket: privateStore.bucket,
    artifactOrigin: "https://artifacts-staging.asimposium.org",
    clock,
    async readUpload(owner, id) {
      const row = sql
        .query("SELECT * FROM artifact_uploads WHERE upload_id=? AND fellow_id=? AND sponsor_id=?")
    ...
    async reserve(input) {
      const old = sql
        .query(
          "SELECT * FROM public_write_attempt_reservations WHERE fellow_id=? AND route=? AND idempotency_key=?",
        )
        .get(input.fellowId, input.route, input.idempotencyKey) as {
        request_digest: string;
        reservation_id: string;
      } | null;
      if (old) {
        if (old.request_digest !== input.requestDigest) throw new Error("KEY_CONFLICT");
        return old.reservation_id;
      }
      reservations++;
      const id = `Q-${reservations}`;
      sql
        .query(
          "INSERT INTO public_write_attempt_reservations VALUES(?,?,?,?,?,?,?,?,?,?,'reserved',NULL)",
        )
        .run(
          id,
          input.fellowId,
    ...
    async writeLedger(input, projection) {
      ...
      writerTail = new Promise<void>((r) => {
        unlock = r;
      });
      await prior;
      try {
        const old = sql
          .query("SELECT * FROM idempotency WHERE problem_id=? AND idempotency_key=?")
          .get(input.problemId, input.idempotencyKey) as {
          request_digest: string;
          event_id: string;
          event_seq: number;
        } | null;
        if (old) {
          if (old.request_digest !== input.requestDigest) throw new Error("KEY_CONFLICT");
          return { eventId: old.event_id, seq: old.event_seq };
        }
        hooks.beforeWrite?.();
        if (
          !sql
            .query(`SELECT 1 FROM problems WHERE id=?${projection.preconditionSql ?? ""}`)
            .get(input.problemId, ...((projection.preconditionBindings ?? []) as never[]))
        )
          throw new Error("PRECONDITION");
        const seq =
          (
            sql.query("SELECT public_seq FROM problems WHERE id=?").get(input.problemId) as {
              public_seq: number;
            }
          ).public_seq + 1;
        const hash = await publicationHash(input.payloadJson);
        const statements = await projection.statementsAfterEvent({
          sequence: seq,
        ...
        await db.batch([
          ...
          prepare("INSERT INTO events VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)", [
            ...
            input.attribution.credentialId,
            input.createdAt,
          ]),
  ...
  const drainer: PublicationDrainerOptions = {
    db,
    privateBucket: privateStore.bucket,
    publicBucket: publicStore.bucket,
    artifactOrigin: options.artifactOrigin,
    clock,
    async screen(job, _content) {
      screens++;
      return {
        decision: "pass",
        provider_status: "ok",
  ...
  return {
    ...
    screens: () => screens,
    count: (table: string) =>
      (sql.query(`SELECT COUNT(*) AS n FROM ${table}`).get() as { n: number }).n,
  };
}
