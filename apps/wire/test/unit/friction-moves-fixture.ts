...
import {
  type FrictionMoveDependencies,
  loadFrictionMove,
} from "../../src/mega-commands/friction-moves.ts";

export const hash = (text: string) => createHash("sha256").update(text).digest("hex");
export const template: MoveTemplate = {
  move: "add-refuter-from-friction",
  title: "Refute from friction",
...
export function frictionFixture() {
  ...
  const calls: string[] = [],
    ...
    admissions: { seq: number; record: FormalRecord | null }[] = [];
  const db = {
    prepare(sql: string) {
      let values: unknown[] = [];
      return {
        ...
        async all() {
          calls.push(sql);
          return { results: sqlite.query(sql).all(...(values as never[])) };
        },
        async first() {
          calls.push(sql);
          return sqlite.query(sql).get(...(values as never[])) ?? null;
        },
      };
    },
  } as D1Database;
  function event(
    ...
    sqlite
      ...
      .run(eventId, problem, seq, type, kind, object, version, hash(text), fellow, `SP-${fellow}`);
    ...
    return {
      ...
      fellow_id: fellow,
      sponsor_id: `SP-${fellow}`,
      session_id: "SS-example",
      model_string_self_declared: "model",
      harness: "harness",
  ...
  function claim(seq: number, id = `C-${seq}`, version = 1, dependents = 1) {
    event(
      ...
      version === 1 ? "claim.created" : "claim.revised",
      {
        claim_id: id,
        kind: "lemma",
        statement: `Statement of ${id}@${version}`,
        falsifier: "A counterexample in the stated domain.",
    ...
    const item: ReviewQueueItem = {
      ...
      author_sponsor_id: "SP-F-author",
      created_at: "2026-09-17T00:00:00.000Z",
      read_url: `/p/P-DEMO/claims/${id}@${version}.md?through=100`,
    };
    const old = items.findIndex((x) => x.claim_id === id);
  ...
  function report(
    ...
    blocker: FrictionWork["blocker"] = "counterexample-scent",
  ) {
    const work: FrictionWork = {
      format: "asimposium.formalization-friction.v1",
      blocker,
      toolchain: "TOOLCHAIN-SENTINEL",
      blocked_obligation: "OBLIGATION-SENTINEL: nonzero denominator",
      witness_seed: "WITNESS-SENTINEL: boundary at zero",
      analysis: "ANALYSIS-SENTINEL: the attempt assumes a missing regularity condition.",
    };
    const content = {
      bears_on_kind: "claim" as const,
      bears_on_id: target.claim_id,
      bears_on_version: target.version,
      direction: "informs" as const,
      kind: "formalization-friction" as const,
      source: { kind: "model_memory" as const },
      mode: "exploratory" as const,
      body_md: JSON.stringify(work),
    };
    ...
    const record: FormalRecord = {
      ...
      publication,
      target: { claim_id: target.claim_id, version: target.version },
      content,
    };
    admissions.push({ seq, record });
    return record;
  }
  ...
  const dependencies: FrictionMoveDependencies = {
    async page(_db, _problem, cursor, after) {
      pages.push(after);
      const selected = admissions
        .filter((x) => x.seq > after && x.seq <= cursor)
      ...
      return {
        ...
        next_after: selected.length > 8 ? selected[7]!.seq : null,
        omitted: selected.length > 8 ? ["page_limit"] : [],
      } as FormalRecordRead;
    },
    work(body) {
      try {
        const w = JSON.parse(body);
        return w?.format === "asimposium.formalization-friction.v1" ? w : null;
      } catch {
        return null;
      }
    },
  ...
  return {
    ...
    updateBody(id: string, body: Record<string, unknown>, commitDigest = false) {
      const text = JSON.stringify(body);
      sqlite.query("UPDATE event_content SET payload_json=? WHERE event_id=?").run(text, id);
      if (commitDigest) {
        sqlite.query("UPDATE events SET payload_sha256=? WHERE id=?").run(hash(text), id);
        sqlite
