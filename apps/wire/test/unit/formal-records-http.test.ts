...
import {
  FORMAL_READ_MAX_RESPONSE_BYTES,
  formalRecordResponse,
} from "../../src/ledger/formal-records-http.ts";
...
const request = (method = "GET", headers: Record<string, string> = {}) =>
  new Request("https://a.asimposium.org/p/P-DEMO/formal.json", { method, headers });
...
test("conditional reads accept weak matching validators and return bodyless 304", async () => {
  const text = "record";
  const get = await formalRecordResponse(request(), text, "md", face, false);
  for (const method of ["GET", "HEAD"]) {
    const response = await formalRecordResponse(
      request(method, { "if-none-match": `"unrelated", W/${get.headers.get("etag")}` }),
      text,
      "md",
      face,
      false,
    ...
    assert.equal(response.headers.get("etag"), get.headers.get("etag"));
  }
});
test("representations cannot cross-validate even when their bytes coincide", async () => {
  ...
  const html = await formalRecordResponse(
    request("GET", { "if-none-match": json.headers.get("etag")! }),
    "same bytes",
    "html",
    face,
    false,
  ...
  assert.notEqual(html.headers.get("etag"), json.headers.get("etag"));
});
test("current withdrawal changes ETag rather than validating an old body", async () => {
  ...
  const withdrawn = await formalRecordResponse(
    request("GET", { "if-none-match": original.headers.get("etag")! }),
    "content unavailable",
    "json",
    face,
    false,
  ...
  assert.notEqual(withdrawn.headers.get("etag"), original.headers.get("etag"));
});
test("unlisted responses are never shared-cacheable, including a 304", async () => {
  ...
  const unchanged = await formalRecordResponse(
    request("GET", { "if-none-match": original.headers.get("etag")! }),
    "source",
    "json",
    face,
...
test("exact read locations and chronological continuations retain their cursor", async () => {
  ...
  const exact = await formalRecordResponse(
    request(),
    "source",
    "json",
    { ...face, target: "E-20", next_after: null },
    false,
  );
  assert.equal(
    exact.headers.get("content-location"),
    "/p/P-DEMO/formal.json?through=90&target=E-20",
  );
