...
function fixture(count = 0) {
  ...
  const readPage = async (afterSeq: number, limit: number): Promise<PublicExportEventRef[]> =>
    sql.prepare(`SELECT id AS eventId, problem_id AS problemId, seq, payload_sha256 AS payloadSha256
...
async function refused(run: () => Promise<unknown>) {
  await assert.rejects(run, (error: unknown) =>
    error instanceof PublicExportUnavailableError && error.message === "PUBLIC_EXPORT_UNAVAILABLE");
}
...
describe("finite and currently public export snapshots on actual SQLite", () => {
  for (const count of [0, 1, 49, 50, 51, 199, 200, 201]) {
    test(`exact ${count}-event export with complete digest-bound payloads`, async () => {
      ...
      try {
        ...
        for (const event of events) assert.equal(sha(String(payloads.get(event.eventId))), event.payloadSha256);
  ...
  test("revalidation catches withdrawal of a payload already read in an earlier chunk", async () => {
    ...
    try {
      ...
      f.sql.exec("UPDATE event_content SET redacted_at='2026-09-22T00:00:00.000Z' WHERE event_id='E-1'");
  ...
  test("cancellation stops work before the next database page", async () => {
    ...
    try {
      ...
      let reads = 0;
      await assert.rejects(() => collectPublicExportEvents(cut, async (after, limit) => {
        reads++;
        const result = await f.readPage(after, limit);
        controller.abort();
        return result;
      }, controller.signal), (error: unknown) => error instanceof DOMException && error.name === "AbortError");
      assert.equal(reads, 1);
    } finally { f.sql.close(); }
  });
  test("served export uses payload/chain validation and revalidates before HTTP conditionals", () => {
    const source = readFileSync(new URL("../../src/ledger/event-tail-feeds.ts", import.meta.url), "utf8");
    ...
    assert.ok(source.indexOf("await revalidatePublicExportCut(db, cut)") < source.indexOf('get("if-none-match")'));
