...
function fixture(count = 1) {
  ...
  const append = (seq: number, problem = "P-DEMO") => {
    sql
      .prepare(
        "INSERT INTO events VALUES (?, ?, ?, 'claim.created', 'claim', ?, 1, ?, ?, 'F-1', 'S-1', 'SES-1', 'model', 'harness')",
      )
  ...
  const run = (wait = 25, since = count, through?: number, method = "GET") =>
    readPublicEventTailWithWait(
      db,
      "P-DEMO",
      { since, limit: 50, wait, ...(through === undefined ? {} : { through }) },
      { ...request, method },
      timing,
    );
  return {
    ...
    request,
    run,
    onDelay: (fn: (index: number) => void) => {
      onDelay = fn;
    },
    beforeQuery: (fn: (query: string) => Promise<void>) => {
      beforeQuery = fn;
    },
    afterQuery: (fn: (query: string) => void) => {
      afterQuery = fn;
    },
    setNow: (value: number) => {
      now = value;
...
describe("W7.3 bounded public event waiting on actual SQLite", () => {
  for (const seconds of [1, 5, 6, 24, 25]) {
    test(`wait=${seconds} expires with a complete unchanged page and bounded reads`, async () => {
      ...
      try {
        ...
        assert.equal(
          f.delays.reduce((sum, ms) => sum + ms, 0),
          seconds * 1000,
        );
        assert.ok(f.queries.length <= 6);
        assert.ok(
          f.queries.every(
            (query) => !query.includes("payload_json") && !query.includes("workshop"),
          ),
        );
  ...
  for (const scenario of ["absent", "zero", "head", "pinned", "backlog"] as const) {
    test(`${scenario} never opens a wait`, async () => {
      ...
      try {
        const result =
          scenario === "absent"
            ? await readPublicEventTailWithWait(
                f.db,
                "P-DEMO",
                { since: 1, limit: 50 },
                f.request,
            ...
            : await f.run(
                scenario === "zero" ? 0 : 25,
                scenario === "backlog" ? 0 : 1,
                scenario === "pinned" ? 1 : undefined,
                scenario === "head" ? "HEAD" : "GET",
              );
  ...
  test("new commit wakes a reader and the canonical page supplies exact event IDs", async () => {
    ...
    try {
      ...
      assert.deepEqual(
        result?.page.events.map((e) => e.event?.id),
        ["P-DEMO-event-2", "P-DEMO-event-3"],
      );
  ...
  test("redaction between notification and readback cannot restore content availability", async () => {
    ...
    try {
      ...
      f.afterQuery((q) => {
        if (q === EVENT_WAIT_HEAD_SELECT)
          f.sql.exec("UPDATE event_content SET redacted_at='2026-09-22T01:00:00.000Z'");
      });
      const result = await f.run();
  ...
  for (const head of [null, -1, 0, 1.5, "bad", 9007199254740992]) {
    test(`corrupt or regressed head ${String(head)} does not mint a cursor`, async () => {
      ...
      try {
        f.onDelay(() =>
          f.sql.prepare("UPDATE problems SET public_seq=? WHERE id='P-DEMO'").run(head),
        );
        await assert.rejects(() => f.run(), code("EVENT_TAIL_UNAVAILABLE"));
        assert.equal(f.delays.length, 1);
      } finally {
  ...
  test("unavailable binding errors are coarse and release admission", async () => {
    const f = fixture();
    try {
      f.beforeQuery(async (q) => {
        if (q === EVENT_WAIT_HEAD_SELECT) throw new Error("PRIVATE-DRIVER-DIAGNOSTIC");
      });
      await assert.rejects(() => f.run(), code("EVENT_TAIL_UNAVAILABLE"));
  ...
  test("cancellation before entry never touches D1 or reflects an abort reason", async () => {
    ...
    try {
      ...
      await assert.rejects(
        () => f.run(),
        (error: unknown) => aborted(error) && !String(error).includes("PRIVATE-"),
      );
  ...
  test("initial canonical query errors preserve the teaching code", async () => {
    const f = fixture();
    try {
      await assert.rejects(() => f.run(25, 2), code("CURSOR_INVALID"));
    } finally {
      f.sql.close();
    }
  });
...
describe("event wait HTTP readback", () => {
      ...
      test(`${format} ${outcome} retains complete framing, cache identity and retry semantics`, async () => {
        ...
        try {
          ...
          const request = new Request(
            `https://a.asimposium.org/p/P-DEMO/events.${format}?since=1&wait=25`,
          );
          ...
          if (format === "json")
            assert.equal(JSON.parse(body).page_end.next_cursor, outcome === "changed" ? 2 : 1);
          if (format === "ndjson") {
  ...
  for (const method of ["GET", "HEAD"] as const) {
    test(`${method} conditional read retains wait outcome and no-store headers on 304`, async () => {
      ...
      try {
        ...
        const first = await eventTailResponse(
          new Request(url),
          result.page,
          "json",
          true,
        ...
        assert.ok(etag);
        const conditional = await eventTailResponse(
          new Request(url, { method, headers: { "if-none-match": etag } }),
          result.page,
          "json",
          true,
          "timeout",
  ...
  test("HEAD does not hold a connection and still returns the GET content length", async () => {
    ...
    try {
      ...
      assert.ok(result);
      const response = await eventTailResponse(
        new Request("https://a.asimposium.org/p/P-DEMO/events.json?wait=25", { method: "HEAD" }),
        result.page,
        "json",
        false,
        result.waitOutcome,
  ...
  test("wait preference survives snapshot pagination and returns to an unpinned poll", async () => {
    const f = fixture(3);
    try {
      const first = await readPublicEventTailWithWait(
        f.db,
        "P-DEMO",
        { since: 0, limit: 1, wait: 25 },
        f.request,
        f.timing,
      );
      assert.ok(first?.page.page_end.next);
      const next = parseEventTailQuery(
        new URL(first.page.page_end.next, "https://a.asimposium.org").searchParams,
      );
    ...
    test(`incoming signal ${legacy}: read succeeds but waiting is explicitly unavailable`, async () => {
      ...
      try {
        const request = Object.defineProperty({ method: "GET" }, "signal", {
          get: () => {
            if (legacy === "throwing") throw new Error("PRIVATE-RUNTIME-DETAIL");
            return undefined;
          },
  ...
  test("ordinary responses keep their original cache policy without a wait header", async () => {
    ...
    try {
      ...
      assert.ok(result);
      const response = await eventTailResponse(
        new Request("https://a.asimposium.org/p/P-DEMO/events.json"),
        result.page,
        "json",
        false,
      );
