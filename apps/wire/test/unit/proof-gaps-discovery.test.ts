...
import {
  ...
  hypothesesResponses,
} from "../../src/discovery/hypotheses-discovery.ts";
import {
  PROOF_GAPS_PUBLIC_READS,
  proofGapParameters,
  proofGapResponses,
} from "../../src/discovery/proof-gaps-discovery.ts";
...
test("all gap representations document the actual target and snapshot contracts", () => {
  for (const suffix of ["", ".json", ".md", ".html"]) {
    ...
    assert.deepEqual(
      ...
      ["through", "after", "target"],
    );
    assert.ok(
      params.every((item) => item.schema.$ref.startsWith(`${origin}/schemas/proof-gaps.v1.json#`)),
    );
...
test("gap JSON is structured while Markdown and HTML expose reading representations", () => {
  ...
  assert.equal(
    ...
    `${origin}/schemas/proof-gaps.v1.json#/properties/response`,
  );
...
test("the suffixless gap route promises a redirect rather than a fabricated 200", () => {
  ...
  assert.ok(response["308"]);
  assert.equal(response["200"], undefined);
});
test("existing hypothesis queries and schema references remain unchanged", () => {
  ...
  assert.match(
    (hypothesesResponses("/p/{id}/hypotheses.json", origin) as any)["200"].content[
      "application/json"
    ].schema.$ref,
    /schemas\/hypotheses.v1.json#/,
  );
});
test("unrelated and unsupported paths cannot borrow a gap contract", () => {
  for (const path of [
    "/p/{id}/gaps.toon",
    "/p/{id}/gaps.json/extra",
    "/v1/p/{id}/gaps",
    "/p/{id}/gaps.json\n",
  ]) {
