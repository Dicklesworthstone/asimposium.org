...
import { commitRequest, readRequest, listRequests, requestReceipt, type RequestCommand, type ReplayProtector } from "../../src/review-requests/store.ts";
import { readReviewRequestTarget, readCompletionReview } from "../../src/review-requests/target.ts";
import { REVIEW_ACCEPTED_MS, REVIEW_OFFER_MS } from "../../src/review-requests/model.ts";

const NOW = 1_800_000_000_000;
const AUTHOR = `F-${"A".repeat(26)}`, REVIEWER = `F-${"B".repeat(26)}`;
...
async function protector(): Promise<ReplayProtector> {
  ...
  return {
    async seal(text, context = "") {
      ...
      return { ciphertext: Buffer.from(bytes).toString("base64"), initializationVector: Buffer.from(iv).toString("base64") };
    },
    async open(value, context = "") {
      const bytes = await crypto.subtle.decrypt({ name: "AES-GCM", iv: Buffer.from(value.initializationVector, "base64"), additionalData: new TextEncoder().encode(context) }, key, Buffer.from(value.ciphertext, "base64"));
...
async function fixture() {
  ...
  sql.run(readFileSync(new URL("../../../../db/migrations/0065_review_requests.sql", import.meta.url), "utf8"));
  function fellow(id: string, sponsor: string, credential: string) {
    ...
    sql.query("INSERT INTO enrollment_grants VALUES(?,?,?,?)").run(id,sponsor,'["promote","review"]','{}');
    sql.query("INSERT INTO fellow_tokens VALUES(?,?,?,?,?,?,NULL,?,?)").run(credential,id,sponsor,`${credential}-hash`,NOW-1000,NOW+1000000000,'["promote","review"]','{}');
    sql.query("INSERT INTO problem_memberships VALUES('P-DEMO',?,'contributor')").run(id);
  }
  fellow(AUTHOR,"usr_author","cred-author"); fellow(REVIEWER,"usr_reviewer","cred-reviewer");
  function claim(id = "C-1", event = "EV-claim", seq = 1) {
    const payload = JSON.stringify({ claim_id: id, statement: "A falsifiable test claim.", kind: "conjecture", falsifier: "A finite counterexample." });
    sql.query("INSERT INTO events VALUES(?,'P-DEMO',?,?,1,'claim.created','claim',?,'usr_author','cred-author',?)").run(event,seq,id,AUTHOR,hash(payload));
  ...
  const db = {
    prepare(query: string) {
      const statement = (...bindings: unknown[]) => ({
        ...
        async first() { return sql.query(query).get(...bindings as never[]) ?? null; },
        async all() { return { results: sql.query(query).all(...bindings as never[]) }; },
        async run() { return sql.query(query).run(...bindings as never[]); },
      });
      return statement();
    },
    batch(statements: { run(): Promise<unknown> }[]) {
      const work = tail.then(async () => { sql.run("BEGIN"); try {
        const results = []; for (const statement of statements) results.push(await statement.run());
        sql.run("COMMIT"); return results;
  ...
  const offer: RequestCommand = {
    receipt: { schema: "https://a.asimposium.org/schemas/review-requests.v1.json", request_id: `RR-${"1".repeat(32)}`,
      problem_id: "P-DEMO", claim_id: "C-1", claim_version: 1, claim_event_id: pin.event_id,
      claim_payload_sha256: pin.digest, author_id: AUTHOR, reviewer_id: REVIEWER, version: 1,
      status: "offered", created_at: NOW, updated_at: NOW, expires_at: NOW+REVIEW_OFFER_MS, review_event_id: null },
    actor: { fellowId: AUTHOR, sponsorId: "usr_author", credentialId: "cred-author", tokenHash: "cred-author-hash" } as RequestCommand["actor"],
    role: "contributor", action: "offer", scope: "promote", cursor: 1, authorSponsor: "usr_author", reviewerSponsor: "usr_reviewer",
  ...
  const count = (table: string) => (sql.query(`SELECT COUNT(*) n FROM ${table}`).get() as { n: number }).n;
...
test("offer, event, recipient notice and encrypted replay commit together without a public event", async () => {
  const f = await fixture(); try {
    ...
    for (const table of ["review_requests","review_request_events","fellow_inbox_notices","review_request_replays"]) assert.equal(f.count(table),1);
    assert.equal(f.count("events"),1);
    const replay = f.sql.query("SELECT response_ciphertext FROM review_request_replays").get() as { response_ciphertext: string };
    assert.ok(!replay.response_ciphertext.includes("RR-"));
    const notice = f.sql.query("SELECT detail FROM fellow_inbox_notices").get() as { detail: string };
    assert.ok(notice.detail.includes(f.offer.receipt.request_id)); assert.ok(!notice.detail.includes("falsifiable test claim"));
  } finally { f.sql.close(); }
});
test("concurrent unchanged retries return exactly one invitation and one notice", async () => {
  const f = await fixture(); try {
    const other = { ...f.offer, eventId: "RRE-race", receipt: { ...f.offer.receipt, request_id: `RR-${"2".repeat(32)}` } };
    const [a,b] = await Promise.all([commitRequest(f.db,f.crypto,f.offer),commitRequest(f.db,f.crypto,other)]);
    assert.deepEqual(a,b); assert.equal(f.count("review_requests"),1); assert.equal(f.count("fellow_inbox_notices"),1);
    await assert.rejects(commitRequest(f.db,f.crypto,{...f.offer,requestDigest:"different"}),/IDEMPOTENCY_CONFLICT/);
  } finally { f.sql.close(); }
});
test("racing accept and decline cannot both settle the same invitation version", async () => {
  const f = await fixture(); try {
    ...
    const results = await Promise.allSettled([commitRequest(f.db,f.crypto,f.response()),commitRequest(f.db,f.crypto,f.response("decline"))]);
    assert.equal(results.filter(r=>r.status==="fulfilled").length,1);
    assert.equal(f.count("review_request_events"),2); assert.equal(f.count("fellow_inbox_notices"),2);
    const row = await readRequest(f.db,"P-DEMO",REVIEWER,f.offer.receipt.request_id);
    assert.equal(row?.version,2); assert.ok(row && ["accepted","declined"].includes(row.status));
  } finally { f.sql.close(); }
});
for (const [name, mutate] of [
  ["credential revoked", "UPDATE fellow_tokens SET revoked_at=1800000000000 WHERE credential_id='cred-author'"],
  ...
  ["membership changed", `UPDATE problem_memberships SET role='observer' WHERE fellow_id='${AUTHOR}'`],
  ["scope removed", "UPDATE fellow_tokens SET granted_scopes_json='[]' WHERE credential_id='cred-author'"],
  ["grant exhausted", "UPDATE fellow_tokens SET granted_resources_json='{\"eventBudget\":1}' WHERE credential_id='cred-author'"],
  ["grant rebound", "UPDATE fellow_tokens SET granted_resources_json='{\"problemBinding\":\"P-OTHER\"}' WHERE credential_id='cred-author'"],
  ["grant expired", "UPDATE fellow_tokens SET granted_resources_json='{\"fellowGrantExpiresAt\":1800000000000}' WHERE credential_id='cred-author'"],
  ...
  ["reviewer loses scope", `UPDATE enrollment_grants SET granted_scopes_json='[]' WHERE fellow_id='${REVIEWER}'`],
] as const) {
  test(`transaction guard rolls back every effect when ${name} after preparation`,async()=>{
    const f=await fixture();try {
      f.sql.run(mutate);await assert.rejects(commitRequest(f.db,f.crypto,f.offer),/CONFLICT/);
      for(const table of ["review_requests","review_request_events","fellow_inbox_notices","review_request_replays"])assert.equal(f.count(table),0);
    }finally{f.sql.close();}
  });
}
test("capacity is global to the recipient, and decline releases a slot",async()=>{
  const f=await fixture();try {
    for(let i=1;i<=5;i++){
      ...
      const command={...f.offer,cursor:i,eventId:`RRE-offer-${i}`,idempotencyKey:`offer-${i}`,requestDigest:`offer-${i}`,pins:[pin],
        receipt:{...f.offer.receipt,request_id:`RR-${String(i).repeat(32)}`,claim_id:`C-${i}`,claim_event_id:pin.event_id,claim_payload_sha256:pin.digest}};
      if(i<5)await commitRequest(f.db,f.crypto,command);else await assert.rejects(commitRequest(f.db,f.crypto,command),/CONFLICT/);
    }
    ...
    await commitRequest(f.db,f.crypto,{...f.offer,cursor:6,eventId:"RRE-six",idempotencyKey:"six",requestDigest:"six",pins:[pin],receipt:{...f.offer.receipt,request_id:`RR-${"6".repeat(32)}`,claim_id:"C-6",claim_event_id:pin.event_id,claim_payload_sha256:pin.digest}});
...
test("target reader verifies actual bytes and rejects old versions or withdrawals",async()=>{
  const f=await fixture();try {
    ...
    assert.equal(row.author_id,AUTHOR);assert.equal(row.pin.digest,f.offer.pins[0]!.digest);
...
test("completion verifies the real review payload as well as its projection",async()=>{
  const f=await fixture();try {
    const body=JSON.stringify({target_claim_id:"C-1",target_version:1,verdict:"cannot-verify"});
    f.sql.query("INSERT INTO events VALUES('EV-review','P-DEMO',2,'R-1',1,'review.created','review',?,'usr_reviewer','cred-reviewer',?)").run(REVIEWER,hash(body));
    ...
    assert.equal((await readCompletionReview(f.db,"P-DEMO","C-1",1,REVIEWER,"R-1")).event_id,"EV-review");
...
test("sponsor-wide daily invitation limit is enforced in the write transaction",async()=>{
  const f=await fixture();try {
    for(let i=1;i<=21;i++){
      const reviewer=`F-${String(i).padStart(26,"X")}`,sponsor=`usr_r${i}`;
      f.fellow(reviewer,sponsor,`cred-r${i}`);
      const command={...f.offer,reviewerSponsor:sponsor,eventId:`RRE-rate-${i}`,idempotencyKey:`rate-${i}`,requestDigest:`rate-${i}`,
        receipt:{...f.offer.receipt,request_id:`RR-${String(i).padStart(32,"0")}`,reviewer_id:reviewer}};
      if(i<=20)await commitRequest(f.db,f.crypto,command);else await assert.rejects(commitRequest(f.db,f.crypto,command),/CONFLICT/);
    }
    assert.equal(f.count("review_requests"),20);assert.equal(f.count("fellow_inbox_notices"),20);
  }finally{f.sql.close();}
});
test("already-reviewed targets cannot generate repeated invitations",async()=>{
  const f=await fixture();try {
    f.sql.query("INSERT INTO events VALUES('EV-reviewed','P-DEMO',2,'R-1',1,'review.created','review',?,'usr_reviewer','cred-reviewer','hash')").run(REVIEWER);
    f.sql.query("INSERT INTO reviews VALUES('R-1','EV-reviewed','P-DEMO',2,'C-1',1,?)").run(REVIEWER);
    f.sql.run("UPDATE problems SET public_seq=2");
...
test("accepted work completes through a verified review and notifies its author exactly once",async()=>{
  const f=await fixture();try {
    ...
    const body=JSON.stringify({target_claim_id:"C-1",target_version:1,verdict:"cannot-verify"});
    f.sql.query("INSERT INTO events VALUES('EV-complete','P-DEMO',2,'R-1',1,'review.created','review',?,'usr_reviewer','cred-reviewer',?)").run(REVIEWER,hash(body));
    f.sql.query("INSERT INTO reviews VALUES('R-1','EV-complete','P-DEMO',2,'C-1',1,?)").run(REVIEWER);
