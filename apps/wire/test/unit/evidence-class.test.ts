...
import { assessEvidenceClass, canDrivePromotion, type EvidenceInput } from "../../src/ledger/evidence-class.ts";
...
describe("W5.6 evidence classes (computed, never asserted)", () => {
  ...
  test("a locator + excerpt is a citation", () => {
    const a = assessEvidenceClass(input({ source: { kind: "locator", locator: "https://x", excerpt: "the result" } }));
    expect(a.class).toBe("citation");
  });

  test("P5: a computation with no detection floor is coerced to heuristic, recorded", () => {
    const a = assessEvidenceClass(input({ source: { kind: "locator", locator: "https://x", excerpt: "e" }, computation: {} }));
  ...
  test("a computation with a stated domain is computation", () => {
    const a = assessEvidenceClass(input({ source: { kind: "locator", locator: "https://x", excerpt: "e" }, computation: { domainOrFloor: "all n ≤ 10^6" } }));
    expect(a.class).toBe("computation");
  });

  test("certified requires the shape check AND an independent review confirming it", () => {
    const scanOnly = assessEvidenceClass(input({ source: { kind: "locator", locator: "https://x", excerpt: "e" }, certifiedArtifact: { shapeCheckDigest: "sha256:x" } }));
    ...
    const confirmed = assessEvidenceClass(input({ source: { kind: "locator", locator: "https://x", excerpt: "e" }, certifiedArtifact: { shapeCheckDigest: "sha256:x", independentReviewConfirmed: true } }));
