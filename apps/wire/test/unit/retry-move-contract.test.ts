...
const retry: VerifiedDeadEndRetry = {
  ...
  cursor: 12,
  dead_end_id: "DE-2",
  author_fellow_id: `F-${"A".repeat(26)}`,
...
test("selected retry advertises a real private-workshop contract, not another negative result", () => {
  ...
  assert.ok(move);
  NextMoveCandidateSchema.parse(move);
  ...
  assert.ok(
    WorkshopPushRequestSchema.safeParse({
      ...
      title: "Re-examine the blocked reduction",
...
test("gap retry matching consumes the actual transition request field outcome", () => {
  const closed = GapTransitionRequestSchema.parse({
    gap_id: "G-3",
    outcome: "closed-by",
    closed_by: "E-4",
  });
  ...
  assert.equal(
    retryEventMatches(
      ...
      { gap_id: "G-3", status: "closed-by" },
    ),
    false,
  );
  assert.equal(
    retryEventMatches(row, { kind: "gap-closed", gap_id: "G-3" }, { ...closed, gap_id: "G-30" }),
    false,
  );
});
...
test("author status never prefills supersession or a scientific outcome", () => {
  ...
  assert.ok(move);
  assert.equal(
    (move.contract.preparation as { author_may_supersede: boolean }).author_may_supersede,
    true,
  );
