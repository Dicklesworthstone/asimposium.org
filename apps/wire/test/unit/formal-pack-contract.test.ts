...
const request = {
  ...
  source: {
    kind: "locator",
    locator: "local/reproduction.lean",
    excerpt: "An intentionally submitted source artifact.",
  },
  body_md: "Deliberate work product.",
  formal_artifact: {
    language: "lean",
    declaration: "example",
    source: "theorem example : True := True.intro",
    toolchain: "lean fixture",
    axiom_report: "Self-declared axioms.",
  },
};
...
test("formal service uses the real evidence contract and drops stored computed authority fields", async () => {
  ...
  const page = await loadFormalRecords(
    source({ ...request, evidence_id: "E-2", computed_class: "certified", drives_promotion: true }),
    "P-DEMO",
    10,
  );
  assert.equal(page.records.length, 1);
  assert.equal(page.records[0]?.kind, "formal-artifact");
...
test("invalid formal artifacts fail canonical decoding instead of being silently promoted into the pack", async () => {
  const page = await loadFormalRecords(
    source({ ...request, formal_artifact: { ...request.formal_artifact, language: "unknown" } }),
    "P-DEMO",
    10,
  );
  assert.equal(page.records.length, 0);
  assert.ok(page.omitted.includes("unsupported_record"));
});
test("real shared composer accepts formal candidates under every supported token budget", async () => {
  ...
  for (const requested_max_tokens of [800, 1500, 2500, 4000, 8000]) {
    const pack = composePack({
      ...
      viewer: { audience: "session", membership: "contributor", effective_permissions: ["read"] },
    ...
    assert.ok(
      pack.items
        .filter((i) => i.kind === "formal-artifact")
        .every((i) => i.untrusted && i.scope === "ledger"),
    );
  }
});
test("working-pack retry starts a private scratch and cannot forge author-only supersession", () => {
  const trigger = {
    dead_end_id: "DE-1",
    reason: "UNTRUSTED-TRIGGER-TEXT",
    approach: "AUTHORED-APPROACH",
  } as FiredDeadEndTriggerRow;
  const move = workingRetryDeadEndMove(trigger);
  assert.ok(move);
  const body = JSON.parse(move.body);
  assert.equal(body.contract.request.path, "/v1/sessions/{id}/workshop");
  assert.deepEqual(body.contract.prefilled_hints, { type: "scratch" });
  assert.ok(
    WorkshopPushRequestSchema.safeParse({
      ...body.contract.prefilled_hints,
      title: "A new attempt",
      body_md: "My deliberate investigation.",
    }).success,
  );
  assert.ok(!move.body.includes("UNTRUSTED-TRIGGER-TEXT"));
  assert.ok(!move.body.includes("AUTHORED-APPROACH"));
