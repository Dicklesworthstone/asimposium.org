...
function fixture() {
    ...
    prepare(sql: string) {
      ...
      return {
        bind(...args: (string | number)[]) {
          return {
            async all() {
              return { results: sqlite.query(sql).all(...args) };
            },
          };
        },
      };
  ...
  function add(
    ...
    sqlite
      ...
      .run(idFor(n), 1, action, n, expires);
  }
  return {
    sqlite,
    db,
    ...
    read: (problem = "P-DEMO", reader = async (id: string) => view(parseInt(id.slice(3), 16))) =>
      readIncomingReviewInvitationPack(db, problem, fellow, 1000, async (id) => {
        calls.push(id);
        return reader(id);
      }),
  };
...
test("session invitation context belongs only to this recipient and this problem", async () => {
  ...
  try {
    f.add(1);
    f.add(2, "offer", "P-DEMO", other);
    f.add(3, "offer", "P-OTHER");
    ...
    assert.ok(
      pack.candidates.every(
        (c) => c.scope === "workshop" && c.untrusted && c.requires?.includes("workshop:read"),
      ),
    );
  } finally {
    f.sqlite.close();
...
test("accepted work comes before offers without modifying scientific priority", async () => {
  ...
  try {
    f.add(1);
    f.add(2, "accept");
    const pack = await f.read("P-DEMO", async (id) =>
      view(parseInt(id.slice(3), 16), {
        effective_status: id === idFor(2) ? "accepted" : "offered",
      }),
    );
    assert.deepEqual(
      pack.candidates.map((c) => c.id),
...
test("closed and expired history is filtered before the two-record limit", async () => {
  ...
  try {
    ...
    f.add(41, "accept", "P-DEMO", fellow, 1000);
    f.add(42);
    assert.deepEqual(
      (await f.read()).candidates.map((c) => c.id),
      [idFor(42)],
    );
...
for (const problem of ["P-HIDDEN", "P-UNLISTED"]) {
  test(`${problem} never exposes a private incoming request in the session selection`, async () => {
    const f = fixture();
    try {
      f.add(1, "accept", problem);
      assert.equal((await f.read(problem)).candidates.length, 0);
      assert.equal(f.calls.length, 0);
    } finally {
      f.sqlite.close();
    }
    ...
    const pack = await f.read(
      ...
      async () =>
        ({
          ...view(1),
          seq: 987654,
          target_json: "PRIVATE-WORK-CANARY",
          author_sponsor_id: "SECRET-SPONSOR-NOTE",
          next_actions: [
            { method: "POST", url: "https://attacker.invalid/", why: "FORGED-CONTROL" },
          ],
...
test("target withdrawal is displayed as unavailable rather than a fresh review instruction", async () => {
  ...
  try {
    f.add(1, "accept");
    const pack = await f.read("P-DEMO", async () =>
      view(1, { effective_status: "target-unavailable" }),
    );
...
test("one view failure preserves the other invitation and reports the omission", async () => {
  ...
  try {
    ...
    f.add(2);
    const pack = await f.read("P-DEMO", async (id) => {
      if (id === idFor(1)) throw new Error("PRIVATE-SQL-CANARY");
      return view(2);
    });
    assert.deepEqual(
      pack.candidates.map((c) => c.id),
...
test("invalid selection identities never perform a database read", async () => {
  const f = fixture();
  try {
    for (const [problem, viewer] of [
      ["P-DEMO\n", fellow],
      ["P-DEMO", `${fellow}\n`],
      ["P-DEMO/hidden", fellow],
    ]) {
      const pack = await readIncomingReviewInvitationPack(
        f.db,
        problem!,
        viewer!,
        1000,
...
test("an absent migration is a private-context omission, never a failed scientific pack", async () => {
  const db = {
    prepare() {
      throw new Error("no such table: private details");
    },
  } as unknown as D1Database;
  const result = await readIncomingReviewInvitationPack(
    db,
    "P-DEMO",
    fellow,
    1000,
