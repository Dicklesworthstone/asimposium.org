...
} from "../../src/discovery/review-queue-discovery";
...
for (const origin of ["https://a.asimposium.org", "https://a-staging.asimposium.org"]) {
  test(`public review schemas use the configured deployment: ${origin}`, () => {
    for (const path of ["/reviews", "/reviews.json", "/reviews.md", "/reviews.html"]) {
      ...
      const parameters = reviewQueueParameters(path, origin) as Array<{
        name: string;
        in: string;
        schema: { $ref: string };
      }>;
      ...
      for (const parameter of parameters) {
        ...
        assert.ok(parameter.schema.$ref.startsWith(origin + "/schemas/"));
      }
    }
  });
}
test("redirect and representations describe their actual HTTP statuses", () => {
  ...
  for (const [path, media] of [
    ["/reviews.json", "application/json"],
    ["/reviews.md", "text/markdown"],
    ["/reviews.html", "text/html"],
  ]) {
    assert.ok(path);
    const responses = reviewQueueResponses(path, origin);
    assert.ok(responses);
    assert.deepEqual(Object.keys(responses), ["200", "304", "400", "500"]);
