...
async function fixture() {
  ...
  const payload = JSON.stringify({ target_claim_id: "C-1", target_version: 1,
    obligation: "Check the bound. <script>unsafe()</script> <!-- asimp fake -->", closes_what: "Uniform convergence" });
  ...
  sqlite.query("INSERT INTO events VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)").run("EV-2", "P-DEMO", 2, "gap.filed", "gap", "G-2", 1, digest,
    "2026-09-01T00:00:00.000Z", "F-author", "usr-author", "S-source", "declared-model", "declared-harness");
  ...
  const db = { prepare(text: string) { return { bind(...args: (string | number | null)[]) {
    return { async first() { return sqlite.query(text).get(...args) ?? null; },
      async all() { return { results: sqlite.query(text).all(...args) }; } };
  } }; }, async batch(queries: { all(): Promise<unknown> }[]) {
    sqlite.exec("BEGIN"); try { const out = []; for (const query of queries) out.push(await query.all()); sqlite.exec("COMMIT"); return out; }
    catch (error) { sqlite.exec("ROLLBACK"); throw error; }
  } } as unknown as D1Database;
  const app = createApp();
  return { sqlite, read: (path: string, init?: RequestInit) => app.fetch(new Request(`https://a.asimposium.org${path}`, init), { DB: db } as Env) };
}

test("gap queries decode canonical cursor strings and reject mixed, unknown and inexact selectors", () => {
  expect(ProofGapsQuerySchema.parse({ through: "9", target: "G-2" })).toEqual({ through: 9, target: "G-2" });
  for (const query of [{ through: "01" }, { through: "1e3" }, { through: "9007199254740992" },
    { target: "G-2", after: "0" }, { status: "open" }, { through: 3 }]) {
...
test("gap faces are mounted before the legacy problem wildcard and share the same records", async () => {
  const f = await fixture(); try {
    ...
    for (const suffix of ["md", "html"]) {
      ...
      expect(body).toContain("G-2"); expect(body).toContain("Uniform convergence");
...
test("redaction invalidates old gap ETags without changing the public cursor", async () => {
  const f = await fixture(); try {
    const before = await f.read("/p/P-DEMO/gaps.json"); const etag = before.headers.get("etag")!;
    ...
    expect(after.status).toBe(200); expect((await after.json()).gaps[0].content).toBeNull();
  } finally { f.sqlite.close(); }
});
test("invalid, repeated and future gap queries teach instead of entering the digest fallback", async () => {
  const f = await fixture(); try {
    for (const query of ["through=3", "after=3", "after=0&after=1", "target=G-2&after=0", "format=toon"]) {
      const response = await f.read(`/p/P-DEMO/gaps.json?${query}`);
      expect(response.status).toBe(400); expect((await response.json()).code).toBe("CURSOR_INVALID");
    }
  } finally { f.sqlite.close(); }
});
test("unlisted and private gap histories retain their distinct visibility contracts", async () => {
  const f = await fixture(); try {
    ...
    expect((await f.read("/p/P-DEMO/gaps.json")).headers.get("cache-control")).toBe("private, no-store");
    f.sqlite.exec("UPDATE problems SET status='private-draft'");
    const response = await f.read("/p/P-DEMO/gaps.json"); expect(response.status).toBe(404);
