...
test("invitation contracts cannot set scientific status or delegate authority", () => {
  ...
  expect(
    RespondReviewRequestSchema.safeParse({ action: "complete", expected_version: 2 }).success,
  ).toBe(false);
  expect(
    RespondReviewRequestSchema.safeParse({
      action: "complete",
      expected_version: 2,
      review_id: "R-1",
    }).success,
...
test("private views omit raw work products and global private sequence counters", async () => {
  ...
  const row: RequestRecord = {
    ...
    claim_version: 1,
    claim_event_id: "EV-1",
    claim_payload_sha256: await hashText(target),
    author_id: AUTHOR,
    ...
    reviewer_sponsor_id: "usr_reviewer",
    target_json: target,
  };
  ...
  expect((await reviewRequestView({ ...row, target_json: "altered" }, 1500)).effective_status).toBe(
    "target-unavailable",
  );
  expect(
    (
      await reviewRequestView(
        {
          ...row,
          status: "completed",
          version: 3,
          review_event_id: "EV-review",
          target_json: null,
        },
        1500,
      )
...
for (const composed of [false, true]) {
  test(`invitation routes preserve authentication and strict queries (session composition=${composed})`, async () => {
    ...
    const service = {
      credentialBinding: async () => (authorized ? binding : undefined),
    ...
    expect(ReviewRequestsResponseSchema.parse(await list.json()).requests).toEqual([]);
    ...
    expect(
      (
        await send(root, {
          ...
          body: JSON.stringify({ disposition: "proved" }),
        })
      ).status,
    ).toBe(400);
  });
