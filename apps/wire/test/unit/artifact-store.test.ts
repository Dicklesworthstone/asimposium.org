...
import {
  ...
  artifactRoute,
  handleArtifactHttp,
} from "../../src/krater/artifact-http.ts";
...
async function fixture() {
  ...
  for (const [fellow, credential, session] of [
    ["fellow-a", "token-a", `S-${"A".repeat(26)}`],
    ["fellow-b", "token-b", `S-${"B".repeat(26)}`],
  ] as const) {
    sql.query("INSERT INTO enrollment_fellows VALUES (?, 'usr_a', 'active')").run(fellow);
    sql
      .query(
        `INSERT INTO fellow_tokens VALUES (?, ?, 'usr_a', '["upload-artifacts"]', '{}', ?, ?, NULL, 'bearer')`,
      )
      .run(credential, fellow, NOW - 10000, NOW + 7 * 86400000);
    sql
      .query(`INSERT INTO enrollment_grants VALUES (?, 'usr_a', '["upload-artifacts"]', '{}')`)
      .run(fellow);
    sql
      .query("INSERT INTO sessions VALUES (?, ?, 'P-DEMO', ?, ?, NULL)")
      .run(session, fellow, iso(-10000), iso(43200000));
    sql.query("INSERT INTO problem_memberships VALUES ('P-DEMO', ?, 'contributor')").run(fellow);
  }
  sql.exec(
    readFileSync(
      new URL("../../../../db/migrations/0069_artifact_uploads.sql", import.meta.url),
      "utf8",
    ),
  );
  ...
  const prepare = (text: string) => {
    ...
    const execute = () => {
      before?.(text);
      return {
        results: sql.query(text).all(...(binds as never[])),
        meta: {
  ...
  const key = await crypto.subtle.importKey("raw", new Uint8Array(32).fill(19), "AES-GCM", false, [
    "encrypt",
    "decrypt",
  ]);
  ...
  const bucket = {
    async get(key: string) {
      const value = objects.get(key)?.slice();
      onGet?.(key);
    ...
    async put(
      ...
      options: {
        ...
        httpMetadata: { contentDisposition: string };
      },
    ) {
      onPut?.(key);
  ...
  const declaration: ArtifactDeclaration = {
    sessionId: `S-${"A".repeat(26)}`,
    sha256: await artifactSha256(bytes),
    size: bytes.length,
  ...
  const rows = () =>
    sql.query("SELECT * FROM artifact_uploads ORDER BY upload_id").all() as Record<
      string,
      unknown
    >[];
  const audit = () =>
    sql.query("SELECT state FROM artifact_upload_events ORDER BY seq").all() as { state: string }[];
  const grant = (resource: object) => {
    for (const table of ["fellow_tokens", "enrollment_grants"])
      sql.query(`UPDATE ${table} SET granted_resources_json = ?`).run(JSON.stringify(resource));
  };
  return {
    sql,
    db,
    ...
    before: (hook: ((text: string) => void) | undefined) => {
      before = hook;
    },
    onGet: (hook: ((key: string) => void) | undefined) => {
      onGet = hook;
    },
    onPut: (hook: ((key: string) => void) | undefined) => {
      onPut = hook;
    },
  };
}
...
test("manifest reserves private storage and seals the PUT capability; no public event or cursor", async () => {
  await using(async (f) => {
    ...
    assert.equal(receipt.storage, "private");
    assert.match(receipt.put.url, /X-Amz-Signature=/);
    assert.equal(f.rows().length, 1);
    assert.deepEqual(
      f.audit().map((x) => x.state),
      ["presigned"],
    );
    assert.ok(!JSON.stringify(f.rows()).includes("X-Amz-Signature"));
    assert.equal(f.writes.length, 0);
    assert.equal(
      (f.sql.query("SELECT cursor FROM public_cursor").get() as { cursor: number }).cursor,
      7,
    );
...
test("thirty same-key admissions share one reservation and the original sealed URL", async () => {
  await using(async (f) => {
    ...
    f.now(NOW + 60000);
    assert.deepEqual(await f.create(), receipts[0]);
  });
});
...
test("idempotency conflict does not mint a second manifest or extend its deadline", async () => {
  await using(async (f) => {
    ...
    await assert.rejects(
      f.create("request-1", { ...f.declaration, sha256: "a".repeat(64) }),
      failure("CONFLICT"),
    );
    f.now(NOW + 86400000);
    await assert.rejects(f.create(), failure("EXPIRED"));
    assert.equal(f.rows().length, 1);
  });
});
...
for (const [name, mutation] of [
  ...
  [
    ...
    `UPDATE fellow_tokens SET granted_scopes_json = '[]'; UPDATE enrollment_grants SET granted_scopes_json = '[]'`,
  ],
  ...
  test(`transactional admission refuses ${name}`, async () => {
    await using(async (f) => {
      f.sql.exec(mutation);
      await assert.rejects(f.create(), failure("NOT_ALLOWED"));
...
test("grants constrain binding, expiry and event budget inside admission", async () => {
  for (const resource of [
    { problemBinding: "P-OTHER" },
    { fellowGrantExpiresAt: NOW },
    { eventBudget: 0 },
  ]) {
    await using(async (f) => {
      f.grant(resource);
...
test("Fellow rotation and another Fellow cannot reset grant or sponsor budgets", async () => {
  await using(async (f) => {
    ...
    await f.create();
    f.sql.exec(
      "INSERT INTO fellow_tokens SELECT 'rotated', fellow_id, sponsor_id, granted_scopes_json, granted_resources_json, issued_at, expires_at, NULL, credential_profile FROM fellow_tokens WHERE credential_id = 'token-a'",
    );
    await assert.rejects(
      f.create("request-2", f.declaration, { ...actor, credentialId: "rotated" }),
      failure("BUDGET"),
    );
    const cap = { ...config, sponsorDailyBytes: bytes.length };
    await assert.rejects(
      f.create("other-1", { ...f.declaration, sessionId: `S-${"B".repeat(26)}` }, other, cap),
      failure("BUDGET"),
    );
  });
});
...
test("daily count and plan's 200 MiB limit stop unbounded capability issuance", async () => {
  await using(async (f) => {
    ...
    await assert.rejects(
      f.create("r2", f.declaration, actor, { ...config, fellowDailyManifests: 1 }),
      failure("BUDGET"),
    );
  });
  await using(async (f) => {
...
test("uploaded bytes verify into private CAS, download by owner, and survive session close", async () => {
  await using(async (f) => {
    ...
    const result = await f.complete(receipt.upload_id);
...
test("size, digest and secret failures quarantine without publishing or retaining the secret in audit", async () => {
  for (const [body, code, matching] of [
    [new TextEncoder().encode("x"), "MISMATCH", false],
    [new Uint8Array(bytes.length).fill(65), "MISMATCH", false],
    [new TextEncoder().encode("sk_live_" + "x".repeat(30)), "CONTENT_REFUSED", true],
  ] as const) {
    await using(async (f) => {
      const input = matching
        ? { ...f.declaration, sha256: await artifactSha256(body), size: body.length }
        : f.declaration;
      const receipt = await f.create("request-1", input);
      f.objects.set(artifactStagingKey(receipt.upload_id), body);
...
test("concurrent verification has one terminal transition and safely retries busy calls", async () => {
  await using(async (f) => {
    ...
    const results = await Promise.allSettled(
      Array.from({ length: 20 }, () => f.complete(receipt.upload_id)),
    );
...
test("same verified bytes dedupe without exposing another Fellow's manifest", async () => {
  await using(async (f) => {
    ...
    const b = await f.create(
      "other",
      { ...f.declaration, sessionId: `S-${"B".repeat(26)}` },
      other,
    );
    f.objects.set(artifactStagingKey(a.upload_id), bytes);
    f.objects.set(artifactStagingKey(b.upload_id), bytes);
    await f.complete(a.upload_id);
    await f.complete(b.upload_id, other);
...
test("a lost lease cannot finish or clear another verifier's ownership", async () => {
  await using(async (f) => {
    ...
    f.onPut(() => {
      ...
      f.sql
        .query(
          "UPDATE artifact_uploads SET lease_token = ?, lease_until = ?, updated_at = ? WHERE upload_id = ?",
        )
        .run("f".repeat(32), NOW + 120001, NOW + 60001, a.upload_id);
    });
    await assert.rejects(f.complete(a.upload_id), failure("BUSY"));
    assert.equal(f.rows()[0]?.lease_token, "f".repeat(32));
    assert.equal(f.rows()[0]?.state, "presigned");
  });
});
...
test("expiry preserves audit and refuses late completion instead of publishing stale capabilities", async () => {
  await using(async (f) => {
    ...
    f.objects.set(artifactStagingKey(a.upload_id), bytes);
    f.now(NOW + 86400000);
    await assert.rejects(f.complete(a.upload_id), failure("EXPIRED"));
    assert.deepEqual(
      f.audit().map((x) => x.state),
      ["presigned", "expired"],
    );
...
test("audit failure rolls back manifest admission; terminal states and identity cannot be rewritten", async () => {
  await using(async (f) => {
    f.sql.exec(
      "CREATE TRIGGER planted BEFORE INSERT ON artifact_upload_events BEGIN SELECT RAISE(ABORT,'planted'); END",
    );
    await assert.rejects(f.create(), failure("UNAVAILABLE"));
  ...
  await using(async (f) => {
    ...
    f.objects.set(artifactStagingKey(a.upload_id), bytes);
    assert.throws(() =>
      f.sql.exec("UPDATE artifact_uploads SET sha256 = '" + "f".repeat(64) + "'"),
    );
    await f.complete(a.upload_id);
...
test("database constraints refuse incomplete verification and one-sided leases", async () => {
  await using(async (f) => {
    const a = await f.create();
    assert.throws(() =>
      f.sql.exec(`UPDATE artifact_uploads SET lease_token = '${"a".repeat(32)}'`),
    );
    assert.throws(() => f.sql.exec(`UPDATE artifact_uploads SET lease_until = ${NOW + 1000}`));
    f.sql.exec(
      `UPDATE artifact_uploads SET lease_token = '${"a".repeat(32)}', lease_until = ${NOW + 1000}`,
    );
    assert.throws(() =>
      f.sql.exec(
        `UPDATE artifact_uploads SET state='verified',content_type='text/plain; charset=utf-8',lease_token=NULL,lease_until=NULL`,
      ),
    );
    assert.equal(f.rows()[0]?.state, "presigned");
    assert.ok(a.upload_id);
  });
});

function http(f: Awaited<ReturnType<typeof fixture>>, token = "valid-test-token") {
  let authentications = 0;
  const options: ArtifactHttpOptions = {
    db: f.db,
    bucket: f.bucket,
    codec: f.codec,
    signing: config,
    ...
    authenticate: async (presented) => {
      ...
      const c = f.sql
        .query("SELECT * FROM fellow_tokens WHERE credential_id='token-a'")
        .get() as Record<string, unknown>;
      const fellow = f.sql
        .query("SELECT status FROM enrollment_fellows WHERE fellow_id='fellow-a'")
        .get() as { status: string };
      return {
        ...actor,
        grantedScopes: JSON.parse(String(c.granted_scopes_json)),
        grantedResources: JSON.parse(String(c.granted_resources_json)),
        issuedAt: Number(c.issued_at),
        expiresAt: Number(c.expires_at),
        credentialProfile: "bearer",
        fellowStatus: fellow.status,
        ...(c.revoked_at === null ? {} : { revokedAt: Number(c.revoked_at) }),
    ...
    raw(
      new Request(`https://a.asimposium.org${path}`, {
        ...
        headers: {
          ...
          "idempotency-key": "http-manifest-1",
          ...extra,
        },
  ...
  const manifest = () => ({
    session_id: f.declaration.sessionId,
    sha256: f.declaration.sha256,
    size_bytes: f.declaration.size,
    encoding: f.declaration.encoding,
  });
...
test("HTTP completes manifest → PUT bytes → verification → private download and exact replay", async () => {
  await using(async (f) => {
    const h = http(f);
    const created = await h.call("/v1/artifacts", "POST", h.manifest());
    ...
    f.objects.set(artifactStagingKey(receipt.upload_id), bytes);
    const done = await h.call(receipt.complete_path, "POST", {});
    assert.equal(done.status, 200);
    const metadata = (await done.json()) as {
      content_path: string;
      verification: string;
      state: string;
    };
    assert.equal(metadata.verification, "bytes-only");
    assert.equal(metadata.state, "verified");
    assert.deepEqual(await (await h.call(receipt.complete_path, "POST", {})).json(), metadata);
    const status = await h.call(receipt.status_path);
    assert.deepEqual(await status.json(), metadata);
    const download = await h.call(metadata.content_path);
...
test("metadata never returns the signed capability, credential, ciphertext or verifier lease", async () => {
  await using(async (f) => {
    const h = http(f);
    const response = await h.call("/v1/artifacts", "POST", h.manifest());
    const created = (await response.json()) as { status_path: string };
    const read = await h.call(created.status_path);
...
test("forged identities in headers, cookies and JSON cannot obtain an upload", async () => {
  await using(async (f) => {
    ...
    assert.equal(
      (
        await h.call("/v1/artifacts", "POST", h.manifest(), {
          "x-sponsor-id": "usr_a",
          cookie: "token=valid-test-token",
        })
      ).status,
      401,
    );
    ...
    const valid = http(f);
    assert.equal(
      (await valid.call("/v1/artifacts", "POST", { ...valid.manifest(), fellow_id: "fellow-b" }))
        .status,
      422,
    );
    assert.equal(f.rows().length, 0);
...
test("HTTP scope and current grant gates run before capability issuance", async () => {
  await using(async (f) => {
    ...
    const h = http(f);
    assert.equal((await h.call("/v1/artifacts", "POST", h.manifest())).status, 401);
    assert.equal(f.rows().length, 0);
  });
  await using(async (f) => {
    f.grant({ artifactBudgetBytes: bytes.length });
    ...
    assert.equal((await h.call("/v1/artifacts", "POST", h.manifest())).status, 201);
    assert.equal(
      (await h.call("/v1/artifacts", "POST", h.manifest(), { "idempotency-key": "new-key" }))
        .status,
      401,
    );
  });
...
test("HTTP refuses query credentials, unsupported methods, media types and shape changes", async () => {
  await using(async (f) => {
    ...
    assert.equal(
      (await h.call("/v1/artifacts", "POST", h.manifest(), { "content-type": "text/plain" }))
        .status,
      415,
    );
    assert.equal(
      (await h.call("/v1/artifacts", "POST", h.manifest(), { "idempotency-key": "" })).status,
      400,
    );
    assert.equal(
      (await h.call("/v1/artifacts", "POST", { ...h.manifest(), size_bytes: "40" })).status,
      422,
    );
    assert.equal(f.rows().length, 0);
  });
...
test("unknown and another Fellow's upload share the same opaque refusal", async () => {
  await using(async (f) => {
    const otherReceipt = await f.create(
      "other",
      { ...f.declaration, sessionId: `S-${"B".repeat(26)}` },
      other,
    );
    const h = http(f);
    const forbidden = await h.call(`/v1/artifacts/${otherReceipt.upload_id}`);
    const absent = await h.call(`/v1/artifacts/AU-${"e".repeat(32)}`);
    assert.equal(forbidden.status, 401);
    assert.equal(await forbidden.text(), await absent.text());
  });
});
...
test("HTTP content failures disclose neither patterns nor uploaded bytes and retain private state", async () => {
  await using(async (f) => {
    ...
    const receipt = await f.create("bad", {
      ...f.declaration,
    ...
    for (const value of ["sk_live_", "SECRET_SHAPED", "pattern", "example", "schema"])
...
test("HTTP completion cannot change a digest or publish bytes and requires a stable key", async () => {
  await using(async (f) => {
    const a = await f.create();
    const h = http(f);
    const path = `/v1/artifacts/${a.upload_id}/complete`;
...
test("router claims only its canonical private paths", () => {
  ...
  for (const path of [
    ...
    "/v1/artifacts/AU-abc",
    `/v1/artifacts/AU-${"a".repeat(32)}/content/extra`,
  ])
...
test("disabled issuance and operational failures remain private and do not mint capabilities", async () => {
  await using(async (f) => {
    ...
    assert.ok(!(await response.text()).includes("PRIVATE-CONFIG-SENTINEL"));
    assert.equal(f.rows().length, 0);
  });
});
