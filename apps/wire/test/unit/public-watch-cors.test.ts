...
test("public cursor and face validators are visible cross-origin without credential permission", async () => {
  for (const path of [
    "/cursor",
    "/p/P-DEMO.json",
    "/p/P-DEMO/claims/C-1%402.json?through=3",
    "/reviews.json",
  ]) {
    const response = await publicWatchFetch(
      request(path),
      () =>
        new Response("public", {
          headers: {
...
test("writes to a read path receive no new cross-origin authority", async () => {
  ...
  assert.equal(
    await publicWatchFetch(request("/cursor", { method: "POST", body: "{}" }), () => original),
    original,
  );
});
  ...
  for (const status of [304, 404, 410, 429, 503]) {
    const response = await publicWatchFetch(
      ...
      () =>
        new Response(null, {
          status,
          headers: { etag: '"digest"', "retry-after": "60", "cache-control": "no-store" },
        }),
    );
