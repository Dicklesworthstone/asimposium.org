...
export function withdrawalFixture() {
  ...
  sql.exec(
    readFileSync(
      new URL("../../../../db/migrations/0071_scientific_withdrawals.sql", import.meta.url),
      "utf8",
    ),
  );
  const actor: FellowCredentialBinding = {
    ...
    model: "declared",
    harness: "test",
    tokenHash: "unused-test-hash",
    grantedScopes: ["promote", "review"],
    grantedResources: { eventBudget: 100 },
    issuedAt: NOW - 10000,
  ...
  const scopes = JSON.stringify(actor.grantedScopes),
    resources = JSON.stringify(actor.grantedResources);
  sql
    .query("INSERT INTO enrollment_fellows VALUES(?,?,'active')")
    .run(actor.fellowId, actor.sponsorId);
  sql
    .query("INSERT INTO enrollment_grants VALUES(?,?,?,?)")
    .run(actor.fellowId, actor.sponsorId, scopes, resources);
  sql
  ...
  sql
    ...
    .run(SESSION, actor.fellowId, "P-DEMO", iso(NOW - 5000), iso(NOW + 300000));
  sql.query("INSERT INTO problem_memberships VALUES('P-DEMO',?,'contributor')").run(actor.fellowId);
  function prepare(text: string) {
    let values: unknown[] = [];
    const execute = () => ({
      success: true,
      results: sql.query(text).all(...(values as (string | number | null)[])),
  ...
  const append = (
    ...
    const content = JSON.stringify(payload),
      digest = sha(content);
    const seq = (
      sql.query("SELECT public_seq + 1 AS n FROM problems WHERE id=?").get(problem) as { n: number }
    ).n;
    ...
    sql
      ...
      .run(
        ...
        digest,
        author,
        author === "fellow-a" ? "sponsor-a" : "sponsor-b",
        SESSION,
        null,
        iso(now),
      );
  ...
  append(
    ...
    "claim",
    "C-1",
    { claim_id: "C-1", statement: "Every n has the stated bound." },
    "fellow-b",
  );
  sql.exec("INSERT INTO claims VALUES('P-DEMO','C-1',1)");
  sql
    .query("INSERT INTO claim_versions VALUES('P-DEMO','C-1',1,?,?, 'conjecture')")
    .run(`sha256:${sha("statement")}`, "Every n has the stated bound.");
  const source = (
    kind: WithdrawalKind,
    id = kind === "review" ? "REV-one" : "E-one",
    ...
    const payload: Record<string, unknown> =
      kind === "review"
        ...
        : {
            ...
            kind: "computation",
            mode: "confirmatory",
            computed_class: "numerical",
            body_md: "Deliberate public work product.",
            ...extra,
          };
    const event = append(`source-${id}`, `${kind}.created`, kind, id, payload, author);
    if (kind === "review")
      sql
        .query("INSERT INTO reviews VALUES('P-DEMO',?,?,?,?,?,?,?,?)")
        .run(
          id,
          event.id,
          event.seq,
          ...
          String(payload.capable_of_failure),
        );
    else
      sql
        .query("INSERT INTO evidence VALUES('P-DEMO',?,?,?,?,'claim','C-1',1,?)")
        .run(id, event.id, event.seq, author, String(payload.direction));
    const request: WithdrawalRequest = {
      source_event_id: event.id,
      source_digest: `sha256:${event.digest}`,
      reason: "An assumption in my recorded work does not hold.",
    };
  ...
  const options: WithdrawalOptions = {
    ...
    async reserve(input) {
      const prior = sql
        .query(
          "SELECT reservation_id,request_digest FROM public_write_attempt_reservations WHERE fellow_id=? AND route=? AND idempotency_key=?",
        )
      ...
      if (prior) {
        if (prior.request_digest !== input.requestDigest) throw new Error("reservation conflict");
    ...
    async settleFailure(id, held) {
      counts.settlements++;
      sql
        .query(
          "UPDATE public_write_attempt_reservations SET status=? WHERE reservation_id=? AND status='reserved'",
        )
    ...
    async writeLedger(input, projection) {
      ...
      try {
        ...
        sql
          .query(
            "UPDATE events SET writer_credential_id=?,actor_session_id=?,actor_sponsor_id=?,created_at=? WHERE id=?",
          )
          .run(
        ...
        const prepared = projection.statementsAfterEvent({
          eventId: input.eventId,
          sequence: event.seq,
          objectId: input.objectId,
          payloadSha256: event.digest,
        });
        const statements = Array.isArray(prepared) ? prepared : await prepared;
        for (const statement of statements)
          (statement as unknown as ReturnType<typeof prepare>).execute();
        sql.exec("COMMIT");
        committed = true;
        if (failAfter) {
          failAfter = false;
          throw new Error("lost committed acknowledgment");
        }
        return { eventId: input.eventId, seq: event.seq };
      } catch (error) {
        if (!committed) sql.exec("ROLLBACK");
        throw error;
      }
    },
  };
