...
import {
  REVIEW_QUEUE_MAX_RESPONSE_BYTES,
  reviewQueueResponse,
} from "../../src/discovery/review-queue-http";

const request = (method = "GET", etag?: string) =>
  new Request("https://a.asimposium.org/reviews.json", {
    method,
...
for (const format of ["json", "md", "html"] as const) {
  test(`${format}: HEAD, cache headers, weak and multiple ETags preserve representation`, async () => {
    ...
    for (const conditional of [etag, `W/${etag}`, `"another", ${etag}`, "*"]) {
      const next = await reviewQueueResponse(
        request("GET", conditional),
...
test("a withdrawal/omission changes bytes and cannot match an earlier ETag", async () => {
  ...
  const fresh = await reviewQueueResponse(
    request("GET", old.headers.get("etag") ?? ""),
    '{"candidates":[]}',
    "json",
  );
  assert.equal(fresh.status, 200);
  assert.notEqual(old.headers.get("etag"), fresh.headers.get("etag"));
});
test("identical text in different formats cannot share a strong ETag", async () => {
  ...
  const md = await reviewQueueResponse(
    request("GET", json.headers.get("etag") ?? ""),
    "same",
    "md",
  );
...
test("exact byte budget is accepted; oversized UTF-8 is refused without truncation", async () => {
  ...
  await assert.rejects(
    () => reviewQueueResponse(request(), "😀".repeat(REVIEW_QUEUE_MAX_RESPONSE_BYTES / 2), "md"),
    /TOO_LARGE/,
  );
});
