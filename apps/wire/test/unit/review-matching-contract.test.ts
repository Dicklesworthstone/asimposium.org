...
test("create contract admits exactly one reviewer-selection mode", () => {
  ...
  assert.ok(
    CreateReviewRequestSchema.safeParse({
      claim_id: "C-1",
      claim_version: 1,
      reviewer_id: matchFellow(1),
    }).success,
  );
  for (const invalid of [
    { claim_id: "C-1", claim_version: 1 },
    { ...body, reviewer_id: matchFellow(1) },
    { ...body, match: "random" },
    { ...body, match: true },
    { ...body, match: { family: "beta" } },
    { ...body, match: undefined },
    { ...body, role: "steward" },
    { ...body, tier: "T3" },
    { ...body, author_id: MATCH_AUTHOR },
    { ...body, cursor: 1000 },
    { ...body, eligibility: { credential_id: "cred-1" } },
    { ...body, claim_version: 0 },
  ])
    assert.equal(
      CreateReviewRequestSchema.safeParse(invalid).success,
      false,
...
function fixture() {
  ...
  const author: FellowCredentialBinding = {
  ...
  const service = {
    async credentialBinding(token: string) {
      return token === "test-author" ? author : undefined;
    },
  } as unknown as EnrollmentService;
  const app = createReviewRequestRouter({
    service,
    replayProtector: {
      async seal(value, context) {
        return {
          ciphertext: JSON.stringify({ value, context }),
          initializationVector: "fixture-only",
        };
      },
      async open(value, context) {
        const r = JSON.parse(value.ciphertext);
  ...
  return {
    ...
    app,
    env,
    post: (value: unknown, key = "match-1", extra: Record<string, string> = {}) =>
      app.request(
        "http://localhost/v1/p/P-DEMO/review-requests",
        {
          method: "POST",
          headers: {
            authorization: "Bearer test-author",
...
test("mounted POST creates and replays one private matched invitation", async () => {
  ...
  try {
    ...
    const receipt = ReviewRequestReceiptSchema.parse(await response.json());
...
test("mounted no-match is an explicit bounded 409, not an authorization failure or empty success", async () => {
  ...
  try {
    ...
    assert.equal(error.matching_result, "no-match-in-bounded-roster");
    assert.match(error.detail, /does not establish/);
...
test("GET and ambiguous writes never initiate matching or deliver notices", async () => {
  ...
  try {
    f.publish(f.person(1), "beta");
    const get = await f.app.request(
      "http://localhost/v1/p/P-DEMO/review-requests?match=different-family",
      { headers: { authorization: "Bearer test-author" } },
      f.env,
    );
