...
import { REVIEW_ACCEPTED_MS, REVIEW_OFFER_MS, effectiveReviewRequestStatus, transitionReviewRequest } from "../../src/review-requests/model.ts";
const now = 1_800_000_000_000;
const offered = { version: 1, status: "offered" as const, occurred_at: now, expires_at: now + REVIEW_OFFER_MS };
...
test("recipient accepts once; accepting does not award a scientific tier", () => {
  assert.deepEqual(transitionReviewRequest(offered, "accept", 1, "reviewer", now + 1), {
    version: 2, status: "accepted", occurred_at: now + 1, expires_at: now + 1 + REVIEW_ACCEPTED_MS,
  });
});
for (const [action, participant] of [["accept", "author"], ["decline", "author"], ["cancel", "reviewer"], ["complete", "author"]] as const) {
...
test("stale versions and backward clocks cannot change request state", () => {
  ...
  assert.throws(() => transitionReviewRequest(offered, "accept", 1, "reviewer", offered.expires_at));
});
test("late opt-out is allowed, but terminal requests cannot be revived", () => {
  const declined = transitionReviewRequest(offered, "decline", 1, "reviewer", offered.expires_at + 1);
  assert.equal(declined.status, "declined");
  assert.throws(() => transitionReviewRequest(declined, "accept", 2, "reviewer", declined.occurred_at));
});
function fixture() {
  ...
  db.run(readFileSync(new URL("../../../../db/migrations/0065_review_requests.sql", import.meta.url), "utf8"));
  db.query(`INSERT INTO review_requests(request_id,problem_id,claim_id,claim_version,claim_event_id,claim_payload_sha256,author_id,author_sponsor_id,reviewer_id,reviewer_sponsor_id,created_at)
    VALUES('RR-demo','P-DEMO','C-1',1,'claim-event','digest','author','usr_author','reviewer','usr_other',?)`).run(now);
  const append = (version: number, action: string, actor = "reviewer", at = now + version, expiry = now + REVIEW_OFFER_MS, review: string | null = null) =>
    db.query("INSERT INTO review_request_events VALUES(?, 'RR-demo', ?, ?, ?, ?, ?, ?, 1)")
...
test("SQLite enforces actor, legal transition and compare-and-swap independently of TypeScript", () => {
  ...
  try {
    ...
    assert.equal((db.query("SELECT COUNT(*) n FROM review_request_events").get() as { n: number }).n, 2);
    assert.equal((db.query("SELECT public_seq FROM problems").get() as { public_seq: number }).public_seq, 10);
  } finally { db.close(); }
});
test("SQLite preserves immutable request identities and audit history", () => {
  const { db } = fixture();
  try {
    for (const sql of ["UPDATE review_requests SET claim_version=2", "DELETE FROM review_requests",
      "UPDATE review_request_events SET actor_id='stranger'", "DELETE FROM review_request_events",
...
test("SQLite refuses expiry bypass and duplicate invitations after a decline", () => {
  const { db, append } = fixture();
  try {
    assert.throws(() => append(2, "accept", "reviewer", offered.expires_at, offered.expires_at + REVIEW_ACCEPTED_MS));
    append(2, "decline", "reviewer", offered.expires_at);
    assert.throws(() => db.run(`INSERT INTO review_requests(request_id,problem_id,claim_id,claim_version,claim_event_id,claim_payload_sha256,author_id,author_sponsor_id,reviewer_id,reviewer_sponsor_id,created_at)
...
test("completion requires the recipient's exact-version committed review, not a status assertion", () => {
  ...
  try {
    ...
    assert.equal((db.query("SELECT action FROM review_request_events ORDER BY version DESC LIMIT 1").get() as { action: string }).action, "complete");
  } finally { db.close(); }
});
test("a replay collision rolls back both a transition and its notice", () => {
  ...
  try {
    ...
    db.query("INSERT INTO review_request_replays VALUES('reviewer','same','original','ciphertext','iv',?)").run(now + 86400000);
    db.run("BEGIN");
    try {
      append(2, "decline");
    ...
    assert.equal((db.query("SELECT COUNT(*) n FROM review_request_events").get() as { n: number }).n, 1);
    assert.equal((db.query("SELECT COUNT(*) n FROM notices").get() as { n: number }).n, 0);
    assert.equal((db.query("SELECT request_digest FROM review_request_replays").get() as { request_digest: string }).request_digest, "original");
  } finally { db.close(); }
});
