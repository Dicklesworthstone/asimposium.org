...
import {
  deliverHeraldRooms,
  HERALD_DELIVERY_LIMITS,
  type HeraldDatabase,
  type HeraldNamespace,
  publicRoomHead,
...
function fixture() {
  ...
  sql.exec(
    readFileSync(
      new URL("../../../../db/migrations/0078_herald_room_outbox.sql", import.meta.url),
      "utf8",
    ),
  );
  ...
  const namespace: HeraldNamespace = {
  ...
  const pending = () =>
    sql
      .prepare("SELECT * FROM herald_room_outbox WHERE generation > delivered_generation")
      .all() as {
      generation: number;
      delivered_generation: number;
      retry_at: number;
      attempts: number;
    }[];
  return {
...
describe("W7 coalesced transactional wake queue on real SQLite", () => {
  ...
  test("rollback rolls back the wake generation together with the public write", () => {
    ...
    try {
      ...
      assert.equal(
        (
          f.sql.prepare("SELECT public_seq FROM problems WHERE id='P-DEMO'").get() as {
            public_seq: number;
          }
        ).public_seq,
        1,
  ...
  test("many public commits coalesce to one pending row and one metadata-only nudge", async () => {
    ...
    try {
      ...
      assert.equal(f.pending().length, 1);
      assert.equal(f.pending()[0]!.generation, 30);
  ...
  test("refused or failed delivery remains retryable, with bounded backoff", async () => {
    ...
    try {
      f.change();
      f.send(async () => new Response("PRIVATE-DRIVER-CANARY", { status: 503 }));
      const first = await deliverHeraldRooms(f.db, f.namespace, () => NOW);
      assert.equal(first.retry, 1);
      assert.equal(f.pending()[0]!.retry_at, NOW + 1000);
  ...
  test("slow failed delivery starts backoff after the attempt ends", async () => {
    ...
    try {
      ...
      assert.equal(f.pending()[0]!.retry_at, NOW + 6000);
  ...
  test("a failed old attempt cannot postpone a new generation", async () => {
    ...
    try {
      ...
      assert.equal(f.pending()[0]!.generation, 2);
      assert.equal(f.pending()[0]!.retry_at, 0);
    } finally {
      f.sql.close();
    }
  });
  test("already delivered rows remain as monotonic generation witnesses, not ABA-prone deletes", async () => {
    ...
    try {
      f.change();
      await deliverHeraldRooms(f.db, f.namespace, () => NOW);
      f.change();
      assert.equal(f.pending()[0]!.generation, 2);
      assert.equal(f.pending()[0]!.delivered_generation, 1);
    } finally {
      f.sql.close();
    }
  });
  test("workshop/private-draft edits do not enqueue public room wake-ups", () => {
    ...
    try {
      f.sql.exec(
        "UPDATE workshop_objects SET body='NEW-PRIVATE-CANARY'; UPDATE problems SET public_seq=1 WHERE id='P-PRIVATE';",
      );
      assert.equal(f.pending().length, 0);
    } finally {
      f.sql.close();
  ...
  test("public-to-private and unlisting changes enqueue invalidation without advancing the cursor", () => {
    ...
    try {
      f.sql.exec(
        "UPDATE problems SET unlisted=1 WHERE id='P-DEMO'; UPDATE problems SET status='private-draft' WHERE id='P-DEMO';",
      );
      assert.equal(f.pending()[0]!.generation, 2);
    } finally {
      f.sql.close();
  ...
  test("redaction, purging and content restoration enqueue same-cursor refresh", () => {
    ...
    try {
      f.sql.exec(
        "UPDATE event_content SET redacted_at='2026-09-22T00:00:00.000Z'; DELETE FROM event_content; INSERT INTO event_content VALUES ('E-1',NULL,'PRIVATE-RESTORED-CANARY');",
      );
  ...
  test("replaying an unchanged projection does not manufacture a wake-up", () => {
    const f = fixture();
    try {
      f.sql.exec("UPDATE problems SET public_seq=public_seq, unlisted=unlisted, status=status");
      assert.equal(f.pending().length, 0);
    } finally {
      f.sql.close();
    }
  ...
  test("batch size bounds delivery work and subsequent calls resume pending rooms", async () => {
    ...
    try {
      for (let i = 0; i < 9; i++)
        f.sql.prepare("INSERT INTO problems VALUES (?,0,'active',0)").run(`P-${i}`);
      assert.equal(
        (await deliverHeraldRooms(f.db, f.namespace, () => NOW)).scanned,
        HERALD_DELIVERY_LIMITS.batch,
      );
  ...
  test("an unavailable room cannot monopolize later pending work", async () => {
    ...
    try {
      for (let i = 0; i < 6; i++)
        f.sql.prepare("INSERT INTO problems VALUES (?,0,'active',0)").run(`P-${i}`);
      f.send(
        async (request) => new Response(null, { status: request.url.includes("P-0/") ? 503 : 204 }),
      );
      const one = await deliverHeraldRooms(f.db, f.namespace, () => NOW);
      assert.equal(one.retry, 1);
      const two = await deliverHeraldRooms(f.db, f.namespace, () => NOW);
  ...
  test("public head is current, read-only, problem-local and distinguishes no private existence", async () => {
    ...
    try {
      ...
      f.change();
      assert.deepEqual(await publicRoomHead(f.db, "P-DEMO"), { seq: 2 });
