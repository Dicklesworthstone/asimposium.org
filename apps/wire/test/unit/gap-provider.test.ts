...
const actor = {
  fellowId: "F-reader",
  sponsorId: "usr-reader",
  credentialId: "CR-reader",
  fellowStatus: "active",
  grantedResources: {},
...
function move(problem: string): NextMoveCandidate {
  return {
    move: "close-gap",
    refs: [problem, "G-2", "C-1@1"],
    why: "A recorded unowned obligation.",
    contract: { captured_cursor: 12 },
    selection_boundary: "explicit-gap-fixture",
  };
}
function template(kind: "review" | "add-refuter" | "state-claim"): MoveTemplate {
  return {
    ...
    title: kind,
    trigger: "recorded need",
    description: "Investigate recorded work.",
    request: {
      method: "POST",
...
function fixture() {
  ...
  const dependencies: LiveMovesDependencies = {
    ...
    gaps: {
      load: async (_db, problem, cursor, now) => {
        calls.push({ problem, cursor, now });
        if (gapFails) throw new Error("gap-private-details");
        return { move: move(problem), degraded: false };
      },
    },
  };
  const provider = new LedgerMovesProvider(dependencies);
  const request = (credential = actor) => ({
    db,
    credential,
  ...
  return {
    ...
    set(patch: { review?: boolean; queueFails?: boolean; gapFails?: boolean; deny?: boolean }) {
      ...
      queueFails = patch.queueFails ?? queueFails;
      gapFails = patch.gapFails ?? gapFails;
      deny = patch.deny ?? deny;
    },
    close() {
      sqlite.close();
    },
...
test("production provider passes its current membership snapshot and clock into gap selection", async () => {
  ...
  try {
    ...
    assert.equal(result.primaryMove?.move, "close-gap");
...
test("removal from the durable roster overrides a caller's steward hint", async () => {
  const f = fixture();
  try {
    f.sqlite.exec("DELETE FROM problem_memberships");
    assert.equal((await f.provider.nextMoves(f.request())).primaryMove, null);
    assert.equal(f.calls.length, 0);
  } finally {
    f.close();
...
test("credential-wide usage is supplied before authorizing gap work", async () => {
  const f = fixture();
  try {
    f.sqlite.exec("INSERT INTO events VALUES('P-OTHER',1,'claim','claim.created','CR-reader')");
    const result = await f.provider.nextMoves(
      f.request({ ...actor, grantedResources: { eventBudget: 1 } }),
    );
    assert.equal(result.primaryMove, null);
    assert.equal(f.calls.length, 0);
    assert.ok(
      f.authCalls.some(
        (value: any) => value.effect === "promote" && value.usage.eventsRecorded === 1,
      ),
    );
  } finally {
    f.close();
...
test("existing review work remains primary with an unowned gap as an alternative", async () => {
  ...
  try {
    ...
    assert.equal(result.alternatives[0]?.move, "close-gap");
  } finally {
    f.close();
  }
});
...
test("gap source outage preserves independent review work", async () => {
  const f = fixture();
  try {
    f.set({ review: true, gapFails: true });
    const result = await f.provider.nextMoves(f.request());
    assert.equal(result.primaryMove?.move, "review");
    assert.equal(result.degraded, true);
...
test("a board without any published claim never invents a claim-bound gap", async () => {
  const f = fixture();
  try {
    f.sqlite.exec("DELETE FROM events");
...
test("triage chooses gap work ahead of empty boards and bounds per-problem concurrency", async () => {
  ...
  try {
    ...
    for (const id of ids) {
      ...
      if (id !== "P-AA")
        f.sqlite
          .query("INSERT INTO events VALUES(?,1,'claim','claim.created','CR-author')")
          .run(id);
    }
    let active = 0,
      peak = 0;
    f.dependencies.gaps!.load = async (_db, problem, cursor, now) => {
      f.calls.push({ problem, cursor, now });
      active++;
    ...
    const triage = await f.provider.triageMove({
      db: f.db,
      credential: actor,
      fellowId: actor.fellowId,
      assignments: ids.reverse().map((problem_id) => ({ problem_id })) as any,
    });
    assert.equal(triage.move?.move, "close-gap");
    assert.equal(triage.move?.refs[0], "P-BB");
    assert.equal(triage.degraded, true);
    assert.ok(peak <= 2);
