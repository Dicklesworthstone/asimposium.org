...
const item: VerifiedDeadEndRetry = {
  ...
  firing: { event_id: "FIRED", seq: 10, payload_sha256: "b".repeat(64) },
  source: {
    approach: "UNTRUSTED-APPROACH",
    why_it_fails: "UNTRUSTED-FAILURE",
    retry_predicate: "UNTRUSTED-PREDICATE",
    retry_when: { kind: "statement-revised" },
  },
};
const permissions = { session_open: true, promote: true, workshop_push: true };
const fake = (move: string) =>
  ({ move, refs: ["P-DEMO", move], why: "fixture" }) as NextMoveCandidate;

test("retry action is private investigation with no fabricated result or copied instruction", () => {
  const move = retryMoveFor(item, "F-1", template)!;
  const contract = move.contract as Record<string, unknown>;
  const request = contract.request as { path: string };
  const prep = contract.preparation as {
    dead_end_pin: { event_id: string };
    firing_pin: { event_id: string };
    author_may_supersede: boolean;
  };
  assert.equal(request.path, "/v1/sessions/{id}/workshop");
  assert.deepEqual(contract.required_fields, ["type", "title", "body_md"]);
  assert.deepEqual(contract.prefilled_hints, { type: "scratch" });
  assert.ok(!JSON.stringify(move).includes("UNTRUSTED"));
  assert.ok(!JSON.stringify(move).includes("invented failure"));
  assert.equal(prep.dead_end_pin.event_id, "SOURCE");
  assert.equal(prep.firing_pin.event_id, "FIRED");
  assert.equal(prep.author_may_supersede, true);
});

test("another Fellow receives investigation, not authority to supersede someone else's negative result", () => {
  const move = retryMoveFor(item, "F-2", template)!;
  const contract = move.contract as {
    preparation: { author_may_supersede: boolean };
    prefilled_hints: Record<string, unknown>;
  };
...
test("typed condition reads distinguish exact gap, claim and current statement", () => {
  for (const trigger of [
    ...
    { kind: "claim-reaches", claim_id: "C-2", reaches: "corroborated" },
  ] as const) {
    const selected = { ...item, source: { ...item.source, retry_when: trigger } };
    const contract = retryMoveFor(selected, "F-1", template)!.contract as {
      preparation: { additional_reads: Array<{ path: string }> };
    };
    const paths = contract.preparation.additional_reads.map((x) => x.path);
    assert.ok(paths.includes("/p/P-DEMO/events.json?since=9"));
    assert.ok(
      paths.includes(
        trigger.kind === "gap-closed"
          ? "/p/P-DEMO/gaps.md?through=20&target=G-3"
          : trigger.kind === "claim-reaches"
            ? "/p/P-DEMO/claims/C-2.md?through=20"
            : "/p/P-DEMO.md",
      ),
    );
  }
...
test("filtered first page advances to a real retry without mixing cursors", async () => {
  const seen: unknown[] = [];
  const result = await loadRetryMove(db, "P-DEMO", 20, "F-1", {
    template: () => template,
    async page(_db, p, c, after) {
      assert.equal(p, "P-DEMO");
      assert.equal(c, 20);
      seen.push(after);
      return after
        ? { items: [item], next: null, omitted: [] }
        : {
            items: [],
            next: { seq: 8, id: "DE-0" },
            omitted: ["content_unavailable", "page_limit"],
          };
    },
  });
...
test("only two pages are examined; filtered backlog is explicit partial discovery", async () => {
  ...
  const result = await loadRetryMove(db, "P-DEMO", 20, "F-1", {
    ...
    async page() {
      calls++;
      return { items: [], omitted: ["page_limit"], next: { seq: calls, id: `DE-${calls}` } };
    },
  });
...
test("wrong scope, cursor and nonadvancing continuation are refused", async () => {
  for (const bad of [
    ...
    await assert.rejects(
      loadRetryMove(db, "P-DEMO", 20, "F-1", {
        template: () => template,
        async page() {
          return { items: [bad], next: null, omitted: [] };
        },
      }),
      /INVALID/,
    );
  ...
  await assert.rejects(
    loadRetryMove(db, "P-DEMO", 20, "F-1", {
      template: () => template,
      async page() {
        return { items: [], next: { seq: 1, id: "DE-1" }, omitted: ["page_limit"] };
      },
    }),
    /INVALID/,
  );
...
test("review and gap work precede retries; retries precede new exploration", async () => {
  ...
  const result = await withRetryMove(
    db,
    "P-DEMO",
    20,
    "F-1",
    ...
    {
      async load() {
        return { move: retryMoveFor(item, "F-1", template), degraded: false };
      },
    },
  );
  assert.deepEqual(
    result.moves.map((x) => x.move),
    ["review", "close-gap", "retry-dead-end", "third-alternative", "state-claim"],
  );
});
for (const denied of ["session_open", "promote", "workshop_push"])
  test(`missing ${denied} skips retry source entirely`, async () => {
    ...
    const result = await withRetryMove(
      ...
      {
        async load() {
          assert.fail("unauthorized discovery");
        },
      },
    );
...
test("retry outage preserves unrelated valid recommendations with a partial flag", async () => {
  ...
  assert.equal(result.moves, moves);
  assert.ok(result.degraded);
  assert.ok(!JSON.stringify(result).includes("database details"));
});
