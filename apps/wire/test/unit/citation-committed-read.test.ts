...
function fixture() {
  ...
  async function event(
    ...
    const type = kind === "citation" ? (version === 1 ? "citation.recorded" : "citation.corrected")
      : kind === "claim" ? (version === 1 ? "claim.created" : "claim.revised") : "evidence.created";
    sqlite.query("INSERT INTO events VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
      .run(eventId, problem, seq, type, kind, id, version, hash, "2026-09-15T00:00:00.000Z",
        "F-AUTHOR", "SP-AUTHOR", "S-SESSION", "model-declared", "harness-declared");
...
function citation(id = "L-1", title = "Published source", unanchored = true) {
  return { citation_id: id, title, authors: ["Example Author"], year: 2026,
    locator_kind: "doi", locator: "10.1234/example", canonical_locator: "10.1234/example",
    excerpt: "Published bounded excerpt", retrieved_at: "2026-09-15", source_provenance: "retrieved",
    unanchored, norm_hash: `citation:doi:${id}` };
}
...
test("redacted head never revives an earlier version; an available exact historical version remains readable", async () => {
  ...
  try {
    ...
    sqlite.query("UPDATE event_content SET redacted_at = 'redacted' WHERE event_id = ?").run("P-TEST-5");
    ...
    assert.deepEqual(historical?.versions.map((version) => version.version), [1]);
...
test("current redaction also wins over an old cursor", async () => {
  ...
  try {
    ...
    assert.equal((await loadCommittedCitations(db, "P-TEST", { through: 1 }, decodeFixture)).citations.length, 0);
    assert.equal(await loadCommittedCitation(db, "P-TEST", "L-1@1", { through: 1 }, decodeFixture), null);
  } finally { sqlite.close(); }
});
...
test("tampered and missing content cannot be replaced by retained projections", async () => {
  for (const mutation of [
    ...
    try {
      ...
      assert.equal((await loadCommittedCitations(db, "P-TEST", {}, decodeFixture)).citations.length, 0);
      assert.equal(await loadCommittedCitation(db, "P-TEST", "L-1", {}, decodeFixture), null);
    } finally { sqlite.close(); }
  }
});
...
test("public cut excludes later events and all private-draft content", async () => {
  ...
  try {
    ...
    assert.equal((await loadCommittedCitation(db, "P-TEST", "L-1", {}, decodeFixture))?.citation.version, 1);
    assert.equal(await loadCommittedCitation(db, "P-TEST", "L-1@2", {}, decodeFixture), null);
...
test("backlinks match exact IDs and versions and retain the matched claim version", async () => {
  ...
  try {
    ...
    assert.deepEqual(exact?.associated_claims.map((claim) => claim.claim_id), ["C-2"]);
    const head = await loadCommittedCitation(db, "P-TEST", "L-1", {}, decodeFixture);
    assert.deepEqual(head?.associated_claims.map((claim) => claim.claim_id), ["C-2", "C-4"]);
    await event(6, "C-2", 2, { statement: "The revision no longer cites that source." }, "claim");
    assert.equal((await loadCommittedCitation(db, "P-TEST", "L-1@1", {}, decodeFixture))?.associated_claims.length, 0);
...
test("backlink content is digest-verified and redaction-safe, including evidence", async () => {
  ...
  try {
    ...
    sqlite.query("UPDATE event_content SET redacted_at = 'redacted' WHERE event_id = 'P-TEST-2'").run();
    sqlite.query("UPDATE event_content SET payload_json = payload_json || ' ' WHERE event_id = 'P-TEST-3'").run();
...
test("attribution is taken from the event and unknown payload fields are not exported", async () => {
  const { sqlite, db, event } = fixture();
  try {
    await event(1, "L-1", 1, { ...citation(), sponsor_id: "SP-SPOOF", private_workshop: "PRIVATE_CANARY" });
    const result = await loadCommittedCitation(db, "P-TEST", "L-1@1", {}, decodeFixture);
...
test("lists preserve numeric citation order and disclose filter and count bounds", async () => {
  ...
  try {
    ...
    assert.deepEqual(all.citations.map((item) => item.citation_id), ["L-1", "L-2", "L-10"]);
    const filtered = await loadCommittedCitations(db, "P-TEST", { limit: 1, unanchored: true }, decodeFixture);
...
test("invalid version syntax and impossible read bounds fail before SQL", async () => {
  const db = { prepare: () => { throw new Error("unexpected SQL"); }, batch: () => { throw new Error("unexpected SQL"); } } as unknown as D1Database;
  for (const target of ["L-1@0", "L-1@01", "L-1@2junk", "L-1@9007199254740992", "L-1/../../", "L-1@1@2"]) {
  ...
  for (const through of [-1, 1.5, NaN, Infinity, Number.MAX_SAFE_INTEGER + 1]) {
    await assert.rejects(loadCommittedCitations(db, "P-TEST", { through }, decodeFixture), /Invalid citation read bounds/);
  }
});
...
test("duplicate citation versions refuse ambiguous history rather than selecting arbitrary bytes", async () => {
  ...
  try {
    ...
    await assert.rejects(loadCommittedCitation(db, "P-TEST", "L-1", {}, decodeFixture), /Citation history is unavailable/);
  } finally { sqlite.close(); }
});
