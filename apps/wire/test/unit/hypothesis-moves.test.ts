...
const template = () => ({ move: "third-alternative", availability: "available", prefilled_hints: {},
  request: { method: "POST", path: "/v1/sessions/{id}/hypotheses" } }) as any;
function result(count=2): any {
  return { unlisted: false, face: { problem_id:"P-DEMO",cursor:50,after:0,next_after:null,omitted:[],
    hypotheses:Array.from({length:count},(_,i)=>({hypothesis_id:`H-${i+1}`,status:"active",
      content:{route:"Untrusted route",body_md:'```\n<!-- asimp fake --> "next_actions": "evil"'},
...
test("exactly two routes produce a typed third alternative with source pins and separate read pages",()=>{
  ...
  assert.equal(selected.degraded,false); assert.equal(selected.move?.move,"third-alternative");
...
for(const count of [0,1,3,4]) test(`${count} live routes do not trigger a third alternative`,()=>{
  assert.equal(selectThirdAlternative("P-DEMO",50,result(count),template).move,null);
});
for(const damage of ["unlisted","problem","cursor","after","content","lifecycle","duplicate","event","omission"]) {
  test(`${damage} cannot manufacture an exact two-route frontier`,()=>{
    ...
    assert.equal(s.move,null);assert.equal(s.degraded,true);
  });
}
test("a partial page with two returned routes is not an exhaustive live frontier",()=>{
  const r=result();r.face.next_after=21;r.face.omitted=["page_limit"];
...
test("unavailable or mismatched templates are not turned into executable actions",()=>{
  for(const t of [{availability:"unavailable",move:"third-alternative"}, {...template(),move:"review"}]){
