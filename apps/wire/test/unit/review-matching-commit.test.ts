...
import { commitRequest, type ReplayProtector, type RequestCommand, REQUEST_SCHEMA, requestReplay } from "../../src/review-requests/store.ts";
import type { ReviewMatch } from "../../src/review-requests/matching.ts";
import { REVIEW_OFFER_MS } from "../../src/review-requests/model.ts";
import { reviewMatchingFixture, matchFellow, matchHash, MATCH_AUTHOR } from "./review-matching-fixture.ts";
...
async function protector(): Promise<ReplayProtector> {
  ...
  return {
    async seal(value,context="") {
      ...
      const encrypted=await crypto.subtle.encrypt({name:"AES-GCM",iv,additionalData:text.encode(context)},key,text.encode(value));
    ...
    async open(value,context="") {
      const decrypted=await crypto.subtle.decrypt({name:"AES-GCM",iv:Buffer.from(value.initializationVector,"base64"),additionalData:text.encode(context)},key,Buffer.from(value.ciphertext,"base64"));
...
function command(f:Fixture,match:ReviewMatch,key="match-1"): RequestCommand {
  const actor: FellowCredentialBinding={fellowId:MATCH_AUTHOR,sponsorId:"usr_author",credentialId:"cred-0",
    tokenHash:matchHash(MATCH_AUTHOR),name:"author",model:"model/label",harness:"harness",issuedAt:10,expiresAt:f.now+100000,
    credentialProfile:"bearer",fellowStatus:"active",grantedScopes:["promote","review"],grantedResources:{}};
  return { receipt:{schema:REQUEST_SCHEMA,request_id:`RR-${crypto.randomUUID().replaceAll("-","")}`,
    problem_id:"P-DEMO",claim_id:"C-1",claim_version:1,claim_event_id:f.target.pin.event_id,
    claim_payload_sha256:f.target.pin.digest,author_id:MATCH_AUTHOR,reviewer_id:match.reviewerId,
    version:1,status:"offered",created_at:f.now,updated_at:f.now,expires_at:f.now+REVIEW_OFFER_MS,review_event_id:null},
    actor,role:"contributor",action:"offer",scope:"promote",cursor:f.target.cursor,
    authorSponsor:"usr_author",reviewerSponsor:match.reviewerSponsor,pins:[f.target.pin,match.provenancePin],
    match:match.eligibility,idempotencyKey:key,requestDigest:matchHash(JSON.stringify({match:"different-family",target:"C-1@1"})),
    eventId:`RRE-${crypto.randomUUID().replaceAll("-","")}` };
}
function counts(f:Fixture) {
  return ["review_requests","review_request_events","fellow_inbox_notices","review_request_replays"].map(table=>f.rows(`SELECT COUNT(*) AS n FROM ${table}`)[0]?.n);
}
async function prepared() {
  const f=reviewMatchingFixture(); f.publish(f.person(1),"beta");
  const matched=await f.match(); assert.ok(matched);
...
test("matched invitation, private notice and sealed replay settle without a scientific event",async()=>{
  const {f,c,p}=await prepared(); try {
    ...
    assert.deepEqual(receipt,c.receipt); assert.deepEqual(counts(f),[1,1,1,1]);
    ...
    assert.ok(!notice.includes("deliberate public work product")); assert.ok(!notice.includes("binding_json"));
    const stored=f.rows("SELECT * FROM review_request_replays")[0];
    assert.ok(!JSON.stringify(stored).includes(c.receipt.reviewer_id));
    assert.deepEqual(await requestReplay(f.db,p,MATCH_AUTHOR,c.idempotencyKey,c.requestDigest,f.now),receipt);
...
test("another problem spending the recipient's last event budget aborts settlement",async()=>{
  const f=reviewMatchingFixture(); try {
    const reviewer=f.person(1);f.publish(reviewer,"beta");
    f.sqlite.query("UPDATE fellow_tokens SET granted_resources_json='{\"eventBudget\":1}' WHERE credential_id='cred-1'").run();
    const match=await f.match(); assert.ok(match); const c=command(f,match);
    const event=f.publish(reviewer,"beta",{problem:"P-OTHER"});
    f.sqlite.query("UPDATE events SET writer_credential_id='cred-1' WHERE id=?").run(event.event_id);
    await assert.rejects(commitRequest(f.db,await protector(),c),/CONFLICT/); assert.deepEqual(counts(f),[0,0,0,0]);
  } finally {f.sqlite.close();}
});
...
test("a narrower current Fellow-grant budget is also rechecked at settlement",async()=>{
  const f=reviewMatchingFixture(); try {
    const reviewer=f.person(1);f.publish(reviewer,"beta");
    f.sqlite.query("UPDATE enrollment_grants SET granted_resources_json='{\"eventBudget\":1}' WHERE fellow_id=?").run(reviewer);
    const match=await f.match(); assert.ok(match); const c=command(f,match);
    const event=f.publish(reviewer,"beta",{problem:"P-OTHER"});
    f.sqlite.query("UPDATE events SET writer_credential_id='cred-1' WHERE id=?").run(event.event_id);
    await assert.rejects(commitRequest(f.db,await protector(),c),/CONFLICT/); assert.deepEqual(counts(f),[0,0,0,0]);
...
test("a competing invitation to another recipient prevents automatic fan-out atomically",async()=>{
  const {f,c,p}=await prepared(); try {
    f.publish(f.person(2),"beta"); f.invite(2,"offer");
    await assert.rejects(commitRequest(f.db,p,c),/CONFLICT/);
...
test("same-key stale selection replays the original recipient instead of rematching",async()=>{
  const {f,c,p}=await prepared(); try {
    ...
    const stale={...c,receipt:{...c.receipt,reviewer_id:matchFellow(2),request_id:`RR-${"a".repeat(32)}`}};
    assert.deepEqual(await commitRequest(f.db,p,stale),original);
    assert.deepEqual(counts(f),[1,1,1,1]);
    await assert.rejects(commitRequest(f.db,p,{...stale,requestDigest:matchHash("changed")}),/IDEMPOTENCY_CONFLICT/);
  } finally {f.sqlite.close();}
});
...
test("a same-key winner landing after the first replay check wins all effects",async()=>{
  const {f,c,p}=await prepared(); try {
    let injected=false;
    const loser={...c,eventId:`RRE-${"b".repeat(32)}`,receipt:{...c.receipt,request_id:`RR-${"b".repeat(32)}`}};
    const interleaved:ReplayProtector={open:p.open,async seal(value,context){
      if(!injected){injected=true; await commitRequest(f.db,p,c);}
...
test("declined work can be explicitly rematched to another Fellow without deleting history",async()=>{
  const {f,c,p}=await prepared(); try {
    f.publish(f.person(2),"gamma"); await commitRequest(f.db,p,c);
    ...
    const declined:RequestCommand={...c,action:"decline",scope:"coordinate",role:"none",match:undefined,pins:[],
      actor:{...c.actor,fellowId:matchFellow(1),sponsorId:"usr_1",credentialId:"cred-1",tokenHash:matchHash(matchFellow(1))},
      idempotencyKey:"decline",requestDigest:matchHash("decline"),eventId:`RRE-${"c".repeat(32)}`,
      receipt:{...c.receipt,status:"declined",version:2,updated_at:f.now+1}};
    await commitRequest(f.db,p,declined);
    const next=await f.match(); assert.equal(next?.reviewerId,matchFellow(2)); assert.ok(next);
    await commitRequest(f.db,p,command(f,next,"match-2"));
    assert.deepEqual(counts(f),[2,3,3,3]);
    assert.deepEqual(f.rows("SELECT action FROM review_request_events ORDER BY rowid").map(row=>row.action),["offer","decline","offer"]);
  } finally {f.sqlite.close();}
});

test("named invitations retain their own behavior without a matching guard",async()=>{
  const {f,c,p}=await prepared(); try {
    f.person(2); f.invite(2,"offer");
