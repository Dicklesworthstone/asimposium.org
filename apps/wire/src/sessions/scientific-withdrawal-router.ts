import {
  SCIENTIFIC_WITHDRAWALS_SCHEMA_ID, ScientificWithdrawalRequestSchema, ScientificWithdrawalReceiptSchema,
...
import { handleScientificWithdrawalHttp, WithdrawalHttpError, type ScientificWithdrawalOperations } from "../ledger/scientific-withdrawal-http.ts";
import { ScientificWithdrawalError, withdrawScientificInput } from "../ledger/scientific-withdrawals.ts";
...
export function scientificWithdrawalOperations(env: Env, options: SessionRouterOptions): ScientificWithdrawalOperations {
  return {
    ...
    async withdraw(actor, session, kind, target, input, key) {
      try {
        return ScientificWithdrawalReceiptSchema.parse(await withdrawScientificInput({
          db: env.DB,
          ...
          settleFailure: (id, held) => settleQuotaReservation(env.DB, id, held ? "settled_held" : "settled_failed"),
          screen: value => options.screenPromotion
            ? options.screenPromotion(value, env)
            : screenPromotionWithWorkersAI(env.AI as unknown as WorkersAiBinding | undefined, value),
          writeLedger: (event, projection) => writeLedgerEvent(env.DB, event, projection),
        }, actor, session, kind, target, input, key));
      } catch (error) {
        if (error instanceof ScientificWithdrawalError) {
          const code = error.code === "NOT_ALLOWED" || error.code === "NOT_FOUND" ? "DENIED"
            : error.code === "RATE_LIMITED" ? "THROTTLED" : error.code;
...
export function createScientificWithdrawalRouter(options: SessionRouterOptions): Hono<{ Bindings: Env }> {
  const app = new Hono<{ Bindings: Env }>();
  for (const path of ["/v1/sessions/:id/evidence/:target/retract", "/v1/sessions/:id/reviews/:target/retract"]) {
    app.all(path, async c => (await handleScientificWithdrawalHttp(c.req.raw, scientificWithdrawalOperations(c.env, options))) ?? c.notFound());
  }
  return app;
}
