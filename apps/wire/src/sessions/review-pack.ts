...
export function composeReviewSelectionPack(
  ...
  if (selection.degraded)
    section.omitted.push({
      ...
      detail:
        "Some canonical review admissions were unavailable or beyond the two-page scan; this is not a complete board-wide queue.",
    });
  ...
  for (const [index, move] of selection.moves.entries()) {
    ...
    if (
      move.move !== "review" ||
      move.refs[0] !== problemId ||
      typeof target !== "string" ||
      /^C-[0-9]+@[1-9][0-9]*$/.exec(target)?.[0] !== target ||
      !item ||
    ...
    const body = JSON.stringify({
      ...
      target,
      independence_note:
        "Sponsor independence is an eligibility filter, not a granted tier. Family, method and capable-of-failure checks are assessed at submission; model names do not establish them.",
    });
    if (body.length > 18000 || neutralize(body).length > 18000) {
      section.omitted.push({
        reason: "item_too_large",
        detail: `eligible-reviews:${target}; read the exact-version claim face`,
      });
      continue;
    }
    section.candidates.push({
      ...
      why_included:
        "canonical scientific need, ranked by consequence, missing check and age after reviewer eligibility; not permission, support or a reservation",
    });
  }
  if (section.targets.length === 0 && section.omitted.length === 0)
    section.candidates.push({
      ...
      why_included: "state only the examined reviewer-specific baseline",
      stable_prefix: 3,
    });
  return section;
}
...
export async function readReviewSelectionPack(
  ...
  try {
    const selection = await loadLedgerMoves(
      db,
      problemId,
      reviewer,
      { session_open: true, review: true, promote: false },
      dependencies,
      cursor,
    );
    ...
    section = {
      ...
      omitted: [
        {
          reason: "review_selection_unavailable",
          detail:
            "Canonical reviewer-specific selection is unavailable at this pack cursor. Other readable pack records remain usable; retry or use live public discovery.",
        },
      ],
    };
  }
    ...
    return {
      ...
      omitted: [
        ...section.omitted,
        {
          ...
          detail:
            "Private invitation context is unavailable; scientific recommendations are unaffected.",
        },
      ],
    };
  }
