...
export async function prepareSessionCloseWorkshopActions(
  ...
  if (
    ...
    new Set(discard).size !== discard.length ||
  ...
  const heads = await db
    .prepare(`SELECT workshop_id,current_version,type,title,body_md,cas_hash,
  ...
  const max = await db
    ...
    .bind(input.problemId, input.fellowId)
    .first<{ n: number }>();
  const base = max?.n ?? 0;
  ...
  const statements: D1PreparedStatement[] = [
    db
      .prepare(`WITH expected(id,version) AS (VALUES ${expected.map(() => "(?,?)").join(",")})
      SELECT CASE WHEN
      ...
      .bind(
        ...expected.flatMap((row) => [row.workshop_id, row.current_version]),
        expected.length,
        input.sessionId,
        input.problemId,
        input.fellowId,
  ...
  all.forEach((id, index) => {
    ...
    statements.push(
      db
        .prepare(`UPDATE workshop_objects SET state=?,current_version=?,workshop_seq=?,updated_at=?
      ...
      db
        .prepare(`INSERT INTO workshop_revisions
