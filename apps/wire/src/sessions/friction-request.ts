...
export class FrictionRequestError extends Error {
  constructor() {
    super("FRICTION_REQUEST_INVALID");
  }
}
...
export function prepareFrictionEvidenceRequest(
  ...
  if (
    request.method !== "POST" ||
    !/^\/v1\/sessions\/[^/]+\/friction$/.test(url.pathname) ||
    url.search !== "" ||
    url.hash !== "" ||
    ...
    bytes.byteLength > FRICTION_REQUEST_MAX_BYTES
  )
    throw new FrictionRequestError();
  let body: string;
  try {
  ...
  return new Request(url.toString(), { method: "POST", headers, body, signal: request.signal });
}
