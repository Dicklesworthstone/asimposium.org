...
export interface FormalPackDependencies {
  records(db: D1Database, problem: string, cursor: number, after: number): Promise<FormalRecordPage>;
  gaps(db: D1Database, problem: string, query: { through: number; after: number }): Promise<ProofGapPage>;
...
function appendRecord(
  section: FormalPackSection, kind: string, id: string, body: object, reference: object,
  detail: string, order: number, neutralize: FormalPackDependencies["neutralize"],
) {
  ...
  if (encoded.length > MAX_ITEM_CHARACTERS || neutralize(encoded).length > MAX_ITEM_CHARACTERS) {
    ...
    encoded = JSON.stringify({ ...reference, content: null, content_omitted: "whole-record-size-limit" });
  }
  section.candidates.push({ kind, id, scope: "ledger", untrusted: true, tokens: 1,
    body: encoded, stable_prefix: order,
    why_included: "version-pinned public formal work; declarations are not independent verification or platform execution" });
}
function formalItem(section: FormalPackSection, problem: string, cursor: number, item: FormalRecord,
  neutralize: FormalPackDependencies["neutralize"], order: number) {
  const source = sourceUrl(problem, item.publication.seq);
  const reference = { kind: item.kind, publication: item.publication, target: item.target,
    source, target_read: `/p/${problem}/claims/${item.target.claim_id}@${item.target.version}.md?through=${cursor}`,
    notice: "Reported work only; this item does not assign an evidence class, independence tier or scientific disposition." };
  appendRecord(section, item.kind, item.publication.object_id, { ...reference, content: item.content }, reference,
    `formal-record:${item.publication.object_id}; full event: ${source}; verify its event ID and payload hash`, order, neutralize);
}
function gapItem(section: FormalPackSection, problem: string, cursor: number, gap: ProofGapRecord,
  neutralize: FormalPackDependencies["neutralize"], order: number) {
  const source = `/p/${problem}/gaps.md?through=${cursor}&target=${gap.gap_id}`;
  const reference = { gap_id: gap.gap_id, status: gap.status, filing: gap.filing,
    last_event: gap.last_event, source, notice: "Recorded obligation/settlement only, not verified proof." };
  appendRecord(section, "proof-gap", gap.gap_id, { ...gap, source }, reference,
    `proof-gap:${gap.gap_id}; full record: ${source}`, order, neutralize);
}
...
export async function readFormalPack(
  db: D1Database, problem: string, cursor: number, dependencies: FormalPackDependencies,
): Promise<FormalPackSection> {
  if (!exact(/^(?!.*--)P-[A-Z0-9][A-Z0-9-]{1,30}$/, problem) ||
      !Number.isSafeInteger(cursor) || cursor < 0) throw new Error("FORMAL_PACK_QUERY_INVALID");
  ...
  await Promise.all([
    (async () => {
      ...
      for (let page = 0; page < MAX_PAGES; page++) {
        ...
        if (face.problem_id !== problem || face.cursor !== cursor || face.after !== after ||
            face.target !== null || face.gaps.length > 8) throw new Error("FORMAL_PACK_GAP_SCOPE");
        let previous = after;
        for (const gap of face.gaps) {
          if (gap.gap_id !== `G-${gap.filing.seq}` || !checkedSequence(gap.filing.seq, previous, cursor) ||
              seen.has(gap.gap_id)) throw new Error("FORMAL_PACK_GAP_ORDER");
          seen.add(gap.gap_id); previous = gap.filing.seq;
        }
        for (const gap of face.gaps) gapItem(gapSection, problem, cursor, gap, dependencies.neutralize, 10 + gapSection.candidates.length);
      ...
      gapSection.omitted.push({ reason: "candidate_limit",
      ...
      gapSection.omitted = [{ reason: "formal_gaps_unavailable", detail: "Canonical proof-gap history is unavailable; retry the formal pack." }];
    }),
    (async () => {
      let after = 0;
      const seen = new Set<string>();
      for (let page = 0; page < MAX_PAGES; page++) {
        ...
        if (face.problem_id !== problem || face.cursor !== cursor || face.after !== after || face.records.length > 8)
          throw new Error("FORMAL_PACK_RECORD_SCOPE");
        let previous = after;
        for (const item of face.records) {
          if (!checkedSequence(item.publication.seq, previous, cursor) || seen.has(item.publication.event_id) ||
              !exact(/^[ER]-[A-Za-z0-9][A-Za-z0-9._:-]{0,61}$/, item.publication.object_id) ||
              !exact(/^C-[0-9]+$/, item.target.claim_id) || !Number.isSafeInteger(item.target.version) || item.target.version < 1)
            throw new Error("FORMAL_PACK_RECORD_ORDER");
          seen.add(item.publication.event_id); previous = item.publication.seq;
        }
        for (const item of face.records) formalItem(recordSection, problem, cursor, item, dependencies.neutralize, 30 + recordSection.candidates.length);
        for (const reason of face.omitted.filter(reason => reason !== "page_limit"))
          recordSection.omitted.push({ reason, detail: "formal: artifact, friction and verification records" });
        if (face.next_after === null) return;
      ...
      recordSection.omitted.push({ reason: "candidate_limit",
      ...
      recordSection.omitted = [{ reason: "formal_records_unavailable", detail: "Canonical artifact/friction/verification history is unavailable; proof-gap context remains separate." }];
    }),
  ]);
  const section: FormalPackSection = {
    candidates: [{ kind: "standing-context", id: "SYS-formal-records-boundary", scope: "system", untrusted: false,
      tokens: 1, body: FORMAL_PACK_NOTICE, stable_prefix: 3,
      why_included: "distinguish recorded formal work from scientific verification and local execution" },
      ...gapSection.candidates, ...recordSection.candidates],
    omitted: [...gapSection.omitted, ...recordSection.omitted],
  };
  if (section.candidates.length === 1 && section.omitted.length === 0)
    section.candidates.push({kind:"standing-context",id:"SYS-formal-empty",scope:"system",untrusted:false,tokens:1,
      ...
      stable_prefix:4,why_included:"state only the observed formal-record baseline"});
  return section;
}
