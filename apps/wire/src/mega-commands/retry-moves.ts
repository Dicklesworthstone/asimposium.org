...
export interface RetryMoveSelection { move: NextMoveCandidate | null; degraded: boolean }
export interface RetryMoveSource {
  load(db: D1Database, problem: string, cursor: number, fellow: string): Promise<RetryMoveSelection>;
}
...
export function retryMoveFor(item: VerifiedDeadEndRetry, fellow: string, template: MoveTemplate): NextMoveCandidate | null {
  if (template.availability !== "available" || template.move !== "retry-dead-end") return null;
  const trigger = item.source.retry_when;
  const conditionRef = trigger.kind === "gap-closed" ? trigger.gap_id :
    trigger.kind === "claim-reaches" ? trigger.claim_id : item.problem_id;
  return {
    move: "retry-dead-end",
    ...
    contract: {
      ...template,
      description: "Investigate a previously blocked route in the private workshop. Publish only the actual result, which is not known at selection time.",
      ...
      preparation: {
        problem_id: item.problem_id, captured_cursor: item.cursor,
        dead_end_pin: { dead_end_id: item.dead_end_id, ...item.publication },
        firing_pin: item.firing, retry_when: trigger,
        read_first: { method: "GET", path: `/p/${item.problem_id}/dead-ends.md?include_superseded=true` },
        additional_reads: [
          { method: "GET", path: `/p/${item.problem_id}/events.json?since=${item.publication.seq - 1}` },
          { method: "GET", path: `/p/${item.problem_id}/events.json?since=${item.firing.seq - 1}` },
          { method: "GET", path: trigger.kind === "gap-closed"
            ? `/p/${item.problem_id}/gaps.md?through=${item.cursor}&target=${trigger.gap_id}`
            : trigger.kind === "claim-reaches" ? `/p/${item.problem_id}/claims/${trigger.claim_id}.md?through=${item.cursor}`
...
export async function loadRetryMove(db: D1Database, problem: string, cursor: number, fellow: string,
  dependencies: RetryMoveDependencies): Promise<RetryMoveSelection> {
  let after: RetryCursor | undefined, degraded = false;
  ...
  for (let n = 0; n < 2; n++) {
    ...
    for (const item of page.items) {
      if (item.problem_id !== problem || item.cursor !== cursor || seen.has(item.dead_end_id) ||
          item.firing.seq > cursor || item.firing.seq <= item.publication.seq ||
          (previous && (item.firing.seq < previous.seq ||
            (item.firing.seq === previous.seq && item.dead_end_id <= previous.id))))
        throw new Error("RETRY_MOVE_SCOPE_INVALID");
      seen.add(item.dead_end_id); previous = {seq:item.firing.seq,id:item.dead_end_id};
    }
    if (page.items[0]) {
    ...
    if (!Number.isSafeInteger(page.next.seq) || page.next.seq < 1 || page.next.seq > cursor ||
        (after && (page.next.seq < after.seq || (page.next.seq === after.seq && page.next.id <= after.id))))
...
export async function withRetryMove(db: D1Database, problem: string, cursor: number, fellow: string,
  permissions: Record<string, boolean>, selected: {moves: NextMoveCandidate[]; degraded: boolean}, source?: RetryMoveSource) {
  if (!source || !permissions.session_open || !permissions.promote || !permissions.workshop_push) return selected;
  try {
    const extra = await source.load(db, problem, cursor, fellow);
    const frontier = selected.moves.findIndex(move => move.move === "state-claim" || move.move === "third-alternative");
    const index = frontier < 0 ? selected.moves.length : frontier;
    return { moves: extra.move ? [...selected.moves.slice(0,index), extra.move, ...selected.moves.slice(index)] : selected.moves,
