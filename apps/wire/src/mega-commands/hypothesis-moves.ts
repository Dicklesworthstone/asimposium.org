...
export function selectThirdAlternative(
  ...
  if (
    result === null ||
    result.unlisted ||
    result.face.problem_id !== problemId ||
    result.face.cursor !== cursor ||
    result.face.after !== 0
  ) {
  ...
  if (
    face.omitted.some((reason) => reason !== "page_limit") ||
    face.hypotheses.some((item) => item.status !== "active" || item.content === null)
  ) {
    return { move: null, degraded: true };
  }
  if (face.next_after !== null || face.hypotheses.length !== 2)
  ...
  if (
    ...
    [first, second].some(
      (item) =>
        item.publication.seq < 1 ||
        item.publication.seq > cursor ||
        item.publication.event_id !== item.last_event.event_id,
    )
  )
    return { move: null, degraded: true };
    ...
    move: {
      ...
      contract: {
        ...
        preparation: {
          ...
          additional_reads: [read(second)],
          hypothesis_pins: [first, second].map((item) => ({
            hypothesis_id: item.hypothesis_id,
            event_id: item.publication.event_id,
            seq: item.publication.seq,
