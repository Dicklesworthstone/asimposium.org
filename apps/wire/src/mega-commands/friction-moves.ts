...
import { formalRecordPayload, type FormalRecord, type FormalRecordRead } from "../ledger/formal-records.ts";
...
export interface FrictionMoveSelection { move: NextMoveCandidate | null; degraded: boolean }
export interface FrictionMoveSource {
  load(db: D1Database, problem: string, cursor: number, items: readonly ReviewQueueItem[]): Promise<FrictionMoveSelection>;
}
...
interface Candidate { report: FormalRecord; claim: ReviewQueueItem; work: FrictionWork }
...
function usefulWork(report: FormalRecord, read: FrictionMoveDependencies["work"]): FrictionWork | null {
  const content = report.content;
  if (report.kind !== "formalization-friction" || !("bears_on_kind" in content) ||
      content.bears_on_kind !== "claim" || content.kind !== "formalization-friction" ||
      content.direction !== "informs" || content.mode !== "exploratory" ||
      content.formal_artifact !== undefined || content.falsification_check !== undefined ||
      content.bears_on_id !== report.target.claim_id || content.bears_on_version !== report.target.version) return null;
  return read(content.body_md);
}

function moveFor(candidate: Candidate, target: TargetRow, template: MoveTemplate): NextMoveCandidate | null {
  if (template.move !== "add-refuter-from-friction" || template.availability !== "available") return null;
  ...
  return {
    ...
    contract: {
      ...template,
      description: "Investigate the reported mathematical obstruction before publishing any result. A negative search result is also useful; do not invent a counterexample.",
      ...
      preparation: {
        problem_id: claim.problem_id, captured_cursor: claim.cursor,
        blocker: work.blocker,
        friction_pin: {evidence_id:report.publication.object_id,event_id:report.publication.event_id,
          seq:report.publication.seq,payload_sha256:report.publication.payload_sha256},
        target_pin: {claim_id:claim.claim_id,version:claim.version,event_id:target.event_id,
          seq:target.seq,payload_sha256:target.payload_sha256},
        read_first: {method:"GET",path:`/p/${claim.problem_id}/formal.md?through=${claim.cursor}&target=${report.publication.object_id}`},
        additional_reads: [{method:"GET",path:`/p/${claim.problem_id}/claims/${key(claim)}.md?through=${claim.cursor}`}],
        ...
        publication_target_hints: {bears_on_kind:"claim",bears_on_id:claim.claim_id,bears_on_version:claim.version},
...
export async function loadFrictionMove(db: D1Database, problem: string, cursor: number,
  items: readonly ReviewQueueItem[], dependencies: FrictionMoveDependencies): Promise<FrictionMoveSelection> {
  if (!exact(/^(?!.*--)P-[A-Z0-9][A-Z0-9-]{1,30}$/, problem) ||
      !Number.isSafeInteger(cursor) || cursor < 0 || items.length > 16)
    throw new Error("FRICTION_MOVE_SCOPE_INVALID");
  const claims = new Map<string, ReviewQueueItem>();
  for (const item of items) {
    if (item.problem_id !== problem || item.cursor !== cursor || !exact(/^C-[0-9]+$/, item.claim_id) ||
        !Number.isSafeInteger(item.version) || item.version < 1 || claims.has(key(item)) ||
  ...
  for (let pageNumber=0;pageNumber<2;pageNumber++) {
    ...
    if (page.problem_id !== problem || page.cursor !== cursor || page.after !== after ||
        page.target !== null || page.unlisted !== false || page.records.length > 8)
    ...
    for (const report of page.records) {
      if (!Number.isSafeInteger(report.publication.seq) || report.publication.seq <= previous ||
          report.publication.seq > cursor || seen.has(report.publication.event_id) ||
          !exact(ID,report.publication.event_id)) throw new Error("FRICTION_MOVE_ORDER_INVALID");
      previous=report.publication.seq;seen.add(report.publication.event_id);
      ...
      if (!work) {degraded=true;continue;}
      if (!["counterexample-scent","statement-too-strong"].includes(work.blocker)) continue;
      if (!exact(/^E-[A-Za-z0-9]+$/,report.publication.object_id) || report.publication.object_id.length > 80 ||
          !exact(/^[0-9a-f]{64}$/,report.publication.payload_sha256) ||
          typeof work.witness_seed !== "string" || work.witness_seed.trim().length===0) {
    ...
    if (!Number.isSafeInteger(page.next_after) || page.next_after <= after ||
        page.next_after < previous || page.next_after > cursor) throw new Error("FRICTION_MOVE_CURSOR_INVALID");
    after=page.next_after;
    if(pageNumber===1)degraded=true;
  }
  if(candidates.length===0)return {move:null,degraded};
  const rows=(await db.prepare(FRICTION_TARGETS_SQL).bind(JSON.stringify({problem,cursor,targets:candidates.map(({claim,report})=>({
    claim_id:claim.claim_id,version:claim.version,event_id:report.publication.event_id,
    evidence_id:report.publication.object_id,digest:report.publication.payload_sha256,seq:report.publication.seq,
  }))})).all<TargetRow>()).results;
  if(!Array.isArray(rows)||rows.length>candidates.length)throw new Error("FRICTION_MOVE_BINDING_INVALID");
  const usable=new Map<string,{candidate:Candidate;row:TargetRow}>(), returned=new Set<string>();
  for(const row of rows){
    const candidate=candidates.find(({report})=>report.publication.event_id===row.source_event_id);
    if(!candidate || returned.has(row.source_event_id))throw new Error("FRICTION_MOVE_BINDING_INVALID");
    ...
    if(!source||!target||row.claim_id!==claim.claim_id||row.version!==claim.version||
      !exact(ID,row.event_id)||!Number.isSafeInteger(row.seq)||row.seq<1||row.seq>=report.publication.seq||
      row.source_fellow!==report.publication.fellow_id||row.source_sponsor!==report.publication.sponsor_id||
      target.claim_id!==claim.claim_id||target.statement!==claim.statement||
      (target.falsifier??null)!==claim.falsifier||target.kind!==claim.kind||
      (claim.version===1 ? row.event_type!=="claim.created" : row.event_type!=="claim.revised"||target.base_version!==claim.version-1)||
      source.body_md!==report.content.body_md||source.bears_on_kind!=="claim"||
      source.bears_on_id!==claim.claim_id||source.bears_on_version!==claim.version||
      source.kind!=="formalization-friction"||source.direction!=="informs"||source.mode!=="exploratory"||
...
export async function withFrictionMove(db:D1Database,problem:string,cursor:number,
  items:readonly ReviewQueueItem[],permissions:Record<string,boolean>,
  selected:{moves:NextMoveCandidate[];degraded:boolean},source?:FrictionMoveSource) {
  if(!source||!permissions.session_open||!permissions.workshop_push||!permissions.promote||items.length===0)return selected;
  try {
    const extra=await source.load(db,problem,cursor,items);
    return {moves:extra.move ? [extra.move,...selected.moves.filter(move=>
      !(move.move==="add-refuter"&&move.refs[0]===problem&&move.refs[1]===extra.move!.refs[1]))] : selected.moves,
