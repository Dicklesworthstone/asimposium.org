...
export function loadDeadEndRetryMove(db: D1Database, problem: string, cursor: number, fellow: string) {
  return loadRetryMove(db, problem, cursor, fellow, {
    page: loadDeadEndRetryPage, template: () => getMoveTemplate("retry-dead-end"),
  });
}
