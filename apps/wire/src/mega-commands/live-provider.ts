...
const NO_PERMISSIONS = { read: true, session_open: false, workshop_push: false, promote: false, review: false };
...
export class LedgerMovesProvider implements MegaCommandsMoveProvider {
  ...
  private preflight(credential: FellowCredentialBinding, problemId: string, now: number): boolean {
    ...
    return this.dependencies.authorize({ effect: "session.open", credential,
  ...
  private async problem(
    ...
    if (role === "none") return { items: [] as ReviewQueueItem[], moves: [] as NextMoveCandidate[],
      degraded: false, continuation: null, role, effectivePermissions: { ...NO_PERMISSIONS } };
    const usage = { eventsRecorded, artifactBytesRecorded: 0 };
    const target = { kind: "existing-problem" as const, problemId, publication: "published" as const,
      unlisted: false, membershipRole: role };
    ...
    const effectivePermissions = {
      read: true,
      session_open: this.dependencies.authorize({ effect: "session.open", credential,
  ...
  async triageMove(request: TriageMovesRequest): Promise<TriageMovesResult> {
    ...
    if (!db || !credential || credential.fellowId !== request.fellowId) return {
      move: null, degraded: true, degradedReason: "MOVES_UNAVAILABLE", selectionBoundary: TRIAGE_BOUNDARY,
    };
    try {
      const now = this.dependencies.now();
      const problems = [...new Set(request.assignments.map(assignment => assignment.problem_id))]
        .filter(id => this.preflight(credential, id, now)).sort();
      if (problems.length === 0) return { move: null, degraded: false, selectionBoundary: TRIAGE_BOUNDARY };
      const eventsRecorded = await this.usage(db, credential);
      const selected = problems.slice(0, TRIAGE_MAX_PROBLEMS);
      const results: Array<Awaited<ReturnType<LedgerMovesProvider["problem"]>> | null> = selected.map(() => null);
      let index = 0;
      let failed = false;
      await Promise.all(Array.from({ length: Math.min(TRIAGE_CONCURRENCY, selected.length) }, async () => {
        while (index < selected.length) {
          const position = index++;
          const id = selected[position];
          if (id === undefined) break;
          try { results[position] = await this.problem(db, id, credential, eventsRecorded, now); }
          catch { failed = true; }
        }
      }));
      ...
      return { move: null, degraded: true, degradedReason: "MOVES_UNAVAILABLE", selectionBoundary: TRIAGE_BOUNDARY };
    }
  }
}
