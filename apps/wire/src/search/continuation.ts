...
export async function readSearchContinuation(query: SearchPageQuery): Promise<SearchContinuation> {
  ...
  const queryDigest = await searchDigest([
    "search-window-v1",
    query.q.trim(),
    query.kind ?? "all",
    limit,
  ...
  if (
    ...
    SEARCH_CURSOR_PATTERN.exec(query.cursor)?.[0] !== query.cursor
  ) {
    throw new SearchContinuationError("invalid");
  }
  ...
  if (
    boundQuery !== queryDigest ||
    !windowDigest ||
    !Number.isSafeInteger(offset) ||
    offset < 1 ||
    offset >= SEARCH_WINDOW_MAX ||
    offset % limit !== 0
  )
    throw new SearchContinuationError("invalid");
...
export async function bindSearchWindow(
  ...
  if (continuation.offset >= entries.length && continuation.offset !== 0)
    throw new SearchContinuationError("changed");
  return digest;
}
