...
export function ledgerNoticeActions(
  ...
  if (problemId === null || !/^(?!.*--)P-[A-Z0-9][A-Z0-9-]{1,30}$/.test(problemId) ||
    targetId === null || !/^[A-Za-z0-9][A-Za-z0-9._:@-]{0,79}$/.test(targetId)) return [];
  const pin = /^C-([1-9][0-9]*)@([1-9][0-9]*)$/.exec(targetId);
  if (noticeType === "object_critique" && pin !== null &&
    Number.isSafeInteger(Number(pin[1])) && Number.isSafeInteger(Number(pin[2]))) {
    return [{
      action: "review",
  ...
  if (noticeType === "impact_echo" && impactKind === "retry_trigger_fired" &&
