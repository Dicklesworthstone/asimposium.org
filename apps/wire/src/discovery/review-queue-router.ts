  ...
  const response =
    kind === "query"
      ? validatedProblem({
          status: 400,
          code: "CURSOR_INVALID",
          title: "Choose a valid review queue page",
          detail:
            "Use at most one problem identifier and one unchanged after cursor. No other query parameters are accepted.",
          fixHint:
            "Read /reviews.json, or preserve the selected problem and URL-encode next_after from the preceding page.",
          rule: "A5",
          extensions: {
      ...
      : validatedProblem({
          status: 500,
          code: "INTERNAL_ERROR",
          title: "Review discovery is temporarily unavailable",
          detail:
            "A complete review queue page could not be established. This is not an empty queue.",
      ...
      try {
        ...
        const body =
          format === "json"
            ? JSON.stringify(data)
            : format === "md"
              ? renderReviewQueueMarkdown(data, parsed.data)
              : renderReviewQueueHtml(data, parsed.data);
