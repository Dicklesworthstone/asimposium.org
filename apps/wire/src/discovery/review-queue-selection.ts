...
export function reviewNeed(
  ...
  if (fold.stale || fold.currentVersion === null ||
      !["open", "disputed", "corroborated", "reduced-to"].includes(fold.disposition)) return undefined;
  ...
  for (const review of fold.context.verified_reviews) {
    ...
    if (tier > best) { best = tier; bestRecordedTier = review.tier; }
  }
  const need: ReviewQueueNeed = fold.disposition === "disputed" ? "resolve-dispute"
    : best < 1 ? "independent-review"
    : best < 2 ? "cross-family-review"
    : fold.context.recorded_refutation_attempts === 0 ? "falsification-attempt"
...
function comparePriority(a: ReviewQueueOrderable, b: ReviewQueueOrderable): number {
  return b.direct_dependents - a.direct_dependents || NEED_PRIORITY[a.need] - NEED_PRIORITY[b.need] ||
...
export function rankReviewQueue<T extends ReviewQueueOrderable>(input: readonly T[]): T[] {
  const remaining = [...input].sort((a, b) => comparePriority(a, b) ||
    compareText(a.problem_id, b.problem_id) || compareText(a.claim_id, b.claim_id));
  const output: T[] = [];
  ...
  while (remaining.length > 0) {
    ...
    for (let i = 0; i < remaining.length; i += 1) {
      ...
      if (!seenSponsors.has(candidate.author_sponsor_id)) { selected = i; break; }
    }
