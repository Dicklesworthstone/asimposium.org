...
export async function reviewQueueResponse(request: Request, body: string, format: ReviewQueueFormat): Promise<Response> {
  const bytes = new TextEncoder().encode(body);
  if (bytes.byteLength > REVIEW_QUEUE_MAX_RESPONSE_BYTES) throw new Error("REVIEW_QUEUE_RESPONSE_TOO_LARGE");
  ...
  return matches ? new Response(null, { status: 304, headers })
