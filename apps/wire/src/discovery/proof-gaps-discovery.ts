...
export const PROOF_GAPS_PUBLIC_READS: Readonly<Record<string, string>> = Object.freeze({
  ...
  "GET /p/:id/gaps.json":
    "Committed proof obligations and recorded settlements, with exact targets, original attribution and snapshot pagination.",
  "GET /p/:id/gaps.md":
    "Proof obligations and settlement history as fenced canonical Markdown. Closure is not independent verification.",
  "GET /p/:id/gaps.html":
    "HTML reading face of the same gap snapshot, including unavailable content and recorded settlements.",
});
...
export function proofGapResponses(
  path: string,
  origin: string,
  ...
  return {
    "200": {
      description:
        "Recorded gap history at one cursor. Closed-by is a recorded reference, not a verified deduction. Unlisted reads are private; unavailable content is explicit.",
      content: {
      ...
      headers: {
        ...
        Link: {
          description:
            "Canonical and next URLs retain the exact snapshot and selected representation.",
          schema: { type: "string" },
        },
      },
    },
    ...
    default: {
      description: "Typed refusal. Do not advance a saved cursor on failure.",
      content: {
