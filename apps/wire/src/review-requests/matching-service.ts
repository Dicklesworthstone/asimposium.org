...
import { type ReviewMatchCandidate, selectReviewMatch } from "./matching.ts";
...
export function matchRecipientMayReview(
  row: ReviewMatchCandidate,
  problem: string,
  now: number,
): boolean {
  ...
  if (
    credential.fellowId !== row.fellow_id ||
    credential.sponsorId !== row.sponsor_id ||
    !Array.isArray(credential.grantedScopes) ||
    !Array.isArray(grant.scopes) ||
    !credential.grantedResources ||
    !grant.resources
  )
    return false;
  const target = {
  ...
  return (
    authorizeFellowWrite({ effect: "review", credential, target, usage, now }).decision ===
      "allow" &&
    authorizeFellowWrite({
      effect: "review",
      credential: { ...credential, grantedScopes: grant.scopes, grantedResources: grant.resources },
      target,
      usage,
      now,
    }).decision === "allow"
...
export function matchReviewRecipient(
  db: D1Database,
  problem: string,
