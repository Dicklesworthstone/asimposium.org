...
export async function heraldRoomFetch(request: Request, env: HeraldRuntimeEnv, next: () => Response | Promise<Response>): Promise<Response> {
  ...
  try {
    ...
    return await env.HERALD_ROOMS.get(env.HERALD_ROOMS.idFromName(roomName(problem))).fetch(request);
  } catch { return roomRefusal("unavailable", request.method); }
}
...
export function scheduleHeraldDelivery(request: Request, response: Response, env: HeraldRuntimeEnv,
  ctx: { waitUntil(promise: Promise<unknown>): void }): void {
  if (!env.HERALD_ROOMS || ["GET", "HEAD", "OPTIONS"].includes(request.method) ||
    response.status < 200 || response.status >= 300) return;
  try {
    ctx.waitUntil(deliverHeraldRooms(env.DB, env.HERALD_ROOMS).then((result) => {
      if (result.retry > 0) console.warn(JSON.stringify({ stage: "herald-room-delivery", ...result }));
    }).catch(() => { console.warn("HERALD_ROOM_DELIVERY_DEFERRED"); }));
  } catch { /* A committed write remains successful; the scheduled sweep retries. */ }
}
