import { roomCursor, roomName, roomProblem } from "../../../../packages/contracts/src/herald-room-model.ts";
...
export const HERALD_HEAD_SQL = "SELECT public_seq AS seq FROM problems WHERE id = ? AND status <> 'private-draft'";
export async function publicRoomHead(db: HeraldDatabase, id: string): Promise<{ seq: number } | null> {
  ...
  if (results.length !== 1 || !roomCursor(results[0]?.seq)) throw new Error("HERALD_HEAD_UNAVAILABLE");
...
interface Pending { problem_id: string; generation: number; attempts: number }
...
async function nudge(namespace: HeraldNamespace, row: Pending): Promise<void> {
  ...
  try {
    const deadline = new Promise<never>((_resolve, reject) => {
      timer = setTimeout(() => { controller.abort(); reject(new Error("HERALD_DELIVERY_TIMEOUT")); }, HERALD_DELIVERY_LIMITS.timeoutMs);
    });
    const attempt = Promise.resolve().then(async () => {
      const stub = namespace.get(namespace.idFromName(roomName(row.problem_id)));
      const response = await stub.fetch(new Request(`https://herald.internal/_herald/${encodeURIComponent(row.problem_id)}/nudge`, {
        method: "POST", headers: { "content-type": "application/json" },
        body: JSON.stringify({ generation: row.generation }), signal: controller.signal,
      }));
...
export async function deliverHeraldRooms(db: HeraldDatabase, namespace: HeraldNamespace | undefined, clock: () => number = Date.now) {
  ...
  for (const row of results) {
    if (!roomProblem(row.problem_id) || !roomCursor(row.generation) || row.generation < 1 ||
      !roomCursor(row.attempts) || row.attempts > 16) throw new Error("HERALD_QUEUE_INVALID");
    report.scanned++;
    try {
      await nudge(namespace, row);
      const ack = await db.prepare(HERALD_ACK_SQL).bind(row.generation, row.problem_id, row.generation).all();
