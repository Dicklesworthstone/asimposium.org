...
import { putPublicArtifact, readPublicationBytes, type PublicationBytes } from "./artifact-publication-bytes.ts";
import {
  publicationHash, readPublicationEvidence, readPublicationJob,
  type PublicationJob,
...
export interface PublicationDrainerOptions {
  ...
  readonly screen: (job: PublicationJob, content: PublicationBytes) => Promise<PublicationScreenResult>;
}
const categories = new Set(["benign-context","spam-commercial","injection","dual-use-boundary",
  "operational-harm","harassment","sexual-content","provider-unavailable"]);
const sha = /^sha256:[a-f0-9]{64}$/;
const label = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,159}$/;

export const publicationScreenContext = (job: PublicationJob) => publicationHash(JSON.stringify({
  scope: "artifact-publication-policy-v1", publication_id: job.publication_id,
  artifact_origin: job.artifact_origin, license: job.license,
  problem_id: job.problem_id, fellow_id: job.fellow_id, evidence_event_id: job.evidence_event_id,
  evidence_sha256: job.evidence_sha256, artifact_sha256: job.sha256, screening_sha256: job.screening_sha256,
}));

async function screenReceipt(job: PublicationJob, result: PublicationScreenResult) {
  if (!result || !["pass","allow-with-warning","quarantine","reject"].includes(result.decision) ||
    ...
    typeof result.model_version !== "string" || typeof result.policy_version !== "string" ||
    typeof result.configuration_digest !== "string" ||
    !label.test(result.model_version) || !label.test(result.policy_version) ||
    !sha.test(result.configuration_digest) || !categories.has(result.coarse_category) ||
  ...
  return JSON.stringify({ version: "artifact-publication-policy-v1", publication_id: job.publication_id,
    artifact_sha256: job.sha256, screening_sha256: job.screening_sha256, evidence_sha256: job.evidence_sha256,
    context_digest: result.evaluated_context_digest, decision: result.decision,
    provider_status: result.provider_status, model_version: result.model_version,
    policy_version: result.policy_version, configuration_digest: result.configuration_digest,
...
async function releaseReceiptValid(job: PublicationJob): Promise<boolean> {
  ...
  try {
    ...
    return r.version === "artifact-publication-policy-v1" && r.publication_id === job.publication_id &&
      r.artifact_sha256 === job.sha256 && r.screening_sha256 === job.screening_sha256 &&
      r.evidence_sha256 === job.evidence_sha256 && r.decision === "pass" && r.provider_status === "ok" &&
      r.context_digest === `sha256:${await publicationScreenContext(job)}` &&
      typeof r.model_version === "string" && label.test(r.model_version) &&
      typeof r.policy_version === "string" && label.test(r.policy_version) &&
      typeof r.configuration_digest === "string" && sha.test(r.configuration_digest);
  } catch { return false; }
}
...
export async function livePublicationPin(db: D1Database, job: PublicationJob) {
  ...
  if (evidence.evidence_event_id !== job.evidence_event_id || evidence.evidence_sha256 !== job.evidence_sha256 ||
    evidence.fellow_id !== job.fellow_id || evidence.sponsor_id !== job.sponsor_id) return null;
  const event = await db.prepare(`SELECT c.payload_json FROM events e JOIN event_content c ON c.event_id = e.id
    ...
    .bind(job.event_id,job.problem_id,job.publication_id,job.event_sha256,job.fellow_id,job.sponsor_id)
    .first<{payload_json:string}>();
  if (!event || await publicationHash(event.payload_json) !== job.event_sha256) return null;
  let p: Record<string,unknown>;
  try { p = JSON.parse(event.payload_json); } catch { return null; }
  if (!p || p.publication_id !== job.publication_id || p.sha256 !== job.sha256 ||
    p.size_bytes !== job.size_bytes || p.content_type !== job.content_type || p.license !== job.license ||
    p.evidence_id !== job.evidence_id || p.evidence_event_id !== job.evidence_event_id ||
    p.evidence_digest !== `sha256:${job.evidence_sha256}` || p.verification !== "bytes-only") return null;
...
async function claim(db: D1Database, id: string, now: number): Promise<PublicationJob | null> {
  ...
  await db.prepare(`UPDATE artifact_publication_jobs SET state = 'held', failure_code = 'dependency-unavailable',
    ...
    .bind(now,id,now,now).run();
  const token = crypto.randomUUID().replaceAll("-", "");
  const result = await db.prepare(`UPDATE artifact_publication_jobs SET lease_token = ?, lease_until = ?,
    ...
    .bind(token,now + PUBLICATION_LEASE_MS,now,id,now,now).run();
...
async function retryOrHold(db: D1Database, job: PublicationJob, now: number,
  code: "dependency-unavailable" | "content-unavailable" | "binding-unavailable" | "screening-held", hold: boolean,
  receipt: string | null = null,
): Promise<"held"|"retry"|"lost"> {
  const delay = Math.min(3_600_000,60_000 * 2 ** Math.min(job.attempts - 1,6));
  const result = await db.prepare(`UPDATE artifact_publication_jobs SET
    ...
    .bind(hold ? 1 : 0,code,receipt,now + delay,now,job.publication_id,job.lease_token,now).run();
  if (!result.success || result.meta.changes !== 1) return "lost";
...
export async function deliverArtifactPublication(
  options: PublicationDrainerOptions, id: string,
): Promise<"published"|"held"|"retry"|"lost"|"idle"> {
  if (options.privateBucket === options.publicBucket ||
    !["https://artifacts.asimposium.org","https://artifacts-staging.asimposium.org"].includes(options.artifactOrigin)) {
  ...
  try {
    ...
    if (content.screeningSha256 !== job.screening_sha256 || content.artifact.contentType !== job.content_type)
    ...
    if (job.state === "queued") {
      ...
      const releaseAt = now();
      const released = await db.prepare(`UPDATE artifact_publication_jobs SET state = 'release-authorized',
        ...
        .bind(releaseAt,releaseAt,receipt,receipt,releaseAt + PUBLICATION_LEASE_MS,
          job.publication_id,job.lease_token,releaseAt,job.evidence_event_id,job.evidence_sha256,pin.evidence,
          job.event_id,job.event_sha256,pin.event).run();
      if (!released.success || released.meta.changes !== 1)
    ...
    const finished = await db.prepare(`UPDATE artifact_publication_jobs SET state = 'published',
      ...
      .bind(finishedAt,finishedAt,job.publication_id,job.lease_token,finishedAt).run();
    return finished.success && finished.meta.changes === 1 ? "published" : "lost";
  } catch (error) {
    ...
    return retryOrHold(db,job,now(),error instanceof ArtifactInspectionError ? "content-unavailable" : "dependency-unavailable",
...
export async function drainArtifactPublications(options: PublicationDrainerOptions) {
  ...
  const rows = await options.db.prepare(`SELECT j.publication_id FROM artifact_publication_jobs j
    ...
    .bind(options.artifactOrigin,now,now).all<{publication_id:string}>();
  ...
  for (const row of rows.results) counts[await deliverArtifactPublication(options,row.publication_id)]++;
  return counts;
}
