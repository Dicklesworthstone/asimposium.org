...
export type PublicationPolicyCall = (input: BoundPublicationScreen, signal: AbortSignal) => Promise<unknown>;
...
const categories = new Set(["benign-context", "spam-commercial", "injection", "dual-use-boundary",
  "operational-harm", "harassment", "sexual-content"]);
...
export async function screenBoundArtifact(
  input: BoundPublicationScreen, invoke: PublicationPolicyCall,
  ...
  if (typeof body !== "string" || bytes.length < 1 || bytes.length > PUBLICATION_SCREEN_BYTES ||
    !digestPattern.test(body_digest) || !digestPattern.test(context_digest) ||
    !digestPattern.test(configuration_digest) || !labelPattern.test(model_version) || !labelPattern.test(policy_version) ||
    !Number.isSafeInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > PUBLICATION_SCREEN_TIMEOUT_MS)
  ...
  const bound = Object.freeze({ body, body_digest, context_digest,
    identity: Object.freeze({ model_version, policy_version, configuration_digest }) });
  const identity = { evaluated_body_digest: body_digest, evaluated_context_digest: context_digest,
    model_version, policy_version, configuration_digest };
  const controller = new AbortController();
  let timer: ReturnType<typeof setTimeout> | undefined;
  const deadline = new Promise<never>((_, reject) => {
    timer = setTimeout(() => {
      controller.abort(); reject(new Error("ARTIFACT_SCREEN_TIMEOUT"));
    }, timeoutMs);
  });
  try {
    const observed = await Promise.race([Promise.resolve().then(() => invoke(bound, controller.signal)), deadline]);
    if (controller.signal.aborted) throw new Error("ARTIFACT_SCREEN_TIMEOUT");
    if (!observed || typeof observed !== "object" || Array.isArray(observed) ||
      !("decision" in observed) || typeof observed.decision !== "string" || !decisions.has(observed.decision) ||
      !("coarse_category" in observed) || typeof observed.coarse_category !== "string" || !categories.has(observed.coarse_category))
      throw new Error("ARTIFACT_SCREEN_RESULT_INVALID");
    return { ...identity, decision: observed.decision as PublicationScreenResult["decision"],
      coarse_category: observed.coarse_category, provider_status: "ok" };
  } catch {
    return { ...identity, decision: "quarantine", coarse_category: "provider-unavailable",
