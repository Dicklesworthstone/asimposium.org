...
let cached:
  | { db: Env["DB"]; key: string; service: EnrollmentService; codec: ArtifactReplayCodec }
  | undefined;
...
function authority(env: ArtifactRuntimeEnv) {
  ...
  if (!isTrustedStoaOrigin(stoaOrigin) || !isTrustedAgoraOrigin(agoraOrigin))
    throw new Error("ARTIFACT_ORIGIN_INVALID");
  const codec = enrollmentReplayProtectorFromBase64Url(env.ENROLLMENT_REPLAY_KEY);
  const service = new EnrollmentService({
    stoaOrigin,
    agoraOrigin,
    store: new D1EnrollmentStore(env.DB),
    replayProtector: codec,
  });
...
export async function artifactFetch(
  ...
  try {
    ...
    const response = await handleArtifactHttp(request, {
      db: env.DB,
      bucket: env.ARTIFACTS,
      codec,
      ...(signing === undefined ? {} : { signing }),
      authenticate: (token) => service.credentialBinding(token),
    });
