...
export class ArtifactPublicationStorageError extends Error {
  constructor() { super("ARTIFACT_PUBLICATION_STORAGE_UNAVAILABLE"); }
}
export interface PublicationBytes extends PublicationInspection {
  readonly bytes: Uint8Array;
}
const unavailable = (): never => { throw new ArtifactPublicationStorageError(); };
const validDigest = (value: string): boolean => typeof value === "string" && /^[a-f0-9]{64}$/.test(value);
...
export function artifactPublicationOrigin(stoaOrigin: string | undefined): string | undefined {
  ...
  if (stoaOrigin === "https://a-staging.asimposium.org") return "https://artifacts-staging.asimposium.org";
...
async function readExact(
  ...
  try {
    ...
    try { reader?.releaseLock(); } catch { /* do not replace a typed storage result */ }
  }
}
...
export async function readPublicationBytes(
  bucket: R2Bucket,
  input: { readonly sha256: string; readonly size_bytes: number; readonly encoding: ArtifactEncoding },
  timeoutMs = PUBLIC_ARTIFACT_STORAGE_TIMEOUT_MS,
): Promise<PublicationBytes> {
  if (!validDigest(input.sha256)) return unavailable();
  const { bytes } = await readExact(bucket, `cas/sha256/${input.sha256}`, input.size_bytes, timeoutMs);
...
export async function putPublicArtifact(
  bucket: R2Bucket, content: PublicationBytes,
  timeoutMs = PUBLIC_ARTIFACT_STORAGE_TIMEOUT_MS,
): Promise<void> {
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > PUBLIC_ARTIFACT_STORAGE_TIMEOUT_MS) return unavailable();
  const expected = content.artifact;
  if (!Number.isSafeInteger(expected.size) || expected.size < 1 || expected.size > 20 * 1024 * 1024) return unavailable();
  ...
  if (bytes.length !== expected.size ||
    ...
    await artifactSha256(bytes) !== expected.sha256) return unavailable();
  let timer: ReturnType<typeof setTimeout> | undefined;
  try {
    await Promise.race([
      bucket.put(key, bytes, {
        ...
        httpMetadata: { contentType, contentDisposition: "attachment", cacheControl: PUBLIC_ARTIFACT_CACHE_CONTROL },
      }),
  ...
  if (await artifactSha256(observed.bytes) !== expected.sha256 ||
    observed.object.httpMetadata?.contentType !== contentType ||
    observed.object.httpMetadata?.contentDisposition !== "attachment" ||
    observed.object.httpMetadata?.cacheControl !== PUBLIC_ARTIFACT_CACHE_CONTROL) return unavailable();
}
