...
export const PUBLICATION_HTTP_SCHEMA =
...
const EXAMPLE = {
  method: "POST",
  path: `/v1/artifacts/AU-${"0".repeat(32)}/publish`,
...
export interface PublicationHttpOperations {
  ...
  readonly publish: (
    ...
    key: string,
  ) => Promise<{ readonly status_path: string }>;
...
function cancel(request: Request): void {
  try {
    ...
    /* A refusal must never wait for an upstream body producer. */
  }
}
...
export async function readPublicationRequestBody(
  request: Request,
  timeoutMs = PUBLICATION_BODY_TIMEOUT_MS,
): Promise<unknown> {
  ...
  if (
    ...
    (length !== null &&
      (!/^(0|[1-9][0-9]*)$/.test(length) || Number(length) > PUBLICATION_BODY_BYTES))
  ) {
    cancel(request);
    throw new Error("PUBLICATION_BODY_INVALID");
  }
  ...
  const deadline = new Promise<never>((_, reject) => {
    onAbort = () => {
      interrupted = true;
      reject(new Error("PUBLICATION_BODY_INTERRUPTED"));
      stop();
    };
  ...
  try {
    ...
    if (length !== null && size !== Number(length))
      throw new Error("PUBLICATION_BODY_LENGTH_INVALID");
    ...
    try {
      ...
      /* Do not replace a bounded refusal. */
    }
  }
}

function teaching(
  ops: PublicationHttpOperations,
  status: number,
  detail: string,
  ...
  return ops.problem({
    status,
    code: "SCHEMA_INVALID",
    title: "Artifact publication request needs correction",
    detail,
    fixHint:
      "Follow the publication schema. Publication is a separate, explicit CC-BY action; upload verification alone does not publish.",
    rule: "A5",
    extensions: { schema: PUBLICATION_HTTP_SCHEMA, example: EXAMPLE },
...
function failure(ops: PublicationHttpOperations, error: unknown): Response {
  ...
  const retry =
    ...
    error.retryAfterSeconds! > 0
      ? String(Math.min(error.retryAfterSeconds!, 86400))
      : "30";
  if (code === "CONFLICT" || code === "BUSY")
    return teaching(
      ops,
      409,
      code === "BUSY"
        ? "An attempt is still in progress. Retry the same request and Idempotency-Key."
      ...
      status === 401
        ...
        : status === 404
          ...
          : status === 403 || status === 429
            ? "WRITE_REFUSED"
            : "INTERNAL_ERROR",
    title: "Artifact publication unavailable",
    detail: "This publication operation could not be completed safely.",
    fixHint:
      status === 503 || status === 429
        ? "Retry the same operation later. Consult the operator if this persists."
...
export async function handleArtifactPublicationHttp(
  ...
  const url = new URL(request.url),
    route = artifactPublicationRoute(url.pathname);
  ...
  try {
    ...
    if (
      !(request.method === "POST" && route.kind === "publish") &&
      !(request.method === "GET" && route.kind !== "publish") &&
      !(request.method === "HEAD" && isPublic)
    )
      ...
      response = teaching(
        ops,
        400,
        "This publication operation accepts no query parameters. Credentials belong only in Authorization.",
      );
    else if (route.kind === "manifest")
      response = await publicJson(request, await ops.manifest(route.problem, route.id));
    else if (route.kind === "evidence") {
      let after: number, through: number | undefined;
      try {
        if ([...url.searchParams.keys()].some((key) => key !== "after" && key !== "through"))
          throw new Error("UNKNOWN_QUERY");
        after = cursor(url.searchParams, "after") ?? 0;
        through = cursor(url.searchParams, "through");
        if (through !== undefined && through < after) throw new Error("REVERSED_RANGE");
      } catch {
        const refused = teaching(
          ops,
          400,
          "Use unique, nonnegative safe-integer after and through cursors, with through at least after.",
        );
        return request.method === "HEAD"
          ? new Response(null, { status: refused.status, headers: refused.headers })
          : refused;
      }
      response = await publicJson(
        request,
        await ops.evidence(route.problem, route.evidence, after, through),
      );
    } else {
      ...
      if (route.kind === "status")
        response = new Response(JSON.stringify(await ops.status(actor, route.id)), {
        ...
        if (
          ...
          return teaching(
            ops,
            415,
            "Send the publication consent as application/json; do not send file bytes to this endpoint.",
          );
        ...
        if (!key || !/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/.test(key))
          return teaching(
            ops,
            400,
            "Supply a stable Idempotency-Key of 1–128 safe identifier characters.",
          );
        let input: PublicationRequest | undefined;
        try {
          input = ops.decodeRequest(await readPublicationRequestBody(request));
        } catch {
          return teaching(
            ops,
            400,
            "Send complete UTF-8 JSON within the 4 KiB limit and request deadline.",
          );
        }
        if (!input)
          return teaching(
            ops,
            422,
            "Specify an owned session, exact evidence digest, publish: true and license: CC-BY-4.0. No other fields are accepted.",
          );
        const receipt = await ops.publish(actor, route.id, input, key);
        response = new Response(JSON.stringify(receipt), {
          status: 202,
          headers: {
            ...PRIVATE,
            "content-type": "application/json; charset=utf-8",
            location: receipt.status_path,
            "retry-after": "30",
          },
        });
      }
