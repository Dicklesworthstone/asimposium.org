...
} from "./artifact-publication-outbox.ts";
import { readEvidenceArtifacts, readPublicArtifactManifest } from "./artifact-publication-read.ts";
import { PUBLICATION_SCREEN_BYTES, screenBoundArtifact } from "./artifact-publication-screen.ts";
import {
...
import { type ArtifactRuntimeEnv, artifactAuthority } from "./artifact-runtime.ts";
...
async function mapped<T>(run: () => Promise<T>): Promise<T> {
  try {
    ...
    if (error instanceof ArtifactUploadError) {
      if (error.code === "NOT_FOUND" || error.code === "NOT_ALLOWED")
...
export function artifactPublicationOperations(env: ArtifactRuntimeEnv): PublicationHttpOperations {
  return {
    ...
    decodeRequest(value) {
      const parsed = ArtifactPublicationRequestSchema.safeParse(value);
      return parsed.success ? parsed.data : undefined;
    },
    publish: (actor, upload, input, key) =>
      mapped(async () => {
        ...
        const reserve: PublicationCommandOptions["reserve"] = async (params) => {
          ...
          if (!result.allowed) {
            if (result.reason === "RATE_LIMITED")
              throw new PublicationHttpError("THROTTLED", result.retryAfterSeconds);
            if (result.reason === "IN_FLIGHT_CONFLICT")
              throw new PublicationHttpError("BUSY", result.retryAfterSeconds);
    ...
    status: (actor, id) =>
      mapped(async () => {
        const origin = originFor(env),
          job = await readPublicationJob(env.DB, id);
    ...
    manifest: (problem, id) =>
      mapped(async () =>
        PublicArtifactManifestSchema.parse(
          (await readPublicArtifactManifest(env.DB, originFor(env), problem, id)).face,
        ),
      ),
    evidence: (problem, evidence, after, through) =>
      mapped(async () =>
        EvidenceArtifactsSchema.parse(
...
export async function artifactPublicationFetch(
  request: Request,
  env: ArtifactRuntimeEnv,
  next: () => Response | Promise<Response>,
): Promise<Response> {
  if (artifactPublicationRoute(new URL(request.url).pathname) === undefined) return next();
  try {
    const response = await handleArtifactPublicationHttp(
      request,
      artifactPublicationOperations(env),
    );
    if (response) return response;
  } catch {
    /* Dependency failures never expose a private exception or binding. */
  }
  ...
  if (artifactOrigin === undefined) {
    ...
    if (isTrustedStoaOrigin(env.STOA_ORIGIN) && new URL(env.STOA_ORIGIN!).protocol === "http:")
      return {
        ...
        lost: 0,
        idle: 0,
        skipped: "local-no-public-origin" as const,
      };
    throw new Error("ARTIFACT_PUBLICATION_ORIGIN_UNAVAILABLE");
  }
  return drainArtifactPublications({
    db: env.DB,
    privateBucket: env.ARTIFACTS,
    publicBucket: env.PUBLIC_ARTIFACTS,
    artifactOrigin,
    screen: async (job, content) => {
      ...
      const identity = {
        ...
        configuration_digest: `sha256:${await publicationHash(
          JSON.stringify({
            scope: "artifact-publication-direct-v1",
            base_configuration_digest: baseConfiguration,
            max_body_bytes: PUBLICATION_SCREEN_BYTES,
          }),
        )}`,
      };
      return screenBoundArtifact(
        {
          body: content.screeningBody,
          body_digest: `sha256:${job.screening_sha256}`,
          context_digest: `sha256:${await publicationScreenContext(job)}`,
          identity,
        },
        async (bound, signal) => {
          if (env.AI === undefined) throw new Error("ARTIFACT_SCREENING_UNAVAILABLE");
          const provider = new WorkersAIScreeningProvider(env.AI as unknown as WorkersAiBinding, {
            maxBodyBytes: PUBLICATION_SCREEN_BYTES,
            resolveBody: (request) => {
              if (
                request.body_digest !== bound.body_digest ||
                request.context_digest !== bound.context_digest
              )
          ...
          return provider.screen(
            {
              example_id: "artifact-publication",
              body_digest: bound.body_digest,
              context_digest: bound.context_digest,
              identity: {
                ...bound.identity,
                corpus_revision: "artifact-publication-direct-v1",
                corpus_digest: bound.body_digest,
              },
            },
            signal,
          );
