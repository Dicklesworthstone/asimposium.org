...
const dependencies: RetryReadDependencies = {
  ...
  async condition(db, row, trigger, payload, through) {
    if (trigger.kind === "statement-revised") {
      ...
      return event.action === "revise-statement" &&
        event.problem.id === row.problem_id &&
        event.problem.current_statement_version === row.object_version &&
        event.acting_principal.id === row.sponsor_id
        ? "holds"
        : "unavailable";
    }
    if (trigger.kind === "gap-closed") {
      ...
      if (
        result.unlisted ||
        result.face.problem_id !== row.problem_id ||
        result.face.cursor !== through ||
        result.face.target !== trigger.gap_id ||
        result.face.gaps.length !== 1 ||
        result.face.omitted.length > 0 ||
        !gap ||
        !gap.content ||
      ...
      return gap.status === "closed-by" &&
        gap.closed_by !== null &&
        gap.last_event.event_id === row.event_id &&
        gap.last_event.seq === row.seq &&
        gap.last_event.payload_sha256 === row.payload_sha256
        ? "holds"
        : "not-held";
    }
...
export function loadDeadEndRetryPage(
  db: D1Database,
  problem: string,
  through: number,
  after?: RetryCursor,
