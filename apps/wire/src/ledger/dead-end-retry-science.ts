...
export async function verifyRetryClaimTransition(
  db: D1Database, row: RetryAdmission, claim: string, reaches: string, through: number,
  science: ReviewQueueScience,
): Promise<"holds" | "not-held" | "unavailable"> {
  const budget = await db.prepare(RETRY_SCIENCE_BUDGET_SQL)
    .bind(JSON.stringify({ problem: row.problem_id, claim, through }))
    .first<{ events: number; bytes: number }>();
  if (!budget || !Number.isSafeInteger(budget.events) || !Number.isSafeInteger(budget.bytes) ||
      budget.events < 1 || budget.events > RETRY_SCIENCE_MAX_EVENTS ||
      budget.bytes < 0 || budget.bytes > RETRY_SCIENCE_MAX_BYTES) return "unavailable";
  const rows = (await science.prepare(db, row.problem_id, through, 1,
    { claimId: claim, version: Number.MAX_SAFE_INTEGER }).all<ScientificRow>()).results;
  if (!Array.isArray(rows) || rows.length !== budget.events || rows.length > RETRY_SCIENCE_MAX_EVENTS)
    return "unavailable";
  const firing = rows.find((event) => event.event_id === row.event_id);
  if (!firing || firing.claim_id !== claim || firing.seq !== row.seq ||
      firing.payload_sha256 !== row.payload_sha256 || firing.payload_json !== row.payload_json ||
      !Number.isSafeInteger(firing.target_version) || firing.target_version < 1) return "unavailable";
  ...
  const pins = rows.filter(event => event.payload_json !== null).map(event => ({
    event_id: event.event_id, digest: event.payload_sha256, body: event.payload_json,
  }));
  const available = await db.prepare(`SELECT COUNT(*) AS n FROM json_each(?) pin
    ...
    .bind(JSON.stringify(pins), row.problem_id, through).first<{n: number}>();
  if (available?.n !== pins.length) return "unavailable";
  return before.disposition !== reaches && after.disposition === reaches &&
    after.currentVersion === firing.target_version && current.currentVersion === after.currentVersion &&
    current.disposition === reaches ? "holds" : "not-held";
}
