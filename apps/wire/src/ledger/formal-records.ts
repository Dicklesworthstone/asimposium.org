...
export async function formalRecordPayload(text: string | null, digest: string): Promise<Record<string, unknown> | null> {
  if (typeof text !== "string" || text.length > FORMAL_RECORD_MAX_BYTES ||
      /^[0-9a-f]{64}$/.exec(digest)?.[0] !== digest) return null;
...
export async function readFormalRecords(
  ...
  for (const row of rows.slice(0, FORMAL_RECORD_PAGE_SIZE)) {
    if (!Number.isSafeInteger(row.seq) || row.seq <= previous || row.seq > cursor ||
        !exact(/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/, row.event_id) || seen.has(row.event_id) ||
    ...
    if (!payload) { omitted.add("content_unavailable"); continue; }
    if (row.object_version !== 1) { omitted.add("unsupported_record"); continue; }
    let kind: FormalRecord["kind"], content: FormalRecord["content"], target: FormalRecord["target"];
    if (row.event_type === "evidence.created") {
      if (!exact(/^E-[A-Za-z0-9]+$/, row.object_id) ||
          (payload.evidence_id !== undefined && payload.evidence_id !== row.object_id)) {
        omitted.add("unsupported_record"); continue;
      }
      if (payload.kind !== "formalization-friction" && payload.formal_artifact == null) continue;
      const evidence = decoders.evidence(payload);
      if (!evidence || evidence.bears_on_kind !== "claim" ||
      ...
      if (!exact(/^R-[A-Za-z0-9][A-Za-z0-9._:-]{0,125}$/, row.object_id) ||
          (payload.review_id !== undefined && payload.review_id !== row.object_id)) {
        omitted.add("unsupported_record"); continue;
      }
      if (payload.verification == null) continue;
      const review = decoders.review(payload);
      if (!review?.verification) { omitted.add("unsupported_record"); continue; }
      content = review;
      kind = "verification-report";
      target = { claim_id: review.target_claim_id, version: review.target_version };
    } else { omitted.add("unsupported_record"); continue; }
    if (!exact(/^C-[0-9]+$/, target.claim_id) || !Number.isSafeInteger(target.version) || target.version < 1) {
      omitted.add("unsupported_record"); continue;
    }
    const { event_type: _type, object_version: _version, payload_json: _bytes, ...publication } = row;
