  ...
  const response = validatedProblem(
    kind === "query"
      ? {
          status: 400,
          code: "CURSOR_INVALID",
          title: "Choose a valid proof-gap snapshot",
          detail:
            "Use one canonical through cursor, and either one after filing cursor or one exact target=G-n. Unknown and repeated parameters are refused.",
          fixHint:
            "Follow the next link unchanged, or omit parameters for the current gap history.",
          rule: "A5",
          extensions: {
            schema: `${PROOF_GAPS_SCHEMA_ID}#/properties/query`,
      ...
      : kind === "not-found"
        ? {
            ...
            rule: "A5",
            extensions: {
              schema: `${PROOF_GAPS_SCHEMA_ID}#/properties/response`,
        ...
        : {
            status: 500,
            code: "INTERNAL_ERROR",
            title: "Proof-gap history is unavailable",
...
export function createProofGapRoutes(): Hono<{ Bindings: Env }> {
      ...
      try {
        ...
        const body =
          format === "json"
            ...
            : renderProjection(
                proofGapsProjection(result.face),
                format === "html" ? "html-fragment" : "md",
              ).body;
        return await proofGapResponse(c.req.raw, body, format, result.face, result.unlisted);
      } catch (error) {
        return refusal(
          error instanceof ProofGapReadError ? error.code : "unavailable",
          c.req.method,
        );
      }
    });
  ...
  app.on(["GET", "HEAD"], "/p/:id/gaps", (c) => {
    if (!ProblemIdSchema.safeParse(c.req.param("id")).success)
      return refusal("query", c.req.method);
