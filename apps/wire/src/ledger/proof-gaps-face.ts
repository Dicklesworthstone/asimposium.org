  ...
  return {
    ...
    items: face.gaps.map((gap) => ({
      ...
      body: JSON.stringify({
        ...
        target_read_url: gap.content === null ? null :
    ...
    omitted: [
      ...
      ...(face.gaps.length === 0 && face.omitted.length === 0
        ? [{ reason: "no_gaps_in_range", detail: "No published gap filings match this requested range or target." }]
        : []),
    ],
    next_actions: [
      ...
      { method: "GET", url: proofGapPath(face.problem_id, "json", face.cursor, face.target, face.after),
        why: "Read the same structured records as JSON." },
      { method: "GET", url: `/p/${face.problem_id}/gaps.md`,
...
export async function proofGapResponse(
  ...
  const headers = new Headers({
    "content-type": format === "json" ? "application/json; charset=utf-8"
  ...
  if (format === "html") headers.set("content-security-policy",
    "default-src 'none'; base-uri 'none'; frame-ancestors 'none'; form-action 'none'");
  if (face.next_after !== null) headers.append("link",
