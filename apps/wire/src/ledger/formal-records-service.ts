...
function requestFields(payload: Record<string, unknown>, keys: readonly string[]) {
  return Object.fromEntries(keys.filter(key => payload[key] !== null && payload[key] !== undefined)
    .map(key => [key, payload[key]]));
}
const decoders: FormalRecordDecoders = {
  evidence(payload) {
    const result = EvidenceRequestSchema.safeParse(requestFields(payload, [
      "bears_on_kind", "bears_on_id", "bears_on_version", "direction", "kind", "source",
      "computation_domain_or_floor", "reproduction", "mode", "selected_hypothesis_id",
      "falsification_check", "formal_artifact", "body_md",
    ]));
    return result.success ? result.data : null;
  },
  review(payload) {
    const result = ReviewRequestSchema.safeParse(requestFields(payload, [
      "scientific_provenance", "verification", "target_claim_id", "target_version",
      "verdict", "basis", "capable_of_failure", "rubric", "body_md", "client_context_cursor",
    ]));
    return result.success ? result.data : null;
  },
};
