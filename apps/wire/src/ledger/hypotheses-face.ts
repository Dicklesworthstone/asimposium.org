...
export function hypothesisPath(
  problem: string,
  format: HypothesisFace,
  cursor: number,
  after: number,
): string {
...
export function hypothesesProjection(face: HypothesesResponse): Projection {
  ...
  return {
    schema: "asimposium.hypotheses.v1",
    kind: "hypotheses",
    profile: "hypotheses",
    ...
    items: face.hypotheses.map((item) => ({
      ...
      why_included:
        "Recorded route and lifecycle at the captured public cursor; null content is unavailable, not an empty argument.",
    })),
    omitted: [
        ...
        detail:
          reason === "page_limit"
            ...
            : reason === "content_unavailable"
              ? "Some committed work products cannot be served."
              : "An unsupported or inconsistent lifecycle is not treated as active.",
      })),
      ...(face.hypotheses.length === 0 && face.omitted.length === 0
        ? [
            {
              reason: "no_hypotheses_in_range",
              detail: "No published hypothesis admissions occur in this requested range.",
            },
          ]
        : []),
    ],
    next_actions: [
      ...(next === null
        ...
        : [
            {
              method: "GET" as const,
              url: hypothesisPath(face.problem_id, "md", face.cursor, next),
              why: "Continue through the same hypothesis snapshot.",
            },
          ]),
      {
        method: "GET",
        url: hypothesisPath(face.problem_id, "json", face.cursor, face.after),
        why: "Read the same typed hypothesis records as JSON.",
      },
      {
        method: "GET",
        url: `/p/${encodeURIComponent(face.problem_id)}/hypotheses.md`,
        why: "Restart at the latest public snapshot.",
      },
    ],
...
export async function hypothesisResponse(
  ...
  if (face.next_after !== null)
    headers.append(
      "link",
      `<${hypothesisPath(face.problem_id, format, face.cursor, face.next_after)}>; rel="next"`,
    );
