...
function references(value: unknown): EvidenceReference[] {
  if (!Array.isArray(value) || value.length > REFERENCE_LIMIT)
  ...
  return value.map((ref: unknown) => {
    if (
      ...
      !EVIDENCE_ID.test(ref.evidence_id) ||
      typeof ref.digest !== "string" ||
      !DIGEST.test(ref.digest) ||
      seen.has(ref.evidence_id)
    )
      throw new EvidenceGroundingError("invalid");
...
export async function readEvidenceGrounding(
  db: D1Database,
  problemId: string,
  claim: { readonly claimId: string; readonly version: number },
  ...
  const result = await db
    .prepare(EVIDENCE_GROUNDING_SQL)
    .bind(problemId, JSON.stringify(roots))
    .all<Row>();
  if (!result.success) throw new EvidenceGroundingError("unavailable");
  if (result.results.length > EVIDENCE_GROUNDING_NODE_LIMIT)
    throw new EvidenceGroundingError("limit");
  ...
  for (const row of result.results) {
    if (
      ...
      row.payload_sha256 !== row.expected_sha256
    )
      throw new EvidenceGroundingError("unavailable");
    ...
    if ((await sha256(row.payload_json)) !== row.expected_sha256)
      throw new EvidenceGroundingError("unavailable");
    let payload: unknown;
    try {
    ...
    resolved.set(row.evidence_id, {
      eventId: row.event_id,
      payloadDigest: row.expected_sha256,
  ...
  for (const source of resolved.values()) {
    for (const pin of dependencies(source.payload)) {
      ...
      if (
        !dependency ||
        pin.digest !== `sha256:${dependency.payloadDigest}` ||
        dependency.sequence >= source.sequence
      )
        throw new EvidenceGroundingError("invalid");
    }
  }
  return {
    roots: roots.map((pin) => {
      ...
      if (!source || pin.digest !== `sha256:${source.payloadDigest}`)
        throw new EvidenceGroundingError("unavailable");
      return source;
    }),
    witnesses: [...resolved.values()].map(
