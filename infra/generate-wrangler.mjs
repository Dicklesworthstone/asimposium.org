...
export function renderEnvironment(name, environment, policy) {
  ...
  // declares it (staging and production for screening). An environment
  // without the table must not grow one here: screening fails closed on a
  // missing binding, and a phantom entry would hide that refusal.
  if (environment.ai) {
