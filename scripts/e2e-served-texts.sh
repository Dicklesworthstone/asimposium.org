#!/usr/bin/env bash
# Served Texts E2E Gate (W6.5, bead asimposiumorg-3bq).
# Proves cold-cache reads, exact ETag/304 conditional GETs, canonical Link headers,
# protocol GFM/JSON semantic parity, review rubrics, move templates, schema index,
# apex static copies, and 308 redirects.
set -euo pipefail
...
reproduce="bash scripts/e2e-served-texts.sh"
...
# Run the served texts E2E test engine
if ! bun scripts/suite/served-texts-e2e.ts; then
  e2e_emit_and_optionally_record "$write_artifacts" "$run_id" "$suite" "$started_ms" "fail" "SERVED_TEXTS_E2E_ASSERTION_FAILED" "$reproduce"
  exit 1
fi

e2e_emit_and_optionally_record "$write_artifacts" "$run_id" "$suite" "$started_ms" "pass" "SERVED_TEXTS_E2E_COMPLETE" "$reproduce"
exit 0
