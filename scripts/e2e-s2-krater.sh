...
if env S2_ORIGIN="${S2_ORIGIN}" S2_PHASE="exercise" bun apps/wire/src/krater/s2-client.ts; then
  :
else
  S2_CLIENT_EXIT=$?
  stop_worker
...
if ! start_worker; then
  emit "{\"tool\":\"wrangler\",\"tool_version\":\"${S2_WRANGLER_VERSION}\",\"package\":\"apps/wire\",\"suite\":\"s2-krater-local-d1\",\"status\":\"fail\",\"code\":\"LOCAL_WORKER_RESTART_UNAVAILABLE\",\"reproduce\":\"scripts/e2e-s2-krater.sh\"}"
  exit 1
fi
if env S2_ORIGIN="${S2_ORIGIN}" S2_PHASE="restart-verify" bun apps/wire/src/krater/s2-client.ts; then
  :
else
  S2_CLIENT_EXIT=$?
  stop_worker
  emit "{\"tool\":\"wrangler\",\"tool_version\":\"${S2_WRANGLER_VERSION}\",\"package\":\"apps/wire\",\"suite\":\"s2-krater-local-d1\",\"status\":\"fail\",\"code\":\"LOCAL_D1_RESTART_SCENARIO_FAILED\",\"reproduce\":\"scripts/e2e-s2-krater.sh\"}"
  exit "${S2_CLIENT_EXIT}"
fi
stop_worker
