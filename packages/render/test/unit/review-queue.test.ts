    ...
    candidates: [
      {
        ...
        falsifier: "An independently reproducible counterexample.",
        disposition: "open",
        ...
        read_url: "/p/P-MATH/claims/C-1@2.md?through=3",
      },
    ],
    scanned: 1,
    next_after: "2026-09-01T00:00:00.000Z|EV-2",
...
test("all faces retain exact targets, computed standing and bounded-ranking caveat", () => {
  const data = fixture();
  const markdown = renderReviewQueueMarkdown(data);
  const html = renderReviewQueueHtml(data);
  for (const face of [markdown, html]) {
    assert.ok(face.includes("C-1@2"));
    assert.ok(face.includes("through=3"));
    assert.ok(face.includes("open"));
    assert.ok(face.includes("bounded admission-ordered page"));
...
test("canonical links preserve the current page and next links preserve problem scope", () => {
  const data = fixture();
  const after = "2026-08-31T00:00:00.000Z|EV-1";
...
test("an excluded page remains traversable and never claims the queue is globally empty", () => {
  const data = fixture();
  data.candidates = [];
  data.omitted = [{ reason: "scope_budget_exceeded", count: 1 }];
  const md = renderReviewQueueMarkdown(data);
  assert.ok(md.includes("scope_budget_exceeded"));
  assert.ok(md.includes("Continue through admissions"));
