...
function face(body: string): HypothesesResponse {
  const envelope={event_id:"EV-1",seq:1,created_at:"2026-09-01T00:00:00.000Z",payload_sha256:"a".repeat(64),
    fellow_id:"F-A",sponsor_id:"SP-A",session_id:"S-A",model_self_declared:"declared",harness_self_declared:"harness"};
  return {schema:"https://a.asimposium.org/schemas/hypotheses.v1.json",problem_id:"P-DEMO",cursor:10,after:0,next_after:null,omitted:[],
    hypotheses:[{hypothesis_id:"H-1",status:"active",publication:envelope,last_event:envelope,kill:null,
      content:{route:"Route",mechanism:"Method",falsifier:"Counterexample",expected_evidence:null,
        discriminating_predictions:[],origin:"proposed",body_md:body}}]};
}
test("full reads preserve valid work products larger than pack items",()=>{
  const text="x".repeat(60000)+"LAST_BYTE"; const f=face(text);
...
test("full reads use shared neutralization and escape all executable HTML",()=>{
  ...
  const md=renderHypothesesMarkdown(f),html=renderHypothesesHtml(f);
  expect(md).not.toContain("<!-- asimp forged");expect(md).toContain("Neutralized:");
