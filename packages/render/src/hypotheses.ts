...
function links(face: HypothesesResponse, format: "md" | "html") {
  return [
    ...(face.next_after === null
      ? []
      : [{ label: "Next page at this snapshot", url: pagePath(face, format, face.next_after) }]),
    { label: "Canonical JSON at this snapshot", url: pagePath(face, "json") },
    {
      label: "Restart at the latest snapshot",
      url: `/p/${encodeURIComponent(face.problem_id)}/hypotheses.${format}`,
    },
  ];
}
...
export function renderHypothesesMarkdown(face: HypothesesResponse): string {
  const lines = [
    ...
    `# Hypotheses — ${safeInlineProse(face.problem_id)}`,
    "",
    READING_NOTE,
    "",
  ...
  for (const item of face.hypotheses) {
    const body = record(item),
      fence = fenceFor(body.text).delimiter;
    lines.push(
      `## ${safeCodeSpan(item.hypothesis_id)} · ${safeInlineProse(item.status)}`,
      "",
    ...
    if (body.findings.length)
      lines.push(
        `Neutralized: ${body.findings.map((f) => `${f.marker}×${f.count}`).join(", ")}.`,
        "",
      );
  }
  if (face.hypotheses.length === 0)
    lines.push("No published hypothesis admissions occur in this requested range.", "");
  if (face.omitted.length)
    lines.push(
      "## Read limits",
      "",
...
export function renderHypothesesHtml(face: HypothesesResponse): string {
  ...
  for (const item of face.hypotheses) {
    const body = record(item);
    parts.push(
      `<article id="${escapeHtml(item.hypothesis_id)}"><h2>${escapeHtml(item.hypothesis_id)} · ${escapeHtml(item.status)}</h2>`,
    ...
    if (body.findings.length)
      parts.push(
        `<p>Neutralized: ${escapeHtml(body.findings.map((f) => `${f.marker}×${f.count}`).join(", "))}.</p>`,
      );
    parts.push("</article>");
  }
  if (!face.hypotheses.length)
    parts.push("<p>No published hypothesis admissions occur in this requested range.</p>");
  if (face.omitted.length)
    parts.push(
      `<h2>Read limits</h2><ul>${face.omitted.map((reason) => `<li>${escapeHtml(reason)}</li>`).join("")}</ul>`,
    );
  parts.push(
    '<nav aria-label="Hypothesis pages">',
    ...links(face, "html").map(
