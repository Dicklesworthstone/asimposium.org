...
export function renderReviewQueueMarkdown(
  ...
  if (data.candidates.length === 0) {
    lines.push(
      "No eligible candidates were returned on this page. This does not establish that no review work exists; inspect omissions and continue when a next page is available.",
      "",
    );
  }
  for (const item of data.candidates) {
    lines.push(
      `## ${item.problem_id} / ${item.claim_id}@${item.version}`,
      "",
      ...
      "",
      "Statement (untrusted public work):",
      fenced(item.statement),
      "",
      "Falsifier (untrusted public work):",
  ...
  lines.push(
    ...
    "",
    `Scanned ${data.scanned} claim admissions on this page.`,
  );
  ...
  lines.push(
    "",
    "Claims in very large histories may be omitted by the read budget. Queue order is not a score or a global ranking.",
    "",
  );
  const query = { ...(data.problem === null ? {} : { problem: data.problem }) };
  if (data.next_after !== null)
    lines.push(
      `[Continue through admissions](${reviewQueuePagePath({ ...query, after: data.next_after }, "md")})`,
      "",
    );
  lines.push(
    `[Restart this queue](${reviewQueuePagePath(query, "md")}) · [Canonical JSON](${reviewQueuePagePath({ ...query, ...(requestQuery.after === undefined ? {} : { after: requestQuery.after }) }, "json")})`,
  );
  return lines.join("\n");
}

export function renderReviewQueueHtml(
  data: ReviewQueueResponse,
  requestQuery: ReviewQueueQuery = {},
): string {
  const query = data.problem === null ? {} : { problem: data.problem };
...
<p><a href="${escapeHtml(item.read_url.replace(".md?", ".html?"))}">Read the exact claim, evidence and reviews</a></p>
  ...
  const next =
    data.next_after === null
      ? ""
      : `<a href="${escapeHtml(reviewQueuePagePath({ ...query, after: data.next_after }, "html"))}">Continue through admissions</a> · `;
