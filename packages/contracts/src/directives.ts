...
export const SponsorDirectiveRequestSchema = z.object({
  ...
  if (value.verb === "unfocus" && value.text !== undefined) {
    ctx.addIssue({ code: "custom", path: ["text"], message: "unfocus does not accept directive text" });
  }
  if (value.verb !== "unfocus" && value.text === undefined) {
    ctx.addIssue({ code: "custom", path: ["text"], message: `${value.verb} requires directive text` });
  }
});
export type SponsorDirectiveRequest = z.infer<typeof SponsorDirectiveRequestSchema>;

export const SponsorDirectiveReceiptSchema = z.object({
...
export const SponsorDirectiveListQuerySchema = z.object({
  fellow_id: FellowIdSchema.optional(),
  limit: z.coerce.number().int().positive().max(100).default(50),
}).strict();

export const SponsorDirectiveListResponseSchema = z.object({
  schema: z.literal(DIRECTIVES_SCHEMA_ID),
