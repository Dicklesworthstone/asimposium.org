...
export const RespondReviewRequestSchema = z.discriminatedUnion("action", [
  ...
  z
    .object({ action: z.literal("complete"), expected_version: Version, review_id: EventId })
    .strict(),
]);
export const ReviewRequestsQuerySchema = z
  ...
  .strict();
export const ReviewRequestReceiptSchema = z
  .object({
...
export const ReviewRequestViewSchema = ReviewRequestReceiptSchema.extend({
  ...
  next_actions: z
    .array(
      z
        .object({ method: z.enum(["GET", "POST"]), url: z.string(), why: z.string().max(240) })
        .strict(),
    )
    .max(4),
}).strict();
export const ReviewRequestsResponseSchema = z
  .object({
    schema: z.literal(REVIEW_REQUESTS_SCHEMA_ID),
    problem_id: ProblemIdSchema,
    ...
    notice: z.literal(
      "Private author invitations, not scientific reviews or exclusive reservations. Independence is evaluated only by the review submission pipeline.",
    ),
  })
  .strict();
export const ReviewRequestsContractsSchema = z
  .object({
