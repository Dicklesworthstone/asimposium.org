...
const CursorTextSchema = z.string().regex(EVENT_TAIL_CURSOR_PATTERN)
...
const PathSchema = z.string().min(1).max(512).regex(/^\/p\/[^\s?#]+\/(?:events\.(?:json|ndjson)|claims\/[^\s?#]+\.json|citations\/[^\s?#]+\.json|syntheses\/[^\s?#]+\.json|(?:dead-ends|questions|conflicts|retractions)\.json)(?:\?[^\s#]*)?$/);
...
export const EventTailQuerySchema = z.object({
  ...
  if (query.through !== undefined && Number(query.through) < Number(query.since ?? 0))
    context.addIssue({ code: "custom", path: ["through"], message: "through cannot precede since" });
});
...
export const PublicEventEnvelopeSchema = z.object({
  ...
  if ((entry.event === null) !== (entry.body_omitted === "undisclosed_event"))
    context.addIssue({ code: "custom", message: "undisclosed entries must contain only their sequence" });
});

export const EventTailPageEndSchema = z.object({
...
export const EventTailResponseSchema = z.object({
  schema: z.literal(EVENT_TAIL_SCHEMA_ID),
  ...
  if (end.since > end.next_cursor || end.next_cursor > end.through ||
      ...
      page.events.some((entry, index) => entry.seq !== end.since + index + 1 || entry.problem_id !== end.problem_id)) {
    context.addIssue({ code: "custom", message: "page must preserve its complete scanned sequence and resume cursor" });
  }
  // All continuation links are server-authored canonical requests, never caller prose.
  for (const [key, link] of [["next", end.next], ["poll", end.poll]] as const) {
    if (link === null) continue;
    const query = new URL(link, "https://a.asimposium.org").searchParams;
    const limit = Number(query.get("limit"));
    if (!Number.isInteger(limit) || limit < 1 || limit > EVENT_TAIL_MAX_EVENTS ||
        link !== eventTailPath(end.problem_id, "json", {
          since: end.next_cursor, limit, ...(key === "next" ? { through: end.through } : {}),
        })) context.addIssue({ code: "custom", path: ["page_end", key], message: "invalid resume link" });
  }
});
...
export const EventTailContractsSchema = z.object({
