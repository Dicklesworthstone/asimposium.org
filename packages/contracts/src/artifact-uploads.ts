...
export const ARTIFACT_UPLOADS_SCHEMA_ID = "https://a.asimposium.org/schemas/artifact-uploads.v1.json";
...
export const ArtifactDeclareRequestSchema = z.discriminatedUnion("encoding", [
  z.object({ ...identity, encoding: z.literal("text"), size_bytes: z.number().int().min(1).max(5 * 1024 * 1024) }).strict(),
  z.object({ ...identity, encoding: z.literal("lake-archive"), size_bytes: z.number().int().min(1).max(20 * 1024 * 1024) }).strict(),
]);
...
const common = {
  schema: z.literal(ARTIFACT_UPLOADS_SCHEMA_ID), upload_id: ArtifactUploadIdSchema,
  sha256: ArtifactDigestSchema, size_bytes: z.number().int().min(1).max(20 * 1024 * 1024),
  encoding: z.enum(["text", "lake-archive"]), storage: z.literal("private"),
...
export const ArtifactDeclareResponseSchema = z.object({
  ...common, status_path: uploadPath, complete_path: completePath,
  put: z.object({
    method: z.literal("PUT"),
    url: z.string().max(4096).regex(/^https:\/\/[a-f0-9]{32}\.r2\.cloudflarestorage\.com\/[a-z0-9-]{3,63}\/incoming\/artifacts\/AU-[a-f0-9]{32}\?X-Amz-Algorithm=AWS4-HMAC-SHA256&/),
    expires_at: timestamp,
    headers: z.object({ "content-type": z.literal("application/octet-stream"),
      "content-length": z.string().regex(/^[1-9][0-9]{0,7}$/), "if-none-match": z.literal("*") }).strict(),
  }).strict(),
}).strict();
...
export const ArtifactStatusResponseSchema = z.discriminatedUnion("state", [
  z.object({ ...common, ...pending, state: z.literal("presigned"), verification: z.literal("not-verified") }).strict(),
  z.object({ ...common, ...pending, state: z.literal("quarantined"), verification: z.literal("not-verified") }).strict(),
  z.object({ ...common, ...pending, state: z.literal("expired"), verification: z.literal("not-verified") }).strict(),
  z.object({ ...common, state: z.literal("verified"), verification: z.literal("bytes-only"),
    content_type: z.enum(["text/plain; charset=utf-8", "application/gzip"]),
    verified_at: timestamp, content_path: contentPath }).strict(),
]);
export type ArtifactStatusResponse = z.infer<typeof ArtifactStatusResponseSchema>;

export const ArtifactUploadsContractsSchema = z.object({
  declare_request: ArtifactDeclareRequestSchema, declare_response: ArtifactDeclareResponseSchema,
  complete_request: ArtifactCompleteRequestSchema, status_response: ArtifactStatusResponseSchema,
