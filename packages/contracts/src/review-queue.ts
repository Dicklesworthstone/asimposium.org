import { z } from "zod";
import { ClaimDispositionSchema, ProblemIndexTimestampSchema, PublicLedgerProblemIdSchema } from "./ledger.ts";
...
export const ReviewQueueQuerySchema = z.object({
  ...
  try { parseReviewQueueAfter(query.after); }
  catch { context.addIssue({ code: "custom", path: ["after"], message: "invalid review queue continuation" }); }
});
...
export const ReviewQueueResponseSchema = z.object({
  ...
  omitted: z.array(z.object({
    reason: z.enum(["content_unavailable", "not_review_ready", "scope_budget_exceeded", "page_limit"]),
  ...
  for (const [index, item] of face.candidates.entries()) {
    ...
    if (seen.has(identity) || (face.problem !== null && face.problem !== item.problem_id) || item.read_url !== path) {
      context.addIssue({ code: "custom", path: ["candidates", index], message: "inconsistent review target or canonical link" });
    }
    seen.add(identity);
  }
  if (face.candidates.length > face.scanned) context.addIssue({ code: "custom", message: "inconsistent scan accounting" });
  try { parseReviewQueueAfter(face.next_after ?? undefined); }
  catch { context.addIssue({ code: "custom", path: ["next_after"], message: "invalid continuation" }); }
});
export type ReviewQueueResponse = z.infer<typeof ReviewQueueResponseSchema>;
export const ReviewQueueContractsSchema = z.object({ query: ReviewQueueQuerySchema, response: ReviewQueueResponseSchema }).strict();
