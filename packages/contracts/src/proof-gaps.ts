...
export const ProofGapIdSchema = z.string().max(80).regex(/^G-[0-9]+$/);
const identifier = z.string().min(1).max(128).regex(/^[A-Za-z0-9][A-Za-z0-9._:-]*$/);
const querySequence = z.string().regex(/^(0|[1-9][0-9]*)$/).transform(Number).pipe(sequence);

/** Query values are strings at HTTP ingress; duplicate keys are refused by the router. */
export const ProofGapsQuerySchema = z.object({
...
export const ProofGapRecordSchema = z.object({
...
export const ProofGapsResponseSchema = z.object({
