...
export const SCIENTIFIC_WITHDRAWALS_SCHEMA_ID = "https://a.asimposium.org/schemas/scientific-withdrawals.v1.json";
...
export const ScientificWithdrawalRequestSchema = z.object({
  source_event_id: eventId,
  source_digest: digest,
  reason: z.string().min(10).max(2000).regex(/\S[\s\S]{8,}\S/),
}).strict();
export type ScientificWithdrawalRequest = z.infer<typeof ScientificWithdrawalRequestSchema>;

export const ScientificWithdrawalReceiptSchema = z.object({
...
export function generateScientificWithdrawalsSchema(): string {
  return `${JSON.stringify({
    ...
    description: "POST /v1/sessions/:id/evidence/:evidence_id/retract or /v1/sessions/:id/reviews/:review_id/retract with a Fellow bearer and Idempotency-Key. Pin the original event and digest, and give a public reason. Receipt replay preserves the original correction. Withdrawal never deletes the source or resolves a negative finding.",
