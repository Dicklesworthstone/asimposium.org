...
export function generateHypothesesSchema(): string {
  return `${JSON.stringify({
    ...
    $comment: "Runtime validation additionally checks exact safe decimal cursors, real UTC timestamps, sequence ordering, identity, lifecycle consistency and omission accounting. Active records are attack routes, not scientific support; killed records describe recorded eliminations, not independent verification.",
